...
**Version:** v3.2.6 (RESUME-state work plan; focused status update. The version label is
kept at the "3.2" generation to stay aligned with Document A v3.2; the `.6` patch level
marks this update. Records the **entire scalar pixel pipeline as proven/committed** — P.3A
weighted blend, P.4A downsampled-luma, P.5A signed-difference bridge, and P.6A chroma-plane
traversal — on top of P.1A/P.2A and the C.14A cache-core milestone; selftest count is now
**38/38**. Notes the deliberate roadmap re-letter (P.6A is chroma-plane traversal, NOT the
original "scene-change"), the settled accuracy rule, and the P.4A edge-clamp divergence. Sets
the **real-frame-buffer traversal / VapourSynth integration** phases as next. See §8. Earlier
sections carry forward except the version pointers below.)  
**Date:** 2026-06-19  
...
NEXT — the scalar pixel pipeline is complete; the remaining phases cross into real VapourSynth
machinery (proof surface shifts from scalar reference vectors to real/synthetic frame buffers):
    1. Source-luma downsample as a REAL BUFFER PASS — apply P.4A's proven scalar shape over the
       luma plane to PRODUCE the downsampled-luma planes that P.6A consumes (currently the only
       pixel piece still in scalar-only form).
    2. Real 8/16-bit FRAME-BUFFER ACCESS — the actual reinterpret_cast<const uint16_t*> stride
       handling on real frame memory (P.6A proved the stride LOGIC on int buffers; this is the
       real-memory version, where 16-bit alignment/stride from the Part 6B checklist finally bites).
    3. SCENE-CHANGE accumulation (the deferred original "P.6A" content) — diff_total accumulates
    ...
    4. VapourSynth getFrame integration (arInitial/arAllFramesReady), source request/retrieve
       lifecycle, return-transfer — where the proven CACHE CORE (through C.14A) finally connects to
       the proven PIXEL KERNEL (through P.6A). The predecessor is the explicit previous FILTERED
    ...
    5. cnr3_frame_processing.cpp membership in the cnr3 PLUGIN project — it is in the selftest
       project (from P.3A); the real-frame traversal and plugin build will need it in the plugin
       project too. Confirm before phase 1/2, via the VS2026 GUI (Add Existing Item), to avoid the
       link error the selftest hit at P.3A.
    6. Visual Studio 2026 project wiring for the full plugin build.

    NOTE on the arc shift: the remaining phases leave the isolated cache-core selftest harness and
    move into pixel maths over real buffers and live VapourSynth interaction. The PDAP delivery
    process, read-first review for load-bearing work, genuine-failure-mode tests, and explicit-known
    expected values all still apply, but the proof surface changes (real/synthetic frame-buffer
    proofs and VS-integration proofs rather than scalar reference vectors). PROCESS NOTE: P.1A was
    reviewed read-first POST-COMMIT (sound — low-risk pure maths); every phase since (P.2A–P.6A) was
    reviewed read-first BEFORE apply, with every reference vector independently recomputed by the
    designer and (from P.3A) cross-checked against the fetched vsCnr2.cpp source. From here, and for
    anything touching cache-core/lifecycle, read-first-BEFORE-apply remains firm. See the
    Designer/Reviewer Role Handover (v1.3) for the full review discipline, the pixel-arc reference,
    the accuracy rule, and the review checklist.
```
