# CNR3 — PATCH SCOPE: HALF-500 cache profile + plan-retry rename (single patch) — v4
...
### A.3 Primitive knobs — three-way, HALF starts with TWO deliberate differences from NORMAL (ceiling + CR4-companion zone count)
...
rule and its 2x factor INTACT (preferred over relaxing the assert). IMPLEMENT OPTION 1 ONLY first and
compile it (it was cold-verified against all 14 static_asserts by W3D). Options 2/3 are FALLBACK PATHS,
to be evaluated only if Option 1 fails real compilation or static_assert verification:
...
`#elif defined(CNR3_CACHE_PROFILE_HALF)` branch with HALF's expected values:
    profile_name "half-500"; active_ceiling_max 500U; **max_hot_zones 3U** (NOT NORMAL's 5U);
    all other unchanged primitive values matching NORMAL; and the changed derived values
    max_protected 228U and grid_floor 15U (see A.4; expecting 348/25/5 would fail the 4-way).
Without this branch the 4-way FAILS under HALF. This selftest is the proof that config and
...
Confirm-report MUST include a whole-tree grep proving (GREP TARGET = the SOURCE TREE being patched:
src/*.h, src/*.cpp, and any build files — NOT archived design notes / historical patch-notes documents,
which legitimately retain the old name as history):
- zero occurrences of `CNR3_EXPERIMENT_PLAN_RETRY_BIAS` remain in the source tree (incl. source comments);
