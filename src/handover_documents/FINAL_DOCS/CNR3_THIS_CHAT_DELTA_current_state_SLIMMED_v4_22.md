# CNR3 — THIS-CHAT DELTA: current-state companion (SLIMMED, through marshalling levers 0A/0B/3a.1/3b.1/3a.2 + validation policy + A-lite + C1 + Repack + F/3c) — v4.22

**Version:** v4.22 (SLIMMED). Supersedes v4.21. Extends the marshalling-optimisation arc with Lever Repack (row-memcpy commit) and Lever F/3c (blend inline+hoist). All steps value-preserving (56/56 four-way, P-series unchanged) and profiled; CMS UNCHANGED (still 07.15 — implementation optimisation only, no design/invariant change). Cumulative vs pre-lever baseline: **93,914 -> ~23,600 samples (~-75%)**.
...
- **Ahead (post-F/3c landscape — top leaf shifted again):** the NEW top leaf is the **repack-STAGING conversion** `cnr3_stage_scalar_plane_to_native_bytes` (~4,753 total / ~203 self — ~20%, the extreme low-self/high-total per-sample-call-chain signature). This is the scalar-int -> native-byte CONVERSION (narrow + clamp) — ARITHMETIC-bearing, so it needs its own investigate-and-scope (narrow/clamp discipline like 3a.1/3a.2, NOT a memcpy). = the clear NEXT lever (STAGING FOLLOW-ON), and the LAST big call-chain leaf. After that, diminishing returns: `cnr3_copy_native_plane_to_scalar_buffer` ~1,627, `cnr3_downsample_native_luma_plane_to_scalar_chroma_grid` ~1,300 (C1, stable), and `cnr3_allocate_scalar_plane_storage` ~923 (allocation — **Lever B / pooling** now worth doing, a lifetime/thread-safety proof under fmUnordered). **D** (EXACT SIMD downsample, NOT biased VPAVGB) and **E** (scene-change local accumulator) remain cheap-but-small for cumulative fps. Parked: diagnostics D-SUM menu, selftest skip-pass fix.
...
```text
Committed/pushed through:  CNR3-OPT-LeverF3c-blend-inline-hoist  (marshalling arc: AVX2 + 0A + 0B + 3a.1 + 3b.1 + 3a.2 + A-lite + C1 + Repack + F/3c)
...
Marshalling arc status:    0A -28% | 0B flat | 3a.1 -37% | 3b.1 flat | 3a.2 flat-on-8bit | A-lite -7.4% | C1 -24.5% | Repack -4% | F/3c -15.7%. cumulative ~-75% (93,914 -> ~23,600). F/3c inlined the blend call chain (bit-exact int64 arithmetic, cross-verified; validation pre-pass preserves no-partial-output); blend leaf ~6,640 collapsed off top children. NEW TOP LEAF = repack-STAGING conversion (cnr3_stage_scalar_plane_to_native_bytes ~4,753 / 203 self — per-sample call chain, ARITHMETIC narrow+clamp). NEXT: staging follow-on (investigate-and-scope, like 3a.1). Then B (pooling, ~923 alloc leaf now worth it), D/E cheap-but-small. Unpack/downsample/blend all now DONE.
