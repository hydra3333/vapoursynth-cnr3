# CNR3 Handover Snapshot v0.9 - Structural Split Checkpoint and Cache v005 Direction
...
This v0.9 snapshot is intentionally detailed. Do not compress it aggressively in a future handover unless the relevant detail has been superseded by code or by a later explicit decision.
...
- The v005 cache direction remains design-only; it is not implemented in current code.
```
...
Recommended next sequence from this v0.9 checkpoint:

```text
1. Commit/save this v0.9 handover update.
2. Confirm the current source/build checkpoint is committed and pushed.
3. Confirm GitHub Actions passes with all current .cpp files:
       src\vapoursynth-Cnr3.cpp
       src\cnr3_cache.cpp
       src\cnr3_response_tables.cpp
4. Confirm Visual Studio Release x64 still builds locally.
5. Preserve one final known-good strict-mode regression log before cache work.
6. Do not do more non-cache splitting unless a specific pain point appears.
7. Begin a cache implementation planning pass based on `cnr3_cache_concept_v005.md`.
8. Start cache implementation only after agreeing the first small phase.
```

Non-cache work remaining before v005 cache implementation is mostly housekeeping:

```text
- update handover/checkpoint documentation;
- confirm Release x64 and GitHub Actions baseline;
- preserve a known-good regression log;
- optionally tag a pre-cache-manager structural baseline;
- avoid further algorithm or file-layout churn unless clearly justified.
```

Do not begin functional cache-manager coding merely because the v005 design exists. The first implementation phase should be agreed explicitly and kept small.
...
Please read CNR3_Handover_Snapshot_v0.9_cache_v005_direction.md and continue from there. The current implementation has real recursive chroma blending, shared luma buffers, vscnr2-style scene-change detection, VS2026 local build, verified GitHub Actions build, and two completed no-program-logic-change structural splits:
...
The accepted draft cache direction is now `cnr3_cache_concept_v005.md`. It targets fmParallelRequests first, uses a per-instance output-frame cache, checkpoint pool, pin_count protection between arInitial and arAllFramesReady, and ascending chain-walk hole filling. It supersedes the earlier speculative/not-ready/cascade-drain framing for implementation planning.

Do not start cache-manager coding until the first small implementation phase is explicitly agreed. Preserve the user's patch style: explain what changes and why, show prior/replacement blocks with enough context, avoid unnecessary changes, and keep comments maintainable.
```
