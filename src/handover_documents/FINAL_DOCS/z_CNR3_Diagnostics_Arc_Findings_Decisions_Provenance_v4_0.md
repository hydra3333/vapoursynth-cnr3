...
**Version:** v4.0 (supersedes v3.0)

*** v4.0 ADVANCE (2026-07-18): the ship cycle — rider, README, build hygiene, production config, CI. ***

## DECISIONS (W3X rulings, dated)

- **[2026-07-16] Ship the DLL SILENT.** `CNR3_EMIT_PLUGIN_STARTUP_PROVENANCE` is gated like a diagnostic
  and OFF in ship builds: "other DLLs do not emit versions etc". The provenance lines return in dev builds
  with the master gate. (The designer had argued to keep them always-on; W3X's call, and consistent with
  ecosystem norms.)
- **[2026-07-16] The master diagnostics gate uses PER-ITEM WRAPS.** Two designer drafts were rejected in
  sequence: a central `#undef` list ("will too easily get out of sync") and a single long wrapping region
  ("a maintainer could put something outside a single long, hard-to-ascertain #ifdef"). The adopted design
  wraps EVERY gate individually so the pattern is visible at every site and the containment rule is
  self-evident. Master lives at the TOP, above filter-mode selection. -> R-BUILD-01.
- **[2026-07-16] Await timeout = 10s and a LOUD CLIP-FAIL** (reservation-table design): "very few scripts
  run at 0.1 fps", so a 10s wait means pathology (leaked claim, deadlock, dying machine) and deserves a
  loud, diagnosable failure rather than a silent fallback. -> proposal D-R5.
- **[2026-07-17] CI builds with an EXPLICIT cl command, not msbuild.** Rationale: "I would unknowingly
  muck it up, and probably so would a future maintainer, when fiddling with VS2026 and not knowing it had
  knock-on implications building here"; VS2026's settings are obscure/invisible. Mitigation: a FLAG MAP
  comment ties every cl flag to its vcxproj setting, and the source list is a wildcard minus one file so
  it cannot fossilize.
- **[2026-07-17] Security flags KEPT in Release** (`/sdl`, `/GS`) despite the speed goal; `/fp:precise`
  explicit and never `/fp:fast`; `/OPT:NOICF` deliberate (speed over size); `/Gy`+`/Zc:inline`+`/OPT:REF`
  added.
- **[2026-07-17] No selftest in CI** — the maintainer's responsibility in VS2026.
- **[2026-07-17] `VS_USE_LATEST_API` NOT defined** — API 4.0 profile for maximum core compatibility.
  W3X asked the right question ("is v4.2 compatible?") and the verified answer is that the macro is an
  offer, not a requirement. -> R-BUILD-02.
- **[2026-07-18] dev_cache_manager MERGED AND RETIRED** — work continues on main.

## FINDINGS (evidence, banked)

- **The rider's confirm pass found THREE defects in the DESIGNER's scope, with probe evidence:**
  (D1) `%g` renders 100.0001 as "100" — defeating the scope's own stated rationale for choosing it;
  (D2) `%.32s` is a MAXIMUM, not a bound — it scans for NUL and reads past non-NUL-terminated map data
  (AddressSanitizer stack-buffer-overflow probe), and the parser's OWN literal matcher documents that
  both NUL and non-NUL forms occur; (D3) echoing raw user text contradicted the one-line requirement.
  **Two of the three would have shipped a defect had the coder complied.** Rulings D1 (std::to_chars
  shortest round-trip — strictly better than the coder's own %.17g suggestion), D2 (value_size-bounded
  byte loop), D3 (sanitise to printable ASCII + truncation marker).
- **VapourSynth's Python binding type-checks BEFORE the plugin:** `typedDictToMap`/`rangeToIntFilter`
  rejects int/float/bool type mismatches, so the plugin's own type-error branches for those kinds are
  unreachable from Python (they remain correct as C-API defence). Data-typed params (curves) DO reach the
  plugin — `y_curve=123` produced `got "123"`.
- **`cnr3_memory_diagnostics.cpp` was safe from the windows.h min/max macros only by include-order luck**
  — it defined NOMINMAX locally before its windows.h include. Project-wide NOMINMAX makes it
  correct-by-construction; the file's local defines are now `#ifndef`-guarded.
- **CNR3 is property-transparent:** every output frame is created via `copyFrame(source_frame)` (all four
  paths verified: fresh-start, floor, hole, ordinary compute), so ALL frame properties flow through
  regardless of compiled API level. The 4.2 `_Range` obligation binds only filters that declare 4.2 AND
  handle those properties.
- **`windows-latest` = WS2025 + VS2026 (v145)** since the June 2026 GitHub image migration; free for
  public repos; the old `windows-2025-vs2026` label is now an alias.

## PROCESS PRECEDENTS ADDED THIS CYCLE

- **An enriched, self-contained scope (verified facts + named traps + due process + proof gate) let a
  fresh memoryless coder implement flawlessly AND catch the designer's errors.** The traps were the
  load-bearing part; the most valuable (TRAP 1) came from the designer re-reading source rather than
  trusting an earlier scope.
- **Harness detours dominate gate time.** The 17/17 run took six attempts — mangled paste, wrong vspipe
  path (exit 9009), an over-generous pass criterion scoring launch failures as passes, and a `.vpy`
  recycled during a tidy-up. **The log file ended it every time.** -> R-PROCESS-35.
- **Verify by content, not by IDE:** VS2026 required a restart before acknowledging externally-edited
  project settings; `findstr` was the arbiter.
- **W3X's design instincts overrode the designer twice on the master gate, correctly both times** — the
  drift-proneness of an #undef list and the invisibility of a long region are exactly the failure modes
  that bite maintainers years later.

*** end v4.0 advance ***


**Version:** v3.0 (supersedes v2.0)

*** v3.0 ADVANCE (2026-07-15): five arcs banked — fmParallel diagnosis, plan-retry experiment, HALF-500
profile, the chroma-desaturation FIX, and the option parser. DECISIONS + FINDINGS: ***

## DECISIONS (W3X rulings, dated)

- **[2026-07-13] S=50 for CNR3_PLAN_RETRY_SLEEP_MS** — chosen at the efficiency knee, not the fps peak.
  S=30-40 maximises throughput (~178 fps, ~4700 dup); S=50 halves duplicates (~2100) for ~29% fps. Rationale:
  on real footage each wasted recompute is an expensive frame, so the waste-minimising side is the safer
  default. Recorded WITH its alternative so the tradeoff travels with the number.
- **[2026-07-14] HALF-500 uses 3 hot zones, not 5** — W3X's conjecture ("can it do as good a job at half
  size?") hit CR4 (ceiling >= ~2x max-protected = 696 at 5 zones). W3X chose to reduce zones rather than relax
  the assert: 3*(50+10)+48 = 228 -> CR4 wants >=456 -> 500 fits, **rule intact**. Preferred over a documented
  1.4x relaxation. Validated: re-churn 0 at both sizes.
- **[2026-07-14] HALF is ENABLED in the ship build** (not opt-in). Rationale: W3X builds the PyPI DLL;
  ~99.999% of users never rebuild, so the compile-time gates ARE the product configuration, and the smaller
  footprint is proven free.
- **[2026-07-14] SHIP MODE = fmParallelRequests** — on evidence, not preference: 337 fps / 1 duplicate vs
  fmParallel's 126 fps / 2342 duplicates. fmParallel's 2% edge over Requests (when Requests was throttled by
  plan-retry) evaporated once plan-retry was correctly turned off.
- **[2026-07-14] CNR3_ENABLE_PLAN_RETRY_BIAS OFF in the ship build** — from W3X's question "with
  fmParallelRequests there is no benefit to plan-retry?" This overturned the designer's "leave it on, it's
  free" advice: measurement showed a 2.7x THROTTLE. -> R-ARCH-08.
- **[2026-07-14] Option names are DESCRIPTIVE, not cnr2's terse names** — y_threshold/y_strength/... rather
  than ln/lm/un/um/vn/vm. Condition: a name-equivalence table is MANDATORY in the README AND in a comment
  block at the parser site. Rationale: cnr3 is a new plugin, not a binary-compatible cnr2 drop-in; a one-time
  rename during migration buys permanently self-documenting scripts.
- **[2026-07-14] `mode="oxx"` retired for three explicit y_curve/u_curve/v_curve params** — the positional
  string is exactly the insider notation the descriptive-naming decision rejects. Side benefit: it DISSOLVED
  the cnr2 permissive-parsing compatibility question (cnr2 treated any non-'x' char as wide) — CNR3 validates
  strictly and records the quirk in the equivalence table only.
- **[2026-07-14] threshold==0 stays VALID** (cnr2 range compatibility), special-cased centre-only rather than
  rejected as 1..255. cnr2 documents 0..255 but its formula divides by n — CNR3 inherits the API, not the
  landmine.
- **[2026-07-14] scene_threshold is CMS07-equivalent, NOT cnr2-exact** at high bit depths (native-depth
  round-to-nearest vs cnr2's power-of-two truncation) — a documented deliberate divergence, recorded as a
  parity STATEMENT, never as "debt".
- **[2026-07-15] The option-parser commit is INTERIM** — defaults operational, user docs deferred. Marker
  kept descriptive at W3X's choice for snapshot commits.

## FINDINGS (evidence, banked)

- **The fmParallel defect is wasted COMPUTE, never wrong pixels.** x=0 held across ~95 sweep cells, every
  mode, every depth, both cache sizes. The cardinal rule (`output[N]=f(source[N],output[N-1])`) makes output
  deterministic even when scheduling is not — this is why byte-identical proofs work under fmParallel.
- **fmParallelRequests has ZERO wasted recompute at every depth** — it overlaps requests/planning but
  serialises compute, so predecessors are published before successors plan. This is why it is the ship mode
  and why plan-retry is useless there.
- **The self-check MISMATCH lines (`stored ... == D-SUM-08 stores`) are a self-check DEFINITION error**
  (kept-stores vs attempted-stores; the gap == discards exactly), NOT a counter bug. Banked to stop a future
  chat chasing it.
- **Re-churn = 0 at BOTH 500 and 1000** on the 3000-frame workload: the working set fits in 500; extra
  evictions at HALF are all cold frames. `frames_recently_evicted_then_re_requested` is THE decisive metric
  for cache-size questions (it was designed for exactly the tunable-vs-inherent distinction, FI-11).
- **K.1E.2 proof placeholders were live in the shipping path** (threshold=255/strength=255/all-narrow) since
  the pixel-layer rebuild, and every structural gate passed for many commits because they compare
  placeholder-build vs placeholder-build. **Only a human eyeballing a stackhorizontal caught it.** ->
  R-PROCESS-33; the chroma-delta analyser is now standing test kit.
- **CNR3's defaults, once fixed, are cnr2's defaults exactly** (ln=35, lm=192, un=vn=47, um=vm=255,
  mode="oxx") — confirmed by source-surveying AviSynth-vsCnr2, not by memory. The fix was right for the right
  reason.
- **cnr2's blend has CROSS-PLANE GATING:** `weight_u = table_y[diff_y] * table_u[diff_u]`. Luma movement
  suppresses chroma blending. CNR3 preserves it. This is the least obvious semantic in the filter and cnr2's
  documentation never stated it; the CNR3 README and parser comment must.
- **The curve enum is genuinely WIRED** (Cnr3ResponseCurveKind -> wide_response -> the j*j vs j cosine
  formula in cnr3_response_tables.cpp). Verified cold; the option patch therefore needed parsing only.
- **The plugin's marker was selftest-only from CMS07-G.6A until the filter-mode-selector patch** — a latent
  OMISSION, not a regression (established by bisecting old GitHub trees, verifying by MARKER not filename).
- **CMS07-FIX.operational-response-defaults shipped without its marker bump** — its logs under-claimed for a
  whole commit. -> R-PROCESS-30.

## PROCESS PRECEDENTS ADDED THIS ARC

- **Gap analysis before patch** (R-PROCESS-31) — the highest-leverage addition; front-loads investigation and
  makes review mechanical (diff must map 1:1 onto reported gaps).
- **The patch is the arbiter, not its notes** (R-PROCESS-32) — the coder's notes were wrong about their own
  patch three times in one round; an apply-test settled each in seconds.
- **Diff before diagnosing** (R-PROCESS-29) — two confident apply-failure diagnoses (CRLF; a 3-line offset)
  were both wrong; the real cause was uncommitted experiment edits in the coordinator's tree vs the coder's
  reconstructed baseline (-> R-PROCESS-28).
- **Both coder chats degraded near their limits** (mangled run instructions; an inline "patch" retracted as
  unvalidated draft). A known, recurring failure mode — escalate scrutiny, restart early.
- **W3X's skepticism is a primary instrument** — every significant correction this arc began with W3X
  questioning a number ("bigger cache should help not hurt"; "the FULL run also says 500?"; "this patch is
  smaller on disk"; "no benefit to plan-retry with fmParallelRequests?"). Each was right.

*** end v3.0 advance ***


**Version:** v2.0 (supersedes v1.8)

*** v2.0 ADVANCE (2026-07-12): analysis/instrumentation sub-arc — four patches committed, selector held,
marker regression open. DECISIONS + FINDINGS banked: ***

- **INTENT COUNTING (committed CMS07-DIAG.intent-counted-lookups).** Decision: a cache probe counts only when
  its outcome is uncertain and changes what happens next (hit_only / full / none policy per site). Rejects the
  prior uniform-count definition. Old L1 oracle 66.664 retired; new L1 exact 7279/7279/0. Rule: hits+misses ==
  queries holds by construction (a probe counts fully or not at all). Identity site-1 hits == D-SUM-12
  frames_cache_hit (exact by source: the cache-hit-return branch is entered iff the site-1 lookup succeeds).
- **LOOKUP-SITE TAXONOMY (committed CMS07-DIAG.lookup-site-breakdown; census in Taxonomy Findings v06).**
  11 named probe sites; three tiers (genuine-demand counted / opportunistic counted / raw-find uncounted).
  Store duplicate-detect nests 10a->10b (would double-count AS2 if both instrumented); resolved by policy
  routing. Selftest routes must count nothing (default unspecified/none) — the leak class the caller-map
  re-derivation exists to catch.
- **FIVE-ORIGIN FRAME LIFECYCLE (committed CMS07-DIAG.frame-lifecycle-bail-counters).** The source has FIVE
  production origins, not three (designer's three-way model was wrong; coder confirm-report caught it):
  frame0_fresh_start, floor_fresh_start, ordinary_target, recovery_hole, recovery_target. COUNT-never-compute:
  16+ independent counters; spine b == f + e + x verified not derived. D-SUM-07 discard tie is NARROW
  (ordinary_target + recovery_target only; AS2 losers have no D-SUM-07 counterpart) — do NOT force the tie.
- **FILTER-MODE SELECTOR (approved, HELD).** Compile-time mode selector; mode suffixed onto CNR3_EDIT_VERSION;
  default fmUnordered identical. Byte-identical accepted BY INSPECTION for this patch (two isolated diagnostic
  change sites, no data-path reach) — a legitimate substitute for fc/b ONLY where the change surface is fully
  traceable and provably off the pixel path.
- **RUN-LOG MARKER REGRESSION (open).** The plugin's edit_version marker emission to the run log was silently
  dropped at some pre-arc commit; CNR3_EDIT_VERSION exists in one source place only (selftest summary). NOT
  caused by this arc. Root defect: no gate asserted run-log emission content -> RATIFIED as R-PROCESS-27
  (Doc A v4.0). Restoration + emission-presence gate owed.
- **L1noR FINDING (fmUnordered, no -r 1).** Prefetch reordering (NOT threading) routes ~50% of a linear clip
  through recovery; all holes ADOPTED not recomputed; zero wasted recompute. e/x race arms unexercised —
  need genuine fmParallel. This is an fmUnordered-prefetch result, not a concurrency result.
- **ORIGINAL DRIVING QUESTION still owed:** D-SUM-01 out_of_order=0 is not the whole story (recovery routing
  proves reordering is real); the DIRECT answer (reconstruct true order from plan-trace ticks; explain the
  false zero) is A1 Q-B, specced (A1 spec v0.4) but not yet built.
