# CNR3 — DESIGNER RESPONSE to coder confirm-report: lookup-site-breakdown — RULINGS, PROCEED — v3 (adds Ruling 4: self-explanatory log output; Ruling 5: gating boundary)
...
## Ruling 5 — gating boundary: gate the new increments/emission; do NOT macro-wrap parameters; do NOT touch the committed count-policy plumbing

The coder's confirm-report correctly flagged that the previously committed `Cnr3LookupCountPolicy`
enum/parameters compile in a gate-off build even though their increments are D-SUM-04-gated, and asked
whether the new breakdown plumbing should be macro-wrapped to disappear entirely from gate-off builds.
Ruling, three parts:

**(a) New breakdown *code* — fully gated. Required.** Everything that *does* work — the per-site struct
fields, observer helpers, the inline site bumps in the walk / hole-catalogue scan / stores, the D-SUM-04
per-site emission, the legend, and the self-check — lives under
`#if defined(CNR3_DIAG_COMPUTE_DSUM04_OWNERSHIP_BALANCE)`, exactly matching the existing D-SUM-04 pattern
(the current ownership stats object, observer decls/defs, and emission are already gated this way; follow it
verbatim). In a gate-off production build none of this compiles.

**(b) New *parameters* (the `Cnr3LookupSite` site tag and the recovery `observe_lookup_site_breakdown`
flag) — defaulted and UNGATED. Do NOT macro-wrap signatures.** Add them the same way the committed
`Cnr3LookupCountPolicy` parameters were added: a plain defaulted parameter, always present in the signature.
Rationale, and this is a deliberate reversal of the coder's proposed macro-wrapping:
  - Macro-conditional function *signatures* are the most build-fragile form of gating — declaration/definition
    arity must stay in lockstep across every gate combination, and callers see different arities under
    different builds. That is real regression surface for zero runtime benefit.
  - An unconditional parameter whose only consumers are gated-out increments is dead in a gate-off build: the
    argument is passed, never read to any observable effect, and the optimizer eliminates it. Cost in the
    shipped DLL is effectively nil.
  - Consistency wins: macro-wrapping the *new* site parameter while the committed *policy* parameter stays
    unconditional would leave sibling primitives with asymmetric signatures under different gates — a bug
    magnet. Match the proven pattern.
So: gate what *executes*; leave the *parameter surface* plain and defaulted.

**(c) The committed `Cnr3LookupCountPolicy` plumbing — do NOT touch. Out of scope.** It is not a defect: in a
gate-off build its increments are gated out and its enum/parameters are dead-code-eliminated. Gating it now
would mean macro-wrapping signatures on already-proven, already-committed primitives purely to remove source
that already compiles away — an R-PROCESS-25 touch of proven code for no behavioural gain, and exactly the
kind of change we do not fold into an unrelated diagnostic patch. The coder's honest note raising this is
noted and the answer is: deliberately left alone.

**Future release cleanup (recorded, not scheduled):** if a final production release ever requires literally
removing *all* diagnostic surface (including the count-policy and site-tag parameters) from gate-off builds,
that is a single deliberate cleanup pass at release time, scoped and reviewed on its own — never smuggled
into a mid-stream diagnostic patch. Bank it as an FI item; do not act on it now.
