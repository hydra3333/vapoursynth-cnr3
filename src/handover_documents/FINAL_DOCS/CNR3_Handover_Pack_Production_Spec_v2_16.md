...
**v2.16 change (currency refresh only; NO rule added, changed, or removed):** advances pointers past a COMPLETE MARSHALLING-OPTIMISATION ARC — twelve value-identical implementation-only levers (AVX2 through Lever E) reduced per-frame native<->scalar marshalling to ~1/5, CUMULATIVE ~-80% (93,914 -> ~18,660), committed through CNR3-OPT-LeverE-scenechange-local-accumulator, selftest **56/56** (forced-fail 55/56 exit 1). Controlling CMS UNCHANGED at **CMS07.15** (the arc is implementation-only — no rule/constant/AS-scope change; every lever value-identical). **NEXT = coordinator call: Lever B (allocation pooling, fmUnordered lifetime proof) OR the diagnostics arc (D-SUM).** Build is now x64-only, /arch:AVX2 HARD REQUIREMENT (see Document A build-protocol note). Doc-set: DELTA v4.23 / Document A v3.12 / Document B v3.12 / Role Handover v1.16 / Reviewer Intro v3.9 / Coder Restart Intro v6.7 / Future Investigations v7.16 / PixelPath Map v0.4. No rule, AS-scope, or section text is changed; the arc added no registered rule. [Prior v2.15 note retained as history:] **v2.15 change (currency refresh only; NO rule added, changed, or removed):** advances pointers to the W.3-closed pack — controlling CMS **CMS07.15** (`cnr3_cache_manager_design_v7_15.md`; additive §7.5 store-status contract over 07.14), selftest **55/55** (forced-fail 54/55 exit 1), the live cache-pressure wiring arc **W.1→W.2→W.3 COMPLETE**, and **NEXT = the diagnostics arc** (D-SUM telemetry, sequenced before the real-footage campaign — coordinator decision 2026-06-30). Doc-set: DELTA v4.16 / Document A v3.11 / Document B v3.10 / Role Handover v1.15 / Reviewer Intro v3.8 / Coder Restart Intro v6.6 / Future Investigations v7.14. No rule, AS-scope, or section text is changed; W.3 added no registered rule. [Prior v2.14 note retained as history:] 
...
foundation invariant), no behaviour change. Build state advanced to: **branch-(d) recovery arc COMPLETE
(D.1 single-hole, D.2 multi-hole+bounded-window-refusal, D.3 floor-fresh-start, D.4 adopt-skip/first-in-
best-dressed primitives, D.5 recovery-pin-survives-prune — all committed, both configs)**; the live
getFrame dispatch is feature-complete across all four branches; selftest count **52/52** (forced-fail
51/52 exit 1). **Next phase: P.11C scene-change uniform wiring across branch-a/c/d** (gates first
real-footage test) — NOT a recovery phase. The charter (§3A.5.0) is unchanged and remains the canonical
governance source mirrored in CMS §0A / Role Handover §D0. Only concrete current-version pointers and
build-state lines are touched; the generic "CMS07.0 or later approved successor" references and ALL
register rules (R-AUTH/R-PACK/R-PROCESS/R-ARCH/...) are unchanged.
**Supersedes:** Production Spec v2.9 (and v2.8/v2.7 and earlier).
**v2.10 change:** (a) adds the **CNR3 Design Alignment and Escalation Charter** as §3A.5.0 (the
three-way governance discipline for CMS alignment, RULE-DEVIATION / CMS-GAP escalation, bidirectional
cross-checking, and durable recording — agreed designer+coder+coordinator 2026-06-27). (b) currency:
build state advanced from "branch-d D.1 is the next phase" to **D.1 committed and proven both configs;
branch-(d) D.2 (multi-hole) is the next phase**; selftest count 49/49; the live getFrame dispatch now
handles ALL FOUR branches (cache-hit, fresh-start, predecessor-present, recovery). No register-owned
rule (R-AUTH/R-PACK/R-PROCESS/R-ARCH/...) is otherwise changed; the charter is additive governance.
**Supersedes:** Production Spec v2.7.
**v2.9 change (currency refresh only; no rule added, changed, or removed):** advances the
named current-CMS pointer from CMS07.8 to **CMS07.10** (`cnr3_cache_manager_design_v7_10.md`)
following the CMS07.9 (pre-compute adopt-and-skip normative; fmParallel assumption caught) and
CMS07.10 (R-LIFECYCLE correction to §9A.1.1, proven by keystone K.1F) revisions, and the
companion to **v7.10**. This is the CMS07.0 Reading Rule clause (c) currency touch only: the
"CMS07.0 or later approved successor" generic references are unchanged; only the concrete
current-version pointers in the change-note, the Reading Rule EXCEPT clause, and the §2 pack
file list are updated. No R-AUTH-*, R-PACK-*, R-PROCESS-*, R-ARCH-*, R-RETIRED-*, R-CAND-* rule
and no canonical-context text is changed. Build state since v2.8: committed/pushed through
CMS07-K.1F (live direct cached-output return, branch-b); selftest count 49/49; the live getFrame
dispatch now handles ALL FOUR branches (D.1 recovery committed); branch-(d) D.2 multi-hole is the next phase.
**Supersedes:** Production Spec v2.7.
Production Spec v2.8 superseded v2.7; v2.6 and earlier which in turn
superseded Production Spec v1.5, which governed continuity
through the now-seperseded CMS06.x proof phases. v2.8 continues along the **CMS07.0 restart** production-spec
...
** v2.8 adds R-PROCESS-23** (patch validation must match the target environment — canonical
LF base, compile-before-green, proof-level honesty, diagnose-failure-class — recorded from the
CMS07-K.1E.2/.3 delivery saga).
...
Currency refresh: the current controlling CMS is **CMS07.13** (`cnr3_cache_manager_design_v7_13.md`);
...
**CMS07.0 or its later approved successor** (currently **CMS07.13**). EXCEPT:
...
`cnr3_cache_manager_design_v7_0.md`, or the current `cnr3_cache_manager_design_v7_13.md`)
...
cnr3_cache_manager_design_v7_13.md
    The current CMS (CMS07.13) — included UNCHANGED as the controlling design authority.
...
The current CMS (`cnr3_cache_manager_design_v7_13.md`, CMS07.13) is included UNCHANGED as the
...
[ ] The current CMS included unchanged (currently cnr3_cache_manager_design_v7_13.md).
