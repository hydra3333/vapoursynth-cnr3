# CNR3 — THIS-CHAT DELTA: current-state companion (SLIMMED, through marshalling levers 0A/0B/3a.1/3b.1/3a.2 + validation policy + A-lite) — v4.20

**Version:** v4.20 (SLIMMED). Supersedes v4.19. Extends the marshalling-optimisation arc with the typed-pointer levers, the validation policy, and Lever A-lite. All steps value-preserving (56/56 four-way, P-series unchanged) and profiled; CMS UNCHANGED (still 07.15 — implementation optimisation only, no design/invariant change). Cumulative vs pre-lever baseline: **93,914 -> ~38,750 samples (~-59%)**.
...
- **Ahead (evidence-sequenced from the 8-bit hot-path review):** **C1** = native-luma downsample bridge is now the TOP leaf (`cnr3_downsample_native_luma_plane_to_scalar_chroma_grid` ~9.2-10.5k, containing a ~4.7k copy-inside-the-bridge + ~1.5k resize; C1 computes the chroma-grid downsample DIRECTLY from native luma bytes, skipping the full-res int materialisation) = the clear NEXT lever. Then **B** (allocation pooling, as a lifetime/thread-safety proof under fmUnordered, NOT an ad-hoc micro-opt), **F/3c** (blend: scalar production-fast path + Tier-2/Tier-3 validation-policy application BEFORE any SIMD gather; vpshufb LUT a later maybe). **D** (EXACT SIMD downsample — NOT the biased VPAVGB, which measured a +0.375-code recursion-compounding bias) and **E** (scene-change local accumulator) are cheap-but-small, deferred for cumulative fps. Parked: diagnostics D-SUM menu, selftest skip-pass fix.
...
```text
Committed/pushed through:  CNR3-OPT-LeverAlite-8bit-unpack-row-pointer-restrict  (marshalling arc: AVX2 + 0A + 0B + 3a.1 + 3b.1 + 3a.2 + A-lite)
...
Marshalling arc status:    0A -28% | 0B flat | 3a.1 -37% | 3b.1 flat | 3a.2 flat-on-8bit (vectorised 16-bit only) | A-lite -7.4%. cumulative ~-59% (93,914 -> ~38,750). CORRECTED FINDING (A-lite): the 8-bit copy was NOT purely memory-bound as inferred after 3a.2 - it was partly OVERHEAD-bound; A-lite (row-pointer + dedicated 8-bit publish loop removing the per-sample cnr3_write_plane_sample call) dropped the unpack leaf ~7k -> ~3.3k and moved the TOTAL -7.4% with NO vectorisation (loops stayed reason 501). PRODUCTION REALITY unchanged: YUV420P8 is the hot path. NEXT: C1 (native-luma downsample bridge) is now the top leaf (~9.2-10.5k, contains ~4.7k copy + ~1.5k resize C1 eliminates). Then B (pooling, lifetime-proof), F/3c (blend), D/E later. 16-bit SIMD secondary.
