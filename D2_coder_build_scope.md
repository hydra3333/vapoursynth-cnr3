CNR3 — D.2 branch-(d) exact-anchor MULTI-hole recovery + bounded-window refusal. BUILD SCOPE for review.
...
2. BOUNDED-WINDOW REFUSAL (the new proof). The H.2A planner searches the bounded interval
   [max(0, N-B), N], B = CNR3_CACHE_BOUNDED_RECOVERY_BACK_RADIUS = 50 (compile-time constant;
   checkpoint-agnostic — a present output beyond N-B is NOT found, proven by the H.1A selftest).
   Two distinct miss outcomes, and D.2 owns only the first:
     (a) anchor present but BEYOND the window (nearest present output is > B back, i.e. <= N-(B+1)):
         the planner returns anchor_found==false (no in-window anchor) -> D.2 CLEAN REFUSAL
         (recover_branch reason = bounded-window-exceeded). THIS IS THE D.2 REFUSAL PROOF.
     (b) NO present output below N at all: this is the floor-fresh-start case -> D.3, NOT D.2.
         For D.2, this also clean-refuses (reason = no-anchor / floor-fresh-start-owed), deferred to D.3.
   Both refusals are clean (frameData discharged + deleted, clear setFilterError), no partial state.
...
arInitial -- recovery start (cnr3_start_live_recovery, in cnr3_arInitial.cpp):
  ...
  - if NOT accepted (anchor beyond window -> anchor_found false; or non-exact/non-contiguous; or no
    anchor): CLEAN REFUSAL -- discharge+delete frameData, setFilterError with a reason string
    distinguishing bounded-window-exceeded (D.2) from no-anchor/floor-fresh-start-owed (D.3).
...
RUN C -- bounded-window refusal (THE NEW PROOF): a clip where the only present output is > 50 frames
  ...
  refusal. Expect a clean refusal KDT (reason=bounded-window-exceeded), non-zero exit or a documented
  refusal signal, and NO partial output / NO crash. (Confirm the refusal reason is the window bound,
  not no-anchor.)
...
QUESTIONS:
  ...
  3. Confirm the refusal path distinguishes bounded-window-exceeded (D.2) from no-anchor (D.3 owed),
     with distinct KDT reasons, and that both are leak-clean.
