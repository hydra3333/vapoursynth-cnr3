...
**keystone era**. v3.1 advances the v3.0 state pointers from the K.1D / K.1E-branch-(c)-in-flight
era to the current baseline: **committed through K.1F (live direct cached-output return, branch-b),
selftest count 49/49, CMS07.10**; the live getFrame dispatch now handles all four branches except
recovery, and **branch-(d) D.1 recovery is the next phase**. v3.0 originally re-pointed off the
...
## PART 3 — CURRENT PHASE-SPECIFIC REQUIREMENTS (branch-(d) D.1 recovery)

**(This replaces v3.0's K.1E branch-(c) section, now three keystones in the past. The detailed
D.1 brief — golden-chain obligation, coder-agreed refinements, the five implementation guards,
the source-request sets, and the full branch-(d) arc D.1–D.5 — lives in DELTA v4 §5. This is the
orientation; the DELTA carries the depth.)**

**Where the live dispatch stands (DELTA v4 §2):** all four getFrame branches are wired and proven
EXCEPT recovery — present-N cache-hit (b, K.1F), fresh-start (a, K.1D), predecessor-present
(c, K.1E.2/E.3). Recovery (d) is the absent-N fall-through, the last branch to wire, and it
replaces the current n>2 clean refusal.

**D.1 = exact-anchor single-hole recovery.** Request output[N] where output[N-1] is ABSENT and
output[N-2] is PRESENT (the anchor). It is the first live COMPOSITION of already-proven
primitives (H.1A search, H.2A AS1 anchor pin+record under one lock, H.3A per-hole AS2 consumer,
Step-0 batch discharge, P.11B compute, K.1E.1 frameData pin-gap pattern); getFrame currently has
ZERO recovery wiring. arInitial plans + pins the anchor at N-2 and requests source set {N-1, N};
arAllFramesReady fills hole N-1 from the anchor via P.11B, computes output[N] from the now-present
N-1, returns it, and batch-discharges the multi-pin list.

**Designer actions owed before the coder scope (DELTA v4 §5):** (1) compute + verify the D.1
golden chain against the real response tables (threshold 255 / strength 255 / narrow) and the
P.11B blend math — same method that produced 161/95 and 163/93; (2) draft the D.1 coder scope
folding in the agreed refinements (final recovery-shaped frameData + KDT vocabulary from D.1, not
single-hole-only fields; distinct instance=/N= KDT fields; the five guards; source set {N-1,N});
(3) settle the recovery-harness construction — needs SetVideoCache(mode=0) (the K.1F core-cache
defeat lesson) AND a way to create the "N-1 absent, N-2 present" starting cache state.

**Settled lifecycle finding to carry (do NOT re-derive — DELTA v4 §4):** every CNR3 getFrame
branch requests >=1 real source frame at arInitial and returns only at arAllFramesReady
(R-LIFECYCLE, CMS §9A.1.1); zero-request arInitial->NULL is not guaranteed an arAllFramesReady
callback under R76. Branch-(b) requests source[N] as a trigger; recovery requests its real hole
sources. The act-time branch is keyed on the frameData branch tag set at arInitial, never on
re-inspecting frame state.
