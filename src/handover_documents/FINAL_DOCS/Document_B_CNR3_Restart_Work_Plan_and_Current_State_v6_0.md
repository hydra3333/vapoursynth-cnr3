# Document B — CNR3 Work Plan and Current Build State (CMS07.15, RESUME) [v4.0]

*** v6.0 UPDATE — 2026-07-18 — THE PLUGIN SHIPS. Option surface complete, README shipped, CI/release
pipeline live, production configuration committed, dev branch retired. Supersedes v5.0.
THIS BLOCK IS AUTHORITATIVE. ***

## CURRENT BUILD STATE (authoritative)

```text
Committed/pushed:   CMS07-RELEASE.production-config
Branch:             main   (dev_cache_manager MERGED and RETIRED; open a feature branch for the next arc)
CMS:                07.15 — UNCHANGED by every arc since
Canonical 4-way:    CONFIG-DEPENDENT IN TWO DIRECTIONS
                      master OFF (ship): 56/56, forced-fail 55/56 invariant_violation e1
                      master ON  (dev):  57/57, forced-fail 56/57  (56 base + PLANTRACE-gated diag3c2)
SHIP CONFIG:        fmParallelRequests + CNR3_CACHE_PROFILE_HALF (500/3 zones)
                    + CNR3_ENABLE_PLAN_RETRY_BIAS OFF   (337 fps null / 274 fps encode, 5.5x realtime)
Diagnostics:        ALL OFF via CNR3_DIAG_MASTER_PERMIT_DIAGS (commented out) — see R-BUILD-01
Ship log:           SILENT (no edit_version=/filter_mode=/response_config:, no D-SUM) by W3X ruling
Response defaults:  y=35/192/wide  u=47/255/narrow  v=47/255/narrow  scene_threshold=10.0  scene_chroma=false
Toolchain:          VS2026 v145; /arch:AVX2; API 4.0 profile (VS_USE_LATEST_API NOT defined); R78 headers
```

## WHAT LANDED SINCE v5.0

1. **CMS07-RIDER.option-error-messages** — self-explanatory errors (range: `got 256, expected an integer
   in the range 0..255 inclusive.`; type: `incorrect value type, expected <same expectation>`), shared
   expectation text, `std::to_chars` float echo, `value_size`-bounded + sanitised + truncated curve echo,
   `expected 0 or 1.` for scene_chroma. 17/17 runtime gate; byte-identical frames. **DISCHARGED the parser
   commit's deferred items 8 and 9.** The new coder's confirm pass found THREE defects in the designer's
   scope (D1/D2/D3) — two would have shipped bugs, one an out-of-bounds read.
2. **CMS07-FIX.provenance-flush** — fflush after provenance emissions; error-domain map settled
   (R-PROCESS-34).
3. **CMS07-DOC.cnr2-descriptive-options-readme** — README.md shipped. **PQ-2 CLOSED.**
4. **CMS07-CHORE.build-hygiene-nominmax-r78-headers** — NOMINMAX both configs + CI; `#ifndef` guards in
   memory_diagnostics; R78 vendored headers + VERSION.txt; API 4.0 rule recorded (R-BUILD-02).
5. **CMS07-RELEASE.production-config** — the MASTER DIAGNOSTICS GATE (R-BUILD-01): every dev-instrumentation
   gate individually wrapped in `#if defined(CNR3_DIAG_MASTER_PERMIT_DIAGS)`; containment rule mandatory for
   new gates; K1E3 dead scaffold trebly commented AND wrapped; stale filter-mode comments corrected.
6. **CI/RELEASE PIPELINE LIVE** — `.github/workflows/build-windows-x64-release.yml`; workflow_dispatch +
   release triggers; explicit cl command with a FLAG MAP to the vcxproj; wildcard sources minus
   selftest_main; dumpbin export hard-fail; BUILD_INFO.txt provenance; zip [dll, README_CNR3.md,
   SHA256SUMS.txt, BUILD_INFO.txt]; PDB separate artifact. First dispatch run PASSED in 40s.

## NEXT WORK, IN ORDER

1. **Cut the first Release** — tag, create a GitHub Release, confirm the zip auto-attaches (the one
   pipeline path not yet exercised).
2. **RESERVATION TABLE** — the last big architectural arc. Fully designed:
   **CNR3_PROPOSAL_Reservation_Table_v1_0.md** (self-contained; coder-annotated v1_1 also in FINAL_DOCS).
   Step A (selector + hookified shared engine + #error guards; byte-identity proof) then Step B (registry,
   intent-mark vs hard claim, collapse-at-F, 10s loud-fail awaits, fetch avoidance; behaviour proof +
   benchmark vs 337 fps decides ship-or-park). Scope A starts by cold-verifying arInitial's real structure.
3. **Residual desaturation** — 3/957 frames, hard-cut/flashing only; users self-serve via
   `scene_chroma=True`. Low priority.
4. **A1 plan-trace tool / A3 real-footage campaign** — parked.

*** end v6.0 update ***


*** v5.0 UPDATE — 2026-07-15 — FIVE ARCS COMMITTED: fmParallel diagnosis, plan-retry, HALF-500, the
desaturation FIX, and the OPTION PARSER. The plugin now has a real user surface and a decided ship config.
Supersedes v4.0. THIS BLOCK IS AUTHORITATIVE. ***
...
CMS:                07.15 — UNCHANGED by every arc below (no cache-manager design change)
Canonical 4-way:    57/57 (Debug 57/57, Release 57/57, forced-fail 56/57 invariant_violation e1, verbose 57/57)
                    CONFIG-DEPENDENT: 56 base + diag3c2_induced_live_bail_plantrace (CNR3_DIAG_COMPUTE_DSUM_PLANTRACE).
                    Under HALF, two NORMAL-geometry hot-zone tests SKIP; total stays 57.
SHIP CONFIG:        fmParallelRequests + CNR3_CACHE_PROFILE_HALF (500 ceiling / 3 hot zones)
                    + CNR3_ENABLE_PLAN_RETRY_BIAS **OFF**
                    -> 337 fps null-run / 274 fps end-to-end encode (5.5x realtime), duplicates ~0, re-churn 0
Response defaults:  y=35/192/wide  u=47/255/narrow  v=47/255/narrow  scene_threshold=10.0  scene_chroma=false
Provenance/run:     edit_version=<marker>:<mode>   filter_mode=<mode>   response_config: <11 resolved live values>
```

## THE ARCS SINCE v4.0 (each committed, each fully gated)

**1. CMS07-SCAFFOLD.filter-mode-selector** — compile-time mode selector (exactly-one #error guard; mode
suffixed onto CNR3_EDIT_VERSION). ALSO fixed the marker gap flagged "open" in v4.0: it was never a
regression but a latent OMISSION (marker had been selftest-only since CMS07-G.6A). The plugin create path
now emits edit_version= and filter_mode=. R-PROCESS-27 ratified from it.

**2. fmParallel defect — DIAGNOSED (no fix committed).** A predecessor-in-flight redundant-recompute race:
activation N+1 needs output N; a peer is still COMPUTING N (unpublished); N+1 cannot see it, recovers,
recomputes N, loses the store race, discards. Active at any concurrency >=2, scaling with depth (197/575/3550
duplicates at -r 2/4/8 on 200 frames; 3750 computes for 200 outputs at -r 8). **Correctness ALWAYS holds
(x=0) — the harm is wasted COMPUTE.** Mode characterisation: fmUnordered 0 waste; **fmParallelRequests 0
waste at every depth** (overlaps requests/planning, serialises compute); fmParallel = the defect lives
entirely in overlapped compute. Deep research banked (111-Eliminating Redundant Recomputation...): the fix is
a **reservation/in-flight registry** (memo-future/singleflight specialised to the chain; closest analogue =
ffmpeg frame-threading progress report/await). PARKED — the designed real fix.

**3. CMS07-EXPERIMENT.plan-retry-bias** -> renamed `CNR3_ENABLE_PLAN_RETRY_BIAS`. Gated polling
approximation of the reservation table: if a formed recovery plan has > HOLE_THRESHOLD holes and attempts
remain, dump the plan, sleep SLEEP_MS (lock-free, pins discharged first), re-plan. Knobs: SLEEP_MS=50,
HOLE_THRESHOLD=2, MAX_CAP=4; derived plan_retry_max = min(CAP, max(1, numThreads/2)). [DSUM-PLANRETRY]
counters incl. the load-bearing dumped_plan_holes_total vs kept_plan_holes_total pair.
**Swept S x threads over ~95 cells, two-run confirmed: ~10x duplicate reduction at tuned S, plateauing
~2000 — a BIAS, not a cure.** S=50 chosen at the efficiency knee (halves duplicates vs the fps peak).
**R-ARCH-08: valid ONLY under fmParallel** (it throttles fmParallelRequests 2.7x for zero benefit).

**4. CMS07-FEATURE.half-cache-profile-and-retry-rename** — gated `CNR3_CACHE_PROFILE_HALF`: three-way
TINY/HALF/NORMAL on every profile knob; HALF = NORMAL except **ceiling 500 + MAX_HOT_ZONES 3**. The zone
reduction is the CR4-preserving companion (3*(50+10)+48 = 228 protected; CR4 wants >= 456; 500 fits — the
2x rule stays intact). Derived values change with it: MAX_PROTECTED_SET_ESTIMATE 348->228,
CHECKPOINT_GRID_FLOOR 25->15. **Proven by a controlled 3000-frame A/B (distinct markers):
frames_recently_evicted_then_re_requested = 0 at BOTH 500 and 1000; fps identical (126 vs 125); slot_count_max
551 vs 1101.** Halving the cache costs nothing measurable — the extra evictions are all COLD frames.

**5. CMS07-FIX.operational-response-defaults** — **THE CHROMA DESATURATION FIX.** W3X's stackhorizontal
eyeball caught brown/red pulling to grey. Root cause: the CMS07 pixel-layer rebuild wired K.1E.2 PROOF
PLACEHOLDERS (threshold=255, strength=255, all curves narrow) into the LIVE config, and the deferred swap to
operational defaults never happened. threshold=255 means the response never shuts off across real chroma
changes; strength=255 means ~98% previous-retention -> recursive averaging toward neutral. **The math never
regressed; only the parameter surface did.** Fixed to vscnr2 operational defaults (Y 35/192/wide, U/V
47/255/narrow — later CONFIRMED to be cnr2's own defaults exactly). Proof: desaturation-bug frames 2->0 on
the original 10-frame clip; frames 8/9 red/brown magnitude loss -6.6 -> -1.8 (~3.6x better); 957-frame
interlaced real-workload clip shows 3 residual frames (0.3%, hard-cut/flashing content only).
**Every structural gate passed throughout — they compared placeholder-build vs placeholder-build.
R-PROCESS-33 ratified from this.** A Python chroma-delta analyser is now a standing QUALITY gate.
NOTE: this commit shipped WITHOUT its marker bump (logs under-claimed for a commit); corrected by the
option-parser commit's provenance note. R-PROCESS-30 ratified from that.

**6. CMS07-FEATURE.cnr2-descriptive-option-parser** (current) — eleven descriptive options parsed at create
time, strictly validated (throw), applied to the existing config, and echoed live in `response_config:`.
Names are DESCRIPTIVE by W3X decision; cnr2's terse names are reference/equivalence documentation only;
`mode="oxx"` is retired in favour of three explicit y_curve/u_curve/v_curve params. Preceded by a GAP
ANALYSIS (R-PROCESS-31) that resolved three unknowns cold: curve enum already WIRED (response_tables.cpp
selects the j*j vs j cosine from Cnr3ResponseCurveKind); threshold==0 already SAFE (centre-only, never
divides by zero); scene_chroma PLUMBING-ONLY (the constructor already accepted the bool; create passed a
literal false). Gate: 57/57; no-options == cnr2 defaults; explicit-defaults == no-options; non-defaults
reflect all 11; **R-PROCESS-19 anchor: no-args output byte-identical to the interim build (595,336,353
bytes, fc /b clean)**.

## NEXT WORK, IN ORDER

1. **CNR3_RIDER.option-error-messages** — scope approved, **NO PATCH EXISTS** (coder chat degraded at its
   limit: emitted an inline "patch", then retracted it as "unvalidated draft content"). A fresh coder chat
   implements from CNR3_Rider_Scope_OptionErrorMessages_v2.md. Message text only; validation logic unchanged.
2. **DISCHARGE the parser's deferred gate items against the rider build** (they are the same runs):
   item 8 = invalid-option throws (y_threshold=256, u_strength=-1, y_curve="o" [cnr2 spelling MUST be
   rejected], scene_threshold=101, scene_chroma=2, y_threshold="bad"); item 9 = threshold==0 must **SUCCEED**
   (0 is a valid cnr2-compatible value; the table builder special-cases it centre-only) and show y=0/192/wide.
   If ANY invalid value is ACCEPTED -> parser bugfix, not the rider.
3. **CMS07-DOC.cnr2-descriptive-options-readme** — the user-documentation half of the option surface
   (README draft banked: CNR3_README_draft_user_options_v1.md; the v6 mapping spec carries the same text).
4. **RESERVATION TABLE** — the real fmParallel fix. **W3X CONSTRAINT: it, CNR3_ENABLE_PLAN_RETRY_BIAS, and
   the cache-profile gating must all COMPOSE** (work separately or together). W3X's parked Stage-1.5 idea:
   an arInitial-entry in-flight registry (per-instance mutex, RAII-removed at every arAllFramesReady exit)
   so a wait loop can distinguish in-flight holes from genuinely-absent ones.
5. **Residual desaturation** (3/957 on interlaced real footage; hard-cut/flashing content). Users can now
   self-serve via `scene_chroma=True`. Revisit only if it proves to matter.
6. **A1 plan-trace analysis tool** / **A3 real-footage 576p50 campaign** — unchanged, parked.

*** end v5.0 update ***


*** v4.0 UPDATE — 2026-07-12 — ANALYSIS/INSTRUMENTATION SUB-ARC: four patches committed + selector held + marker regression open ***
Supersedes v3.20. Since v3.20, an analysis/instrumentation sub-arc made the cache's behaviour under
out-of-order requests fully legible. All committed patches passed the full gate (canonical 4-way 56/56,
forced-fail 55/56 exit 1; R-PROCESS-19 macro-off byte-identical S8=497,668,851 bytes; L1/L2 oracles;
CMS DESIGN UNCHANGED at 07.15).

COMMITTED (in order):
(1) CMS07-DIAG.intent-counted-lookups — replaced uniform cache-lookup counting with INTENT counting: a probe
    is counted only when its outcome was uncertain and changes what happens next. Mechanism:
    Cnr3LookupCountPolicy{none,full,hit_only} defaulted on the two lookup primitives; six counted sites opt
    in. Old uniform L1 oracle (66.664) RETIRED. New L1 oracle EXACT: queries 7279 / hits 7279 / misses 0 /
    hit-rate 100.000. L2: 12109 / 7279 / 4830. Identity site-1 hits == D-SUM-12 frames_cache_hit (L1 0; L2 4830).
(2) CMS07-DIAG.lookup-site-breakdown — 11 per-site D-SUM-04 counters (invocations/looks/hits/misses) with a
    plain-English legend + per-row purpose lines + a print-only self-check (per-site sums == merged totals ->
    OK / *** MISMATCH ***, NEVER wired to selftest). Store routing avoids AS2 double-count (10a/10b nesting;
    live routes opt in, nested/selftest routes pass none). Selftest prints all 11 rows at 0/0/0 (leak-proof).
(3) CMS07-DIAG.frame-lifecycle-bail-counters — FIVE-origin frame taxonomy (frame0_fresh_start,
    floor_fresh_start, ordinary_target, recovery_hole, recovery_target). Events a(bail-before-compute),
    b(computed), e(bail-after-compute), x(computed-returned-after-duplicate), f(computed-and-stored); each
    total + 5 origins, ALL INDEPENDENTLY COUNTED (COUNT, never compute). Spine self-check b == f + e + x
    (total and per-origin). 19 self-checks OK. L1 exact (b: frame0 1 + ordinary 7279). L2 exact (b: floor 1 +
    ordinary 884 + hole 4829 + target 1566; production-store tie 2450 = 884+1566; AS2 tie 4830 = 1+4829).
    Files(7) incl. vapoursynth-Cnr3.cpp (teardown snapshot). D-SUM-07 discard tie is NARROW (ordinary_target +
    recovery_target only); AS2-widening banked as FI, not forced.

IN FLIGHT — reviewed-APPROVED, NOT committed (HELD): CMS07-SCAFFOLD.filter-mode-selector. Compile-time selector
at the TOP of cnr3_build_config.h (under the marker): uncomment exactly one of
CNR3_FILTER_MODE_UNORDERED/_PARALLEL_REQUESTS/_PARALLEL; exactly-one #error guard; CNR3_SELECTED_FILTER_MODE
token used at createVideoFilter; CNR3_SELECTED_FILTER_MODE_TEXT_SUFFIX (":fmUnordered") suffixed onto
CNR3_EDIT_VERSION so the marker reads CMS07-SCAFFOLD.filter-mode-selector:fmUnordered; one filter_mode=<token>
provenance fprintf at filter creation. DEFAULT fmUnordered = behaviour identical to the prior commit. 4-way
PASS (marker check passed against the suffixed string). Byte-identical satisfied BY INSPECTION (designer traced
both change sites cold: token resolves to identical fmUnordered; the fprintf is stderr-only, no pixel/compute
reach). filter_mode=fmUnordered confirmed in a real VS run. HELD pending the marker regression fix (both touch
cnr3_create_filter).

OPEN — RUN-LOG MARKER REGRESSION: the plugin used to print its edit_version/CMS07 marker to the RUN LOG; it no
longer does. Verified cold: CNR3_EDIT_VERSION appears in EXACTLY ONE source location (cnr3_cache_core_selftest_
main.cpp — selftest summary only), in the current tree AND at the earliest snapshot (honest-cache-hit-metrics);
no plugin-run-path emission, no orphaned scaffolding. So the regression PREDATES this arc and is NOT caused by
any of the four patches (all reviewed cold; none touch marker emission). Root defect (the real finding): the
proof gate verifies SELFTEST (4-way) + FRAME BYTES (fc /b) but never RUN-LOG EMISSION content -> a run-log line
can vanish with all gates green. Fix in progress: bisect older GitHub trees (verify by MARKER not filename) to
pin the removing commit + capture the exact original emission line; restore the run-path emission (home:
cnr3_create_filter, beside filter_mode=); add an emission-presence check to the harness/gate (now R-PROCESS-27).

## FORWARD (order)
1. Pin + restore the run-log marker; add the emission-presence gate. 2. Commit filter-mode-selector.
3. Sample runs for the A1 example section: 200-frame UNSHUFFLED TINY-100 baseline (plantrace 0..199, no -r 1);
   then shuffled; then the FIRST fmParallel whirl (plain in-order L1, TINY-100, plantrace on, no -r 1) via a
   one-comment flip on the committed selector — first run able to exercise e/x / L-codes / C1-under-race.
   PROOF runs NORMAL profile; EXPERIMENT runs TINY. 4. A1 tool build (spec v0.4 handoff-ready; Q-B answers the
   original D-SUM-01 out_of_order=0 question). 5. A2 (fmParallel churn = C1-ownership-under-race gate). 6. A3
   (real 576p50 via A1).

The v3.20 block and detail below are retained as history; the repository + this top block are authoritative.


*** v3.20 UPDATE — 2026-07-09 — IN-PLUGIN DIAG ARC CLOSED at DIAG.4; TWO analysis-track diagnostics committed; forward = A1/A2/A3 ***
Supersedes v3.19. Three commits pushed on dev_cache_manager since v3.19:
(1) DIAG.4 COMMITTED + PUSHED (marker CMS07-DIAG.4-memory-dsum02-and-arc-close). Memory D-SUM-02 implemented (was a
    stub): 5 snapshot points (baseline/periodic@1000/pre-cleanup/post-cleanup/summary); 13 dynamic metrics min/avg/max
    + 3 peaks + baseline; 7 misleading/duplicate metrics REMOVED from print+accumulation (retained as raw fields only);
    [DSUM02-SNAPSHOT|SUMMARY]; two-gate #ifdef. ONE ungated production change (R-PROCESS-21/25): explicit
    output_cache.clear() in cnr3_free_filter (status-captured, (void)'d, never aborts teardown, fail-safe) to enable
    post-cleanup measurement; pins proven zero (D-SUM-04 balance==0 same run). Memory spec -> v3.4. ARC CLOSED: all 14
    D-SUM families live. 56/56.
(2) Prune-rechurn recency-gate COMMITTED (ANALYSIS-TRACK; CMS07-DIAG.prune-rechurn-recency-gate). Misleading D-SUM-10
    frames_evicted_then_re_requested (age-blind; far-revisit dominated; 481 on S9) REPLACED by
    frames_recently_evicted_then_re_requested gated on eviction_gap <= 3*BACK_RADIUS (45 TINY / 150 NORMAL), a value
    chosen from the empirical valley (events cluster gap<=50 recovery-local or gap>=101 far-revisit; 51-100 empty).
    Ring scan / histogram / top-thrashers UNCHANGED. Proven S9/S9c/S9d=0, S9e=40.
(3) Derived-health-ratios COMMITTED (ANALYSIS-TRACK; CMS07-DIAG.derived-health-ratios). Additive end-of-run
    [DSUM-HEALTH] block, 8 ratios from already-final counters; per-ratio all-operands-live gating with
    "(source D-SUM-NN disabled)" markers; "n/a" on zero denominator (distinct from 0.000); Option-A quiescent-snapshot
    wiring at teardown. Proven: macro-on hand-check all 8 == raw (S8 576p50); macro-off byte-identical frame output
    (fc /b, 497,668,851 bytes); 56/56. Derived from an (a)/(b) counter classification; 3 candidate ratios REJECTED as
    misleading -> A1 (recovery-search-hit ~100%; hole-source cross-lifecycle; recalc-and-stored = A1 L-rate).

ROADMAP now: IN-PLUGIN DIAG ARC COMPLETE. Forward = external ANALYSIS TRACK only:
    A1  plan-trace analysis tool (external Python; absorbs FI-11 offline; may run parallel; input set now includes the
        3 rejected health ratios). Seed question-set banked (recalc %, K/C/L rates, O->R divergence, recovery-span vs
        BACK_RADIUS, FI-11 thrash correlation, arrival-disorder). Fresh designer chat recommended (open with reconciled
        snapshot: DELTA v4.30 / Doc B v3.20 / Condensed v1.10 / Provenance v1.8; verify against live src/).
    A2  fmParallel concurrency churn = the C1-ownership-under-race acceptance gate owed from the 3b commit.
    A3  real-footage 576p50 campaign analysed via A1.
Standing (do not decay): 3c.2 live bail-site->helper state-plumbing diff-verified, confirm at first A-series real
induced failure. CODER-CHAT CAUTION: a coder chat twice mangled run instructions; evaluate against known-good
baselines (canonical 4-way is now Document A R-PROCESS-26). CMS design UNCHANGED at 07.15. CMS 07.15.
...
# Document B — CNR3 Work Plan and Current Build State (CMS07.15, RESUME) [v3.20]
