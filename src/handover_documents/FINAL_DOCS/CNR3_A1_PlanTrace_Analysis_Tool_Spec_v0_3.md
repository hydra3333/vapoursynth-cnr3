# CNR3 A1 — Plan-Trace Log Analysis Tool — SPECIFICATION v0.3
...
**Status:** v0.3. Adds the Part A onboarding primer; folds in everything settled since v0.2 (intent-counted
...
- an **O record** ("plan open") at **arInitial** — when the frame is first requested and the plan is
  decided;
- an **R record** ("plan result") at **arAllFramesReady** — when the frame has been produced and returned.
...
**The four-point tick lifecycle.** Each record carries high-precision monotonic timestamps ("ticks",
nanoseconds). Together the O and R records give four moments per frame:
`O.enter_tick` (frame N requested) → `O.exit_tick` (plan formed) → `R.enter_tick` (sources ready) →
`R.exit_tick` (frame produced/returned). These ticks are the **true order** of what happened — and
reconstructing that true order is the tool's headline job (see Q-B / A8). Each record also carries a human
UTC timestamp, but that is *derived and coarse* — use the integer ticks for ordering, the UTC only for
display.
...
are part of the full set, not optional.
