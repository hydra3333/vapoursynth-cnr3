...
**Version:** v4 (DELTA through K.1F + CMS07.10)
**Date:** 2026-06-25
**Supersedes:** the v2/v3 DELTA (which covered K.1A–K.1E branch-(c)). This v4 carries the state
forward across the K.1E.2, K.1E.3, ledger/rules refresh, CMS07.9, Recovery-Step-0, K.1F, and
CMS07.10 commits, and states the immediate next action (branch-(d) D.1 recovery).
...
```text
Committed/pushed through:  CMS07-K.1F-live-direct-cached-output-return-proof
...
**Stale-pointer note:** the Introduction v3.0, Role Handover v1.7, and Document B v3.4 front-matter
still reference the K.1D/47/branch-c-in-flight era and CMS07.7/07.8. Their DEEP content (role
disciplines D1-D16, process rules, the design model) is current; only the state-pointers and
version tables lag. This DELTA carries the authoritative current state until those are refreshed.
