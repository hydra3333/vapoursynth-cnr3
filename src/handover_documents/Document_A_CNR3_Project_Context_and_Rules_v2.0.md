...
**Version:** v2.0  
**Date:** 2026-06-11  
**Status:** Stable project context and operating-rules document; v2.0 control-spec update from v1.9 after CMS06.11 superseded CMS06.10.  
...
## A0. v2.0 current-state override and non-regression note

This v2.0 pack supersedes the v1.9 handover pack as the current handover baseline. It is generated from v1.9 after the designer accepted the coder's eight CMS06.10 review points and produced CMS06.11.

The controlling design authority for cache-manager work is now:

```text
cnr3_cache_manager_design_v6_11.md
```

CMS06.11 supersedes CMS06.10. CMS06.11 is not a reversal of CMS06.10; it is a correctness and proof-audit tightening pass over CMS06.10. The most important correction is removal of stale H15.6/H15.7 body wording that contradicted the CMS06.10 H15.6B.1 / H15.6B.2 / H15.6B.3 sequence. CMS06.11 also adds implementation-facing anti-regression rules for reserved predecessor refs, frameData cleanup, proof counters, and deferred H17 optimisation.

Any v1.9 wording that says CMS06.10 is the controlling design authority is superseded by this v2.0 note. The v1.9 documents remain retained below for continuity and historical context, but CMS06.11 controls new coding.

### A0.1 Immediate non-regression rules after CMS06.11

The following rules are now standing context and must be carried into future handovers unless a later design authority explicitly supersedes them:

```text
1. CMS06.11 is the controlling design authority.
2. H15.6B coding starts at H15.6B.1 against CMS06.11.
3. The earlier fail-closed-only H15.6B active source-request reduction patch remains retired as a superseded draft.
4. H15.6B.3 source-request reduction is allowed only if arInitial successfully acquired and stored a reserved predecessor ref for output_cache[N-1].
5. A cache-contains check or any non-owning observation that N-1 exists is not sufficient for source-request reduction.
6. Reserved predecessor refs are lookup-addref refs, not checkpoint pins and not non-checkpoint pins.
7. Reserved predecessor refs participate in the existing lookup-owned-ref invariant: acquired == released + transferred.
8. The reserved predecessor is an input frame, not the returned frame; in H15.6B.1 and H15.6B.2 it is expected to be released, not transferred.
9. frameData cleanup must release a non-null reserved predecessor ref through the named/null-guarded helper or currently named equivalent.
10. The null field is the idempotency guard; cleanup release and fast-path consumption must not both release the same ref.
11. H17 sparse-hole/minimal fallback recovery remains deferred and must not be mixed into H15.6B.
```

### A0.2 H15.6B phase order now controlling

The next H15.6B work must proceed in this order:

```text
CMS02-H15.6B.1 / arInitial predecessor reservation lifecycle proof
CMS02-H15.6B.2 / reserved-predecessor fast-path consumption proof
CMS02-H15.6B.3 / active sequential source-request reduction
```

H15.6B.1 must not reduce source requests. H15.6B.2 must still keep conservative source requests. H15.6B.3 is the first phase allowed to reduce safe class-B sequential fast-path candidates to source N only, and only where the reserved predecessor ref has been successfully acquired and carried.

### A0.3 Single-ownership/null-on-consume rule, CMS06.11 form
...
- the null field is the idempotency guard;
...
- cleanup and consume paths must attribute counters correctly.
```

CMS06.11 specifies that `reserved_predecessor_released_by_cleanup` must increment only on the cleanup path. Successful consumption increments the consumed/released accounting appropriate to the consumption path, not the cleanup counter.

### A0.4 Ownership-proof standing rule retained and tightened

Every cache-affecting proof or validation phase must explicitly prove both ownership systems where relevant:

```text
Lookup-addref ownership for ordinary cached-frame use:
  - acquired/released/transferred counts;
  - zero lookup-ref balance;
  - reserved predecessor refs included in the same invariant.

Checkpoint pin/unpin ownership for checkpoint/recovery anchors:
  - pin/unpin counts;
...
Reserved predecessor refs are ordinary lookup-addref refs. They must not be represented as checkpoint pins.

### A0.5 Future H17 sparse-hole/minimal fallback recovery optimisation remains deferred

CMS06.11 records H17 sparse-hole/minimal fallback recovery as a deferred item. The current fallback remains correctness-first and conservative. It may retrieve/recompute a bounded lower frame even when that output is already cached, relying on first-in-best-dressed duplicate handling to discard duplicate output safely.

A later H17-style path should start from the nearest suitable cached predecessor or checkpoint inside the bounded recovery window, retrieve only missing source frames needed to walk forward, compute/store only missing outputs plus the requested output, and return through output-cache authority.

This is not part of H15.6B. Do not mix sparse-hole optimisation with H15.6B reservation or source-request reduction.

---

## Appendix A - Full v1.9 Document A retained for continuity

The following is the full v1.9 Document A content. It remains useful project context, but any CMS06.10-as-current wording inside it is superseded by the v2.0 override above and by CMS06.11.
