...
**Version:** Document A v3.10 — RESUME-state regeneration generated from Production Spec **v2.14**
populated §3.2 (canonical context, reproduced verbatim) and §3A (Prevailing Rules Register, reproduced
in full). **v3.10 change over v3.9 (currency touch only; NO register-owned rule text changed):** folds in the VS2026
...
fmParallel validation phase. **Next phase: STEP 0 CLOSED (wiring contract settled in CMS §7.4-§7.6). Now the WIRING implementation: W.1 §7.4 checkpoint-retention trigger primitive (cache-core + selftest) → W.2 hot-zone observation wiring → W.3 combined live store-and-prune helper → real-footage → diagnostics → fmParallel**
