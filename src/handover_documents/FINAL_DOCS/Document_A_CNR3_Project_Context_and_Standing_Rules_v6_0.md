...
*** v6.0 ADVANCE (2026-07-18) — READ FIRST; supersedes all older notes below where they differ ***
...
### R-PROCESS-26 SECOND ADDENDUM — the count is config-dependent in TWO directions (RATIFIED)
**Rule:** with `CNR3_DIAG_MASTER_PERMIT_DIAGS` **OFF** (ship) the canonical 4-way is **56/56**
(forced-fail 55/56 e1). With the master **ON** (dev, all families as configured) it is **57/57**
(forced-fail 56/57 e1). Always state which configuration a proof count belongs to. LOCATE any deviation
in code.

### R-BUILD-01 — the master diagnostics permit and the CONTAINMENT RULE (RATIFIED)
**Rule:** `CNR3_DIAG_MASTER_PERMIT_DIAGS` is the single production switch for ALL dev instrumentation.
Every diagnostic / dev-instrumentation gate — D-SUM families, PLANTRACE, keystone scaffolds, startup
provenance — MUST be wrapped INDIVIDUALLY:
```c
#if defined(CNR3_DIAG_MASTER_PERMIT_DIAGS)
#define CNR3_DIAG_COMPUTE_<FAMILY> 1
#endif
```
**Every NEW gate MUST receive the same individual wrap.** Per-item wrapping is REQUIRED in preference to
one long `#ifdef` region (W3X ruling): the pattern is visible at every site, so a maintainer cannot put a
new gate outside it by accident, and there is no central `#undef` list to fall out of sync.
Deliberately UNWRAPPED (production-meaningful configuration): filter mode, cache profile, plan-retry and
its knobs, the scdthr default.
**Rationale:** ship builds must be silent and free of observation overhead; dev builds must return the
entire instrumented world by flipping ONE line. The first draft of this used a central `#undef` list and
a single wrapping region; both were rejected by W3X as drift-prone — correctly.

### R-BUILD-02 — API level is opt-in and stays at 4.0 (RATIFIED)
**Rule:** CNR3 does NOT define `VS_USE_LATEST_API` / `VS_USE_API_42`. The vendored VapourSynth headers
self-version and compile CNR3 at the default **API 4.0 profile**. Vendored headers are refreshed from
RELEASE TAGS only (currently R78) with a VERSION.txt recording tag + date + this rule.
**Rationale:** declaring 4.2 makes the plugin REFUSE TO LOAD on older cores for zero gain — CNR3 uses no
4.1/4.2 features and touches no colour-range properties. Frame properties are runtime data: CNR3 is
property-transparent (every output frame is `copyFrame(source)`), so `_Range` and everything else flow
through downstream REGARDLESS of the compiled API level. Availability of a property = the runtime core's
version; the compile-time declaration is only a promise about which names YOUR code uses.

### R-PROCESS-34 — the error-domain map (RATIFIED)
**Rule:** `vsapi->mapSetError` for CREATION errors; `setFilterError` for FRAME-PROCESSING errors; stderr
for DIAGNOSTICS/STATUS only. API error surfaces are not streams and need no flush; stderr diagnostics
flush per line (R-PROCESS-24). Never duplicate a user-facing API error onto stderr.
**Rationale:** settled while confirming the rider's compliance with the flush standard; the coder
correctly defended the boundary and the designer confirmed it cold against source.

### R-PROCESS-35 — the log file is the arbiter of a failing harness (RATIFIED)
**Rule:** when a harness fails, READ ITS LOG before theorising. Do not diagnose from exit codes, folder
names, or memory.
**Rationale:** the 17/17 gate took six attempts, all harness-side: a mangled paste, a wrong vspipe path
(exit 9009 = command-not-found), an over-generous pass criterion that scored launch failures as passes,
and a `.vpy` that had been RECYCLED during a tidy-up. One `type <log>` named the last one instantly. This
is R-PROCESS-29 (diff before diagnosing) applied to harnesses.

*** end v6.0 ADVANCE note ***


*** v5.0 ADVANCE — 2026-07-15 — ratifies R-PROCESS-28..33 and R-ARCH-08 into the §3A register (document
HEALTHY, advanced in place, NOT regenerated). No existing rule is changed. Also records the CONFIG-DEPENDENT
selftest count under R-PROCESS-26. ***

### R-PROCESS-26 ADDENDUM — the canonical selftest COUNT is config-dependent (RATIFIED)
**Rule:** the canonical 4-way's expected total is NOT a fixed constant. It is 56 base + one per enabled
conditional test. Currently **57** (`CNR3_DIAG_COMPUTE_DSUM_PLANTRACE` enables
`diag3c2_induced_live_bail_plantrace`); forced-fail is 56/57 invariant_violation exit 1. Under
`CNR3_CACHE_PROFILE_HALF` two NORMAL-geometry hot-zone tests SKIP (visibly, via skip_line) and the total
STAYS 57. **A changed count must be LOCATED IN CODE with file:line evidence before any gate is claimed.**
**Rationale:** a coder reported "the expected count we had been using was 56/56, but this log shows 57/57"
and moved on. W3X refused the hand-wave and required the cause be identified; it was a legitimately
config-enabled test, present in the baseline BEFORE the patch. Had it been anything else, a silent test-count
change would have sailed through a green gate.

### R-PROCESS-28 — Patches are cut against the ACTUAL committed source, never a reconstructed baseline (RATIFIED)
**Rule:** W3X uploads the real current committed `src.zip`; W3C cuts the patch against exactly that and
proves self-apply against it before delivery. W3D verifies the uploaded source by MARKER before reviewing.
Reconstructed, remembered, or "equivalent" baselines are not acceptable.
**Rationale:** a patch cut against a reconstructed baseline failed to apply repeatedly. The cause was NOT
CRLF and NOT a line offset (both were asserted, both wrong): the coordinator's committed tree carried
deliberate experiment edits (plan-retry enabled, sleep=50, fmParallel selected) that the coder's baseline
lacked, so the patch's context lines collided. A whole session was lost. **Corollary: a marker check
confirms COMMIT IDENTITY but not WORKING-TREE EDITS.**

### R-PROCESS-29 — Diff before diagnosing an apply failure (RATIFIED)
**Rule:** when a patch fails to apply, the FIRST action is to diff the target file against the patch's
expected context. Do not theorise about CRLF, strip levels, or offsets before the diff has been read.
**Rationale:** two confident diagnoses (CRLF; "3 lines above") were both wrong and sent the session chasing
tooling ghosts. The diff named the true cause in seconds. Applies to W3D as much as W3C.

### R-PROCESS-30 — Verify the marker LANDED, after every commit (RATIFIED)
**Rule:** after a commit, confirm the edit_version marker in the committed source is the intended one
(`git grep CNR3_EDIT_VERSION_LITERAL src/cnr3_build_config.h`, or the marker line in the first post-commit
run log). Every proof gate must surface the marker somewhere, whatever the patch's primary instrument is.
**Rationale:** CMS07-FIX.operational-response-defaults shipped WITHOUT its marker bump. Its logs
under-claimed (showed the prior snapshot marker while containing the fix) for a whole commit, undetected,
because that patch's proof instrument was a chroma analyser that never surfaces edit_version. The marker
discipline had no run that could reveal the gap.

### R-PROCESS-31 — GAP ANALYSIS BEFORE PATCH for surface/parameter work (RATIFIED)
**Rule:** before any patch that adds or changes a user-facing/parameter surface, W3C delivers a GAP ANALYSIS:
a per-item table (for each parameter: parse / default / validate / apply — each EXISTS with file:line,
MISSING, or PARTIAL), definitive answers to the named unknowns with evidence, and a FINAL PATCH SHAPE
statement. W3D cold-checks the evidence. **The eventual diff must map ONE-TO-ONE onto the reported gaps;
diff content with no corresponding gap is a review finding.**
**Rationale:** this front-loaded the investigation for the option parser and resolved three unknowns cheaply
(curve enum already wired; threshold-zero already safe; scene_chroma plumbing-only), turning the subsequent
review into a mechanical audit. It is the single highest-leverage process addition of the session.

### R-PROCESS-32 — The patch is the arbiter, not its notes (RATIFIED)
**Rule:** when a patch's description and the patch itself could disagree, TEST THE PATCH (apply it to a copy
of the real tree and inspect). Never reason about a diff from its notes when the diff is in hand.
**Rationale:** in a single round the coder's notes were wrong about their own patch three times: the debt note
described the wrong debt; the notes listed a README.md the patch did not contain; and the coder verbally
called a standalone replacement patch a "delta on v1" — an apply-test proved the opposite in seconds (it
applied clean to baseline and FAILED on a v1-patched tree). Descriptions drift; bytes do not.

### R-PROCESS-33 — Pixel-affecting changes require a QUALITY gate, not only structural gates (RATIFIED)
**Rule:** any change that can affect pixel output must be proven by (a) the chroma-delta analyser on the
reference 10-frame clip AND (b) a human stackhorizontal look, in addition to the structural gates. Record
the analyser's classification per frame and the expected shape. Structural gates alone are NOT sufficient.
**Rationale:** the K.1E.2 proof placeholders (threshold=255/strength=255/all-narrow) sat in the live shipping
path across many commits, desaturating chroma (brown/red -> grey), while EVERY structural gate stayed green —
selftests, byte-identical, ownership balances — because they compare placeholder-build against
placeholder-build. Only W3X's eyeball on a stackhorizontal caught it. **Three gate classes now exist:
selftests gate STRUCTURE; byte-identical gates INERTNESS; the analyser + eyeball gate the PICTURE.**
**Corollary (also ratified):** a "temporary" placeholder in a live path is a defect with a timer on it. Any
deferral comment in a live path must be recorded as an owed item that decays loudly.

### R-ARCH-08 — CNR3_ENABLE_PLAN_RETRY_BIAS is valid ONLY under fmParallel (RATIFIED)
**Rule:** `CNR3_ENABLE_PLAN_RETRY_BIAS` may be enabled ONLY when compiling `CNR3_FILTER_MODE_PARALLEL`. It
must be OFF for fmUnordered and fmParallelRequests. A compile-time guard (#error when enabled without
fmParallel) is a recommended future safety.
**Rationale:** plan-retry dumps a hole-heavy plan and sleeps, betting an in-flight predecessor will publish.
Under fmParallel that works (~10x duplicate reduction at S=50). Under fmParallelRequests compute is SERIAL,
so the missing predecessors are never in flight — the sleep waits for something that cannot arrive. Measured:
8903 dumped plans, 8903 sleeps, 2993 plans hitting the retry cap, dumped-plan holes 136043 vs kept-plan holes
4852 (holes did NOT shrink), throttling the mode from 337 fps to 122 fps — a 2.7x loss for zero benefit, and
duplicates were WORSE (139 vs 1). It is a mitigation for one specific race, not a general good.

*** end v5.0 note ***


*** v4.0 ADVANCE — 2026-07-12 — ratifies R-PROCESS-27 into the §3A register (document HEALTHY, advanced in
place, NOT regenerated). No existing rule is changed. ***

### R-PROCESS-27 — Proof gates must assert RUN-LOG EMISSION content, not only selftest + frame bytes (RATIFIED)
**Rule:** a phase's proof gate must include a check that the plugin actually EMITS the run-log lines it is
required to emit (build/version marker, config/provenance lines, and any D-SUM lines the phase touches). It
is NOT sufficient to prove the selftest passes (4-way) and the frame output is byte-identical (R-PROCESS-19);
those are blind to run-log content. Emission presence is verified by grepping the run log for the required
line(s) and failing the gate if absent.
**Rationale (the precedent):** the plugin's edit_version/CMS07 marker silently stopped being emitted to the
run log at some past commit and NO gate caught it, because every gate checked selftest and frame bytes only.
The regression was invisible for an unknown span ("what else broke unnoticed?"). Verify-cold established the
marker exists in exactly one source location (the selftest summary) with no plugin-run-path emission. This
rule closes that class of gap: an emission that no gate asserts can vanish with all gates green.
**Scope:** applies to every diagnostic/scaffold patch that adds or relies on run-log output, and to the
harness (the .vpy/.bat, a DESIGNER deliverable) which must carry the emission-presence findstr checks.
