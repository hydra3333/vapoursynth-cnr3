# CNR3 — PATCH SCOPE: DIAG.3c.1 — PLAN-TRACE BUFFERED CAPTURE + CLEAN-END DUMP (observe-only)

**From:** designer/reviewer (W3D), via coordinator (W3X), to coder (W3C).
**Controlling input:** `CNR3_DIAG_PlanResult_Vocabulary_and_Architecture_Spec_v2_1.md` (the plan-trace spec).
...
Spec v2.1 §12 deliberately left the 3c.1/3c.2 packaging to scope time. Decision for this cycle:
...
block(s) at CLEAN END-OF-RUN only. Per spec v2.1:
...
- Three legend-headed sorted views (datetime / frame / phase), each sub-#ifdef gated (spec §8).
...
- action_seq: monotonic per-instance counter stamped at record write.
