...
specified). **Next phase: live cache-pressure WIRING** — wire hot-zone observation (CMS §5.7: at arInitial)
then pruning into the live getFrame path. The prune logic + hot-zone machinery are built and selftest-proven
...
state. The detailed current state lives in CNR3_THIS_CHAT_DELTA_current_state_SLIMMED_v4_12.md.
...
NEXT phase:                 live cache-pressure WIRING (hot-zone obs @arInitial, then prune) -> real-footage
                            validation -> diagnostics (condensed 4-phase) -> fmParallel. Prune/hot-zone LOGIC
                            proven but ZERO live callers; prune-trigger contract needs designer+coder review first.
```
...
### MOST RECENT PHASE — K.1D (DONE); the keystone is UNDER WAY; K.1E branch-(c) IS IN FLIGHT
```text
PHASE:    CMS07-K.1D — live frame-0 fresh-start store/return (the FIRST real output frame)
STATUS:   DONE. Committed and pushed. Baseline marker:
          CMS07-K.1D-live-frame0-fresh-start-store-return-proof
RESULT:   Debug 47/47 exit 0; Release 47/47 exit 0; forced-fail 46/47 exit 1; verbose 47/47.
          Plugin-only patch (two files); cache-core selftests unchanged. A/B harness green
          (frame-0 byte-identical to source; N>0 clean refusal = header-only y4m, no FRAME).
IN FLIGHT NOW:  CMS07-K.1E branch-(c) — live predecessor-present frame-1 compute — at
          ACKNOWLEDGEMENT-ACCEPTED / PRE-PATCH. Three confirmations accepted; a FOURTH is drafted
          but NOT yet sent (see the DELTA §4; it is the immediate next action). See Part 6B.
NOTE:     The pixel path is fully proven on CALLER-SUPPLIED frames, so the keystone reasons ONLY
          about frame SOURCING / lifecycle / ownership over an already-proven pixel path.
```
...
K.1A  keystone request-plan structures + temporary KDT dev-trace  <-- DONE (committed; count 46)
K.1B  direct cached-output-return ownership (synthetic)           <-- DONE (committed; count 47)
K.1C  live getFrame passthrough scaffold                          <-- DONE (committed; plugin-only)
K.1D  live frame-0 fresh-start store/return (FIRST real output)   <-- DONE (committed; copyFrame; plugin-only)
K.1E  branch (c): live predecessor-present frame-1 compute        <-- IN FLIGHT (pre-patch; 3 confirmations accepted, 4th not sent)
...   branch (d) recovery live wiring; multi-frame VS-LIFECYCLE-01; live scene-change wiring; longer sequential run  <-- OWED
...
```text
CMS07.13                      cnr3_cache_manager_design_v7_13.md      — controlling DESIGN authority (§0A charter; §9.5 materialized-floor invariant)
...
Document A v3.5                Document_A_CNR3_Project_Context_and_Standing_Rules_v3_5.md — context + §3A register incl. charter §3A.5.0 (§3A authoritative per R-PACK-02)
Document B v3.5.1              Document_B_..._Current_State_v3_5_1.md  — current build state + work plan (top UPDATE block authoritative; through D.5)
...
Companion v7.13               CNR3_CMS_Future_Investigations_..._v7_13.md — NON-NORMATIVE, NOT in coder pack (FI-01..08; lockstep with CMS)
Reviewer Intro v3.3           CNR3_Handover_Introduction_to_new_reviewer_chat_v3_3.md — concise entry point / role for the reviewer
This document v1.10           CNR3_Designer_Reviewer_Role_Handover_v1_10.md — the role/disposition handover
```
