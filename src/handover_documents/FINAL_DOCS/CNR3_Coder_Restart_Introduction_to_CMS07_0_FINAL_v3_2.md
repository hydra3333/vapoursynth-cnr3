...
over lock scopes.** It protects a single `addFrameRef` or `freeFrame` operation only. It
is not a licence to take a pin outside the cache lock or to reduce any cache critical
section.
...
The authoritative source is Production Spec §3A. **Non-authoritative orientation only —
the one-line summaries below are NOT the rules; §3A holds the authoritative wording.**
They are a quick pointer to likely early failure points:
