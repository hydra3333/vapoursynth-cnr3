...
## HANDOFF SUMMARY — r2 -> C (read this first)

> **Round 2 (designer) summary for the next receiver (coder).**
>
> **!! ARCHITECTURE CORRECTION (most important):** there is **NO "checkpoint pool"** and has not been for a
> long time. The current design is a **SINGLE UNIFIED CACHE** (`slots_`, one `std::vector<Cnr3CacheSlot>`)
> in which **"checkpoint" is a bool FLAG on a slot**, governed by a **separate RETENTION RULE**
> (`CHECKPOINT_MIN/MAX_RETAIN` are retention COUNTS, not a pool capacity; `checkpoint_count` is an INDEX of
> flagged-slot positions, not a container). This is CMS §6.1 ("a checkpoint is a FLAG ... NOT a pin"). Any
> earlier "checkpoint pool" phrasing in this register (SR-C-04, SR-D-05, comments #0011/#0016/#0023/#0028)
> is STALE — read it as "checkpoint-FLAGGED-slot retention." The substance of those findings still stands;
> only the vocabulary was wrong. **Do not build toward a pool container.** (Corrections: #0032-#0034;
> SR-D-05 header rewritten in-place with a visible marker.)
>
> **What was decided in r2 (verified against the live src):**
> - SR-C-01 (AGREED): a prune must NOT be inserted inside `store_owned_frame_locked` — verified that store
>   happens BEFORE pin there, so the just-stored slot would be unpinned. Use a NEW combined locked helper:
>   store/adopt -> set checkpoint flag -> pin/record (only if AS2 consumer) -> retire stale zones -> prune
>   decide/detach -> unlock -> free losers/victims. (Ratified as SR-D-07.)
> - SR-C-02 (AGREED): the final TARGET and FRAME-0 stores do NOT pin output[N] (production-never-pins).
>   output[N] is protected during a same-activation prune by **hot-zone membership** from the arInitial
>   observation of N, not by a pin. So `record_hot_zone_observation(N)` at arInitial is a HARD PREREQUISITE
>   of the target/frame0 store-prune.
> - SR-C-03 (AGREED): pass a CONSERVATIVE once-computed cached-output byte estimate to the prune trigger,
>   computed outside the cache lock.
> - SR-C-05 (AGREED): hot-zone retirement is LAZY inside the prune pass (retire-before-select), not eager at
>   arInitial.
> - SR-D-04 (RESOLVED): single-activation wiring now; concurrent prune/observation DEFERRED to fmParallel
>   (FI-06/07/08), written to the all-under-`cache_mutex_` discipline so it won't need restructuring.
> - SR-D-06 (RESOLVED): the non-checkpoint capacity trigger is self-debouncing by construction.
>
> **OPEN / needs the next receiver:**
> - **SR-C-04 / SR-D-05 — X (coordinator) RULING pending; CODER ADVICE REQUESTED first.** The question:
>   CMS §6.3 reads as if checkpoint trimming has an INDEPENDENT trigger (fires when flagged-slot count >
>   MAX_RETAIN), but the live code has no such trigger — checkpoint-flagged slots are only trimmed AS
>   COMPOSITE CANDIDATES within the total-slot capacity-pressure prune (verified this round). So the CMS
>   prose promises a trigger the code does not have. Two options:
>     (A) AMEND the CMS §6.3 prose to match the code — checkpoint trimming occurs within the capacity-pressure
>         bounded prune (composite selection via retain count), no standalone trigger. No code change.
>     (B) ADD an independent trigger on (flagged-slot count > MAX_RETAIN), separate from capacity pressure.
>         Requires a new/extended proven prune pass + selftest before live wiring.
>   **Designer lean: (A)** — the flagged set is already bounded by trimming at every capacity prune and by
>   MAX_RETAIN at promotion, so a second standalone prune path adds little before fmParallel; the prose just
>   over-promised. **CODER: before X rules, please advise on (A) vs (B)** from an implementation standpoint —
>   in particular: (i) is there any SINGLE-ACTIVATION scenario where the flagged-slot count can grow past
>   MAX_RETAIN while total slots stay UNDER the capacity ceiling (i.e. would option A ever leave the flagged
>   set unbounded in practice)? (ii) if (B) were chosen, how much new code/selftest does an independent
>   flag-count trigger actually need? Record your advice as a comment on SR-C-04; X rules after.
> - **SR-D-07 (OPEN) — coder to confirm:** the ratified 6-step combined-helper order is buildable, and the
>   step-(3) "AS2 consumer vs production store" distinction is cleanly determinable at the call site.
>
> No code produced (Step 0 stays a paper review until the contract is locked). Next free comment seq: #0035.
...
```text
BLOCKER: open 1 / total 4   (open: SR-D-02 [waits on SR-C-02 hot-zone basis, basically settled])
MAJOR:   open 3 / total 7   (open: SR-D-05/SR-C-04 [X ruling on checkpoint trigger], SR-D-07 [ratify helper order])
MINOR:   open 0 / total 2   (SR-D-06 RESOLVED, SR-C-06 AGREED)
NIT:     open 0 / total 0
...
### SR-D-02   [severity: BLOCKER]   [status: PARTIAL]
...
### SR-D-05   [severity: MAJOR]   [status: PARTIAL]
...
### SR-C-04   [severity: MAJOR]   [status: PARTIAL]
