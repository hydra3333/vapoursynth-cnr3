...
**Version:** v05 — adds Section 13 (Document B decision cross-check: which prior
settled decisions survive / are superseded by the new architecture; D13 overturn
flagged) and Section 14 (handover production-spec + Doc C notes). v04 mined Doc A.
v03 closed Items 3+7. v02 closed Item 6 + bounded prune. v01 covered foundation,
Position B, pinning, arInitial/arAllFramesReady atomics, new hot-zone scheme,
checkpoints, Q1/Q2.
...
- **V1 — CNR2 temporal reach:** is the source window genuinely backward-only
  `[N-B, N]`, or does CNR2's noise estimate peek forward? **CORROBORATED by
  Document A v2.0 §A4:** the blend is `output_chroma[N] = blend(output_chroma[N-1],
  source_chroma[N])` plus a downsampled-luma compare against the previous frame —
  i.e. predecessor OUTPUT + current SOURCE only, no forward source peek. Scene-change
  detection resets (copies current source, skips recursive blend). Backward-only
  window assumption is well-supported; confirm against actual code, but no longer an
  open guess.
...
- **V8 — CNR3 operating pixel format (bit depth + subsampling):** determines
  ...
  hits the 1000-frame hard ceiling (Section 9A). **NARROWED by Document A v2.0 §A3:
  integer YUV formats only (no float).** So the question is purely bit-depth (8 vs
  10/16) and subsampling. Working assumption 8-bit 4:2:0 PAL 720×576 → ceiling binds
  at 1000 (~590 MiB). Confirm the actual integer format; if 10/16-bit, recompute the
  derived ceiling.
...
## 12. Reusable material from Document A v2.0 (for the CMS07.0 introduction)

Most of Document A v2.0 is superseded (the H15.6B held-ref anti-regression rules
A0.1–A0.5, version-history notes A17–A20, stale boot sequence A16). But the
PROJECT-ORIENTATION material is high quality and pitched for a reader who
remembers nothing — adapt it for CMS07.0's introduction rather than composing
from scratch. Specifically:

- **§A2 (orientation preamble) — adapt near-verbatim for the CMS07.0 intro.**
  "CNR3 is not a normal stateless filter; output[N] depends on the *already-filtered*
  output[N-1] (NOT source[N-1]); a modern VapourSynth graph may request frames out
  of order, e.g. 0,3,1,2,4; a naive previous-frame-only implementation fails at
  frame 3 because filtered output[2] doesn't exist yet; therefore the cache is a
  CORRECTNESS subsystem, not a performance cache." Crisp statement of why the whole
  subsystem exists.
- **§A8 (goals) — SOURCES the fmParallel-final-goal invariant** we flagged as
  missing from the CMS. Use this text outright near the top of CMS07.0:
  "safe under fmUnordered now / structurally compatible with fmParallelRequests /
  final design target: fmParallel; interim development may pass through fmUnordered
  and fmParallelRequests, but design/coding must avoid choices that block eventual
  safe fmParallel unless explicitly justified and recorded." (The doc-update item
  is now sourced, not to-be-composed.)
- **§A4 (algorithmic core) — the pixel algorithm we treated as a black box.**
  Y: copy luma unchanged. U/V: compare current source chroma vs previous filtered
  chroma AND current downsampled luma vs previous downsampled luma; use signed-diff
  response tables; blend `output_chroma[N] = blend(output_chroma[N-1],
  source_chroma[N])`. Scene-change detection → copy current source chroma, skip
  recursive blend. (This is what corroborates V1 backward-only.)
- **§A3 (identity/scope):** API4-only; **integer YUV only (no float)**; inspired by
  CNR2/vscnr2; for analogue/video-capture chroma shimmer / temporal chroma noise.
  (Narrows V8 to bit-depth + subsampling only.)
- **§A9 (file architecture):** cache manager lives in
  `cnr3_output_cache_manager.h/.cpp`; per-frame pixel work in
  `cnr3_frame_internal_processing.h/.cpp` and MUST NOT own cache or scheduling
  policy; `Cnr3Data` (in `cnr3_common.h`) is per-instance and currently owns BOTH
  the old strict-streaming bridge AND the new output cache. Useful for CMS07.0 to
  reference where things live.
- **§A5 (arInitial/arAllFramesReady semantics) + §A6 (mode descriptions):** clean
  statements; A5's "register hot-zone activity at arInitial, not arAllFramesReady"
  aligns with our pin-at-arInitial decision.
- **§A12 (code-comment rule) — BACKS our "coder codifies CR1–CR5 as comments"
  decision.** A12 already mandates NOT over-compressing safety-critical comments
  (locking/threading invariants, ownership/lifetime, refcount discipline,
  non-obvious pre/postconditions). So codifying the coherence rules as constant
  comments is an APPLICATION of existing A12, not new policy. §A13 (before/after
  patch-instruction rules), §A14 (phase naming), §A15 (commit format) remain the
  standing coder-facing process rules and carry forward unchanged.

**NOT to carry forward:** A0.1–A0.5 (old held-ref H15.6B reservation model —
superseded by Position B); A16 boot sequence (stale refs); A17–A20 version notes.

---

## 13. Document B decision cross-check (the bring-across-and-revalidate map)

Document B v2.0 is the authoritative SETTLED-DECISION ledger (D01–D58). The new
architecture must be cross-checked against it so CMS07.0 neither silently
contradicts a settled decision nor loses one. Classification below.

### 13.1 Foundational decisions that SURVIVE unchanged (carry forward; several
are the very mechanisms our pin scheme is built on):
- **D01** API4-only, no fmFrameState. ✓
- **D03** output cache per-instance, never global. ✓
- **D04** ordered frame-number maps (nearest prior checkpoint = highest frame ≤ N).
  ✓ — our anchor/recovery logic depends on this.
- **D05** pool slots OWN refs, cache_index is NON-owning, release exactly once. ✓
  — foundation of "cache holds references" (1.1) and the pin scheme.
- **D06** lookup helpers addFrameRef WHILE holding the mutex
  (`cnr3_output_cache_find_frame_and_add_ref`). ✓ — this IS our find-and-pin
  primitive; it's why search-and-pin is atomic.
- **D07** first-in-best-dressed; duplicate store no overwrite/no extra addref;
  losing computation releases its own frame. ✓ — exactly Section 4.4.
- **D08** sliding hot zones, not extend-only. ✓ — retained (zone machinery).
- **D09** pruning hot-zone-aware, centralised through the single remove helper. ✓
- **D10** active_ceiling frame-count based, derived from 1 GiB budget. ✓ —
  Section 9A. D10 is the authority for the derivation.
- **D11** store prunes before hard-ceiling rejection. ✓ — Section 8.5.
- **D14** diagnostics prove safety before authority; non-zero error counter = hard
  gate. ✓ — our instrumentation stance.
- **D15** hot-zone update at arInitial. ✓ — directly supports pin-at-arInitial.
- **D16** per-invocation frameData, not shared Cnr3Data state. ✓ — foundation of
  the pin-list-in-frameData.
- **D26 / D33** old_strict_cache.next_needed/prev_output are NOT final fmParallel
  authority; must be retired/bypassed/redesigned before final authority; review
  before parallel. ✓ — still a live hard gate; carry forward prominently.
- **D27 / D28** recovery/warm-up compute MUST reuse existing processing boundaries;
  NO parallel pixel/frame algorithms without explicit agreement (override
  discipline). ✓ — durable rules, carry forward verbatim. (Our walk/recompute uses
  `process_cnr3_frame_with_explicit_previous_output`, NOT new pixel code.)
- **D29** bounded-start S>0 reset/start semantics (documented approximation). ✓
- **D30** compute / store / return-decision / return-transfer / output-authority
  each separately provable. ✓ — this is the proof-granularity rule our phase
  splits honour.
- **D31 / D32** CMS02-J0 mandatory before CMS02-J; compile-time diagnostic
  consolidation is a J0 deliverable. ✓ — carry forward as mandatory gate.
- **D36 / D37** sequential predecessor reuse valid when find_and_add_ref(N-1)
  succeeds; fast path = source N + cached output N-1 via explicit-predecessor
  boundary. ✓ — consistent with our scheme (now generalised: the predecessor is
  PINNED, and the same applies to walk intermediates).

### 13.2 The ONE foundational decision the new architecture OVERTURNS:
- **D13 — "non-checkpoint pinning is DEFERRED unless diagnostics prove need."**
  SUPERSEDED by Position B. We have decided consumer-pinning of all actively-needed
  frames is the MANDATORY baseline (Section 2), not a deferred escalation. CMS07.0's
  decision log MUST record this supersession EXPLICITLY with reasoning (goal-driven
  + KISS), exactly as Doc B records other supersessions (e.g. D46). Otherwise a
  future chat will object "D13 says pinning is deferred." This is the single most
  important entry for the CMS07.0 decision log.
  - Related: **D58 / Section 8 of CMS06.11** (fail-closed-only H15.6B draft retired)
    — stays retired; consistent.

### 13.3 The H15.6B-cluster (D42–D58, D38–D41): superseded IN MECHANISM, sub-rules
RETAINED. These describe the OLD held-ref-only predecessor-reservation model
(Position A). Position B supersedes the mechanism, BUT several sub-rules survive
and must be carried forward (generalised from "the reserved predecessor ref" to
"any consumer pin"):
- single-ownership / null-on-consume discipline (D45/D53/D54) → generalises to the
  pin-list discharge discipline (Section 3.5).
- reserved/owned refs participate in the existing lookup-ref accounting
  `acquired == released + transferred` (D55) → Section 13.18.6 / our 9B.
- ordinary cached-frame refs use lookup-addref, NOT pinning-as-a-distinct-kind
  (D56) → CORRECTED/clarified: under Position B there IS one pin concept
  (consumer-claim); D56's intent (don't conflate with checkpoint pins) survives as
  "checkpoint = flag, not pin" (Section 6).
- H17 sparse-hole deferred (D48/D57) → retained (Section 13.19 / our 9B refs).
- Option A (rely on serial callback ordering) REJECTED (D49) → retained as rejected.
- CMS07.0 must handle these per-decision: supersede the held-ref MECHANISM, retain
  the listed sub-rules, and say so explicitly (no silent drops).

### 13.4 Action for CMS07.0 decision log:
Issue new decisions that (a) record Position B as adopted and D13 as superseded;
(b) record the new pin scheme, zone-as-decay-hint scheme, bounded-prune, and
single-cache-wide-lock as settled; (c) carry forward D01/D03–D11/D14–D16/D26–D33/
D36–D37 unchanged; (d) mark the D42–D58 held-ref mechanism superseded with sub-rules
retained as listed. Use Doc B's decision format (Decision / Rejected alternatives /
Reason / Implementation consequence / Override discipline / Reference).

---

## 14. Handover production spec + Document C notes

**14.1 Handover Pack Production Spec v1.5** governs HOW the A/B/C pack is produced
when CMS07.0 lands. Key rules that bind our doc-update task:
- Continuity-preserving, non-regressive, NO abbreviation-by-default (§2A.1–2A.3):
  build the new pack FROM the previous approved A/B/C as baseline; correct in-place;
  never regenerate as a short summary; preserve all still-valid context/examples/
  rationale/warnings. (This is the SAME discipline as our item-4 bring-across-and-
  revalidate — they agree.)
- Matched version number across A/B/C; zip bundle; boot sequence; new-chat starter
  prompt (§8); the §11 Final Production Checklist is the acceptance test.
- Authority wording "CMS06.3-or-later" becomes "CMS07.0-or-later" throughout when
  we produce the pack.
- §9.1–9.3 update triggers: Doc A on project-wide/durable-rule/architecture change
  (CMS07.0 qualifies — major architecture change → Doc A WILL need updating); Doc B
  on major decisions/supersessions (CMS07.0 qualifies); Doc C every session.

**14.2 Document C** is the volatile current-state authority (current phase, last
completed, output authority, next task, latest test evidence, do-not-implement
list, files needed, hard gates, deferred notes). NOT mined for design content —
it's a state snapshot. When CMS07.0 lands, Doc C's "next task" becomes the new
phase plan and its do-not-implement / hard-gate lists are updated. (The current
Doc C content is about the OLD H15.6B path and is superseded by this session's work
on the cache architecture — but note CMS07.0 is a DESIGN supersession; the
implemented CODE state Doc C describes is still the real code state until coding
against CMS07.0 begins. Keep design-supersession and code-state distinct.)

**14.3 Important distinction to preserve:** this session changed the DESIGN
(architecture). It did NOT change the CODE. The code is still at the
CMS06.11-described state (sequential fast path proven through H15.5; H15.6B not yet
implemented). So CMS07.0 is the new design authority, but the first coding step
against it must be reconciled with where the code actually is. Doc C remains the
truth for code state; CMS07.0 for design intent. Do not let the design
supersession imply the code has moved.
