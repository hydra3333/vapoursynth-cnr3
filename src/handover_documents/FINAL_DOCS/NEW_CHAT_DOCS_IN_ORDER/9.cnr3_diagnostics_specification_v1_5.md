# CNR3 Diagnostics Specification v1.5

**Status:** Draft v1.5 for review and iteration, separate numbering convention to CMS
**Date:** 2026-06-21  
**Design epoch:** CMS07.0 restart  

**v1.5 change (additive clarification only):** adds §2.8 distinguishing the formal,
permanent CNR3_DIAG_* / D-SUM framework (this specification) from the TEMPORARY keystone
development trace used during the cache-to-pixel/getFrame keystone phase (K.1A-K.1G). The
keystone dev-trace (stderr line prefix [KDT], end-of-run [KDT-SUMMARY], compile guard
CNR3_KEYSTONE_DEV_TRACE) is deliberately OUTSIDE this framework, carries no D-SUM
compute/print gate and no R-PROCESS-19 macro-off obligation, and is removed in a
post-proven cleanup patch. The note exists so [KDT] is not mistaken for a gap in, or an
addition to, the formal framework. No D-SUM summary, field, or gate is added, changed, or
removed; this does not itself trigger R-PROCESS-19.
...
### 2.8 Temporary keystone development trace ([KDT]) — outside this framework

During the cache-to-pixel / getFrame keystone phase (sub-phases K.1A-K.1G) the build emits
a TEMPORARY development trace to make the otherwise-invisible predecessor-sourcing decision
visible while it is being implemented and validated. This trace is **not part of this
specification** and must not be confused with it:

```text
Formal D-SUM framework (this spec)         Temporary keystone dev-trace ([KDT])
-----------------------------------        --------------------------------------
CNR3_DIAG_* names                          CNR3_KEYSTONE_DEV_TRACE compile guard
end-of-run summaries                       one compact stderr line PER FRAME, prefix [KDT],
                                             plus one [KDT-SUMMARY] line at end of run
has compute/print gates                    no D-SUM gate
changing a gate triggers R-PROCESS-19      NO R-PROCESS-19 obligation
permanent, durable                         REMOVED in a post-proven cleanup patch once the
                                             K.1G out-of-order proof is green and committed
```

It shares this spec's hard global rules — **stderr only (§2.1)** and **never printed inside a
locked / atomic scope (§2.2)** — but it has no compute/print gate, carries no R-PROCESS-19
macro-off obligation, and is throwaway. It exists so that a failing seek/recovery behaviour
can be located during development (a pass/fail proof says "wrong", the trace says "wrong
WHERE"). The full specification of its fields and the labelled test scenarios it supports
lives in the keystone coder message (the policy-answers / CMS-conformance document) and the
`.vpy` test harness, which are written against the exact tokens [KDT] / [KDT-SUMMARY] /
CNR3_KEYSTONE_DEV_TRACE.

This note is recorded here ONLY so the keystone dev-trace is not mistaken for a gap in, or an
informal addition to, the formal D-SUM framework. When the keystone is proven and the
dev-trace is stripped, whatever PERMANENT observation the keystone genuinely needs (if any)
is added to this framework as a normal D-SUM summary at that time; the [KDT] tokens are not
durable and are intentionally absent from the D-SUM catalogue.
