# Document B — CNR3 Work Plan and Current Build State (CMS07.2, RESUME)

**Version:** v3.2 (RESUME-state work plan; supersedes the v3.1 "first milestone" work plan)  
**Date:** 2026-06-18  
...
## 8. Remaining work plan (after H.2A)

```text
H.3A (and any H.4A):
    First real AS2 consumer from recovery: use the AS2 combined store-and-record helper
    to repair holes. THIS is where the G.12A usage note becomes load-bearing — every AS2
    call yields one recorded pin to discharge, even a duplicate store that rejects the
    incoming frame and pins the existing winner. The recovery planner must account for
    discharging those pins.

C.14A — aggregate cache-core proof (the milestone capstone):
...
Then (downstream of a proven, complete cache core):
    ...
    - VapourSynth getFrame integration (arInitial/arAllFramesReady), source request/
      retrieve lifecycle, return-transfer.
