...
**Candidate lever (NOT a decision).** A typed-row-pointer / in-place native-access rewrite of the
P.7A-P.11B pixel path — template the plane traversal on sample type (const uint8_t* / const uint16_t*
with stride), do the arithmetic in a wider accumulator inline, write straight to the native output —
would collapse unpack+process+repack from three passes to one and delete the per-frame vector
allocation. Estimated ~1.5-2x on this path (the fraction eliminated is ~half of frame time). This is a
plausible, well-localized optimization, but it is NOT scheduled by this item.

**Open questions to settle IF/WHEN an optimization arc is opened.**
1. Does the multi-threaded (num_threads>1, higher -r) case change the calculus? The marshalling
   parallelizes across cores (each frame independent), so per-frame cost is unchanged but wall-clock
   throughput scales; confirm CPU-bound vs decode/IO-bound at full parallelism before sizing ROI.
...
3. Buffer reuse alone (pre-sized, frame-persistent scalar buffers) is a smaller, lower-risk partial
   win that removes the allocation churn without the full typed-pointer rewrite; weigh as a cheaper
   first step.
