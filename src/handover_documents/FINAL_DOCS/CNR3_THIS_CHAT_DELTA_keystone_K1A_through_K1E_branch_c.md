...
- **Doc updates this chat is producing** (so you can check them): Role Handover **v1.5 → v1.6**
  (state pointer only), Introduction **v2.0 → v3.0** (large — its baseline was the obsolete
  CMS06.11/H15.6B era), and this DELTA as the Document B companion. See Section 9.
...
- **Role/Reviewer Handover `v1.5` → `v1.6`** — a **state-pointer-only** update. Changes confined to:
  ...
  table (CMS **v7.7.1**, companion **v7.8** [contains FI-04], diagnostics **v1.5**, Document B
  **v3.2.8**, this DELTA); (3) Part 6B keystone hunting list annotated to mark **what K.1A–K.1D
  proved** (request-plan shape; synthetic ownership accounting; live passthrough scaffold; first real
  output[0] via copyFrame; N>0 clean refusal) and **what is still owed** (branch (c) predecessor
  consumption — in flight; branch (d) recovery; multi-frame VS-LIFECYCLE-01; live scene-change;
  post-K.1G KDT cleanup). **Everything else in the Role Handover (Parts 1, 3, 4, 5, 6, 6A, 7, 8, 9 —
  the entire disposition transfer, disciplines D1–D15, worked examples, accuracy rule) is byte-unchanged
  and remains correct.** If a v1.6 you are handed differs from v1.5 anywhere outside those three spots,
