...
**Version:** v1.1
**Date:** 2026-06-19
**Status:** CLEAN-SEAM handover. v1.1 supersedes the v1.0 interim issue: P.1A is now proven,
committed, and pushed (it was "in flight" in v1.0), so this is now a clean-seam handover with
no phase in flight — P.2A is the next phase and has not yet been proposed (see Part 2). This
is the companion to Document B: **Document B says WHAT is done; this document says HOW the
design/review role is performed.** It exists so that if the current designer/reviewer chat
ends, a new chat (and Dave) can re-establish not just the factual state but the *way the work
has been done* — the review disciplines, the decision heuristics, and the reflexes that
produced the quality so far.

**v1.1 change:** P.1A moved from in-flight to done (Part 2 state pointer now reads "proven
through P.1A, 33/33"; in-flight marker now "none — P.2A next"). Added a note recording that
P.1A went patch→four-way→commit and was read-first reviewed POST-COMMIT (sound; see Part 2),
with the process lesson that read-first-before-apply should be firm again from VS.1A onward.
No discipline (D1–D15), trigger, worked example, or checklist changed.
...
Latest committed phase:     CMS07-P.1A-response-table-salvage-vector-proof (first pixel phase)
Selftest count:             33/33 PASS (forced-fail 32/33, exit 1)
...
contiguity guard)** → **C.14A (aggregate proof — cache-core milestone)** → **P.1A (response-table salvage, first pixel phase)**.

### MOST RECENT PHASE — P.1A (DONE) and the NEXT phase (P.2A, not yet proposed)

The project has pivoted to the **downstream pixel/integration arc**. The first phase of that
arc, **P.1A**, is **proven, committed, and pushed**:

```text
PHASE:    CMS07-P.1A — response-table salvage and vector proof
STATUS:   DONE. Committed and pushed. Baseline marker:
          CMS07-P.1A-response-table-salvage-vector-proof
RESULT:   Debug 33/33 exit 0; Release 33/33 exit 0; forced-fail 32/33 exit 1; verbose 33/33.
          No D-SUM change (no macro-off run needed).
IN FLIGHT NOW:  NONE. The next phase is P.2A (pixel-configuration parameter surface for
          response tables), which has NOT yet been proposed. When it is proposed, review it
          as text first (Part 1 cadence), establishing the proof approach and salvage
          governance before any patch.
```

What P.1A delivered (so a new chat has the context for what P.2A builds on):

- Salvaged two PURE functions from `src/superseded_by_v7/cnr3_response_tables.cpp.txt/.h.txt`
  into the active `src/cnr3_response_tables.cpp/.h`:
  ...
  adapted to return `Cnr3Status`: invalid_argument for bad geometry, allocation_failed for
  bad_alloc).
- Did NOT salvage `build_cnr3_lookup_tables(Cnr3Data&, VSMap*, const VSAPI*)` — it depends on
  Cnr3Data/VSMap/parsed-mode/VS-error-reporting and is deferred to a later deliberate
  config-parsing phase (this is the kind of thing P.2A may begin to address — the config/
  parameter surface). Cnr3Data was NOT resurrected.
- Added ONE selftest `response_table_vector_proof` to the EXISTING `cnr3_cache_core_selftest`
  runner (NOT a new module — the baseline lacks .vcxproj files, so a new module's VS project
  wiring could not be verified; revisit a separate pixel selftest module at P.2A/P.3A when the
  suite grows and project wiring can be reviewed). Count 32→33.
- Proved NUMBERS via known-input → known-output reference vectors, asserting EXACT integer
  equality (no tolerance — bit-exact vscnr2 compatibility is the point). Six vector groups plus
  an invalid-argument path test. ALL expected values were independently verified in Python
  against the salvaged formula and found correct (narrow 255/10 d5=127, wide d5=216, peak=254
  from the strength/2 integer-division quirk; the 200/20 family narrow d7=145, wide d7=192;
  threshold-zero; clamp groups giving d0=255 and d0=0). The committed patch was read-first
  reviewed POST-COMMIT and confirmed sound: formula transcribed exactly, lookup total, status
  adaptation clean and tested, non-scope held.

The salvaged vscnr2 formula now lives in `build_cnr3_weight_table` (P.3A's weighted blend will
build on these tables — preserve the formula's meaning if it is ever touched):
...
PROCESS NOTE (for the record and the discipline): P.1A went patch → four-way → commit WITHOUT a
pre-apply read-first; the read-first review was done POST-COMMIT and confirmed the result sound.
That was defensible here because P.1A was low-risk (pure maths, touched no proven cache/atomic
code, and its reference vectors were independently verified before approval), so a transcription
error was the only real risk and the verified vectors would have caught it. This must NOT become
the default for phases that touch proven or atomic code: from VS.1A onward (and any phase
touching cache-core/lifecycle), read-first-BEFORE-apply is firm again — a post-hoc review can
only catch, not prevent, a bad apply to proven code. Pixel phases P.2A–P.6A are "new code, not
proven-cache", so pre- vs post-apply review is a judgement call there; lean to pre-apply for
...
P.2A  pixel-configuration parameter surface for response tables   <-- NEXT (not yet proposed)
...
This document v1.1            CNR3_Designer_Reviewer_Role_Handover_v1_1.md — the role/disposition handover
```
