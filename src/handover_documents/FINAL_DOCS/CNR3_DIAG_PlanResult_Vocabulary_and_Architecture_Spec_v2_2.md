# CNR3 — PLAN/RESULT DIAGNOSTIC (D-SUM PLAN-TRACE) — VOCABULARY + ARCHITECTURE SPEC v2.2
...
```text
    (a) sort by enter-tick (display column: UTC)  — temporal: what arrived/began-completing when
        primary key : enter_tick   ASC
        tie-break   : action_seq   ASC
...
```
**View (a) sort key is the ordered tuple (enter_tick ASC, action_seq ASC). [clarified v2.2; §6 already keyed
this view on the tick, this makes it explicit]** `phase` is NO LONGER a sort term for view (a) — it remains a
DISPLAYED field. Reasoning, bound in place:
- **enter_tick primary** gives the printed UTC timestamp column a MONOTONIC ASCENDING read down the block —
  ...
  readable-column property is the whole reason view (a) exists. (The sort is on the enter TICK, per §6; UTC
  is the derived display column — NOT a string sort on the datetime. The old label "sort by enter_datetime"
  was tightened to "enter-tick (display column: UTC)" to prevent exactly that misread.)
- **action_seq tie-break** gives a DETERMINISTIC, GAP-FREE TOTAL ORDER. It is globally unique per instance
  (the lock-protected counter — see §8 buffer invariant and §3), so it resolves EVERY remaining tie by
  itself. It supersedes `phase` as the tie-break because phase only distinguishes O from R and leaves
  same-tick ties among DIFFERENT frames (TT / RR) unresolved and non-deterministic; action_seq resolves all
  of them uniquely.
- **O-before-R still falls out naturally** — dropping phase as a sort term loses nothing. Within one frame
  the O record is entered and committed BEFORE its R record, so O carries the lower action_seq and sorts
  above its own R automatically. Pairing/readability is preserved by construction. Do NOT re-add phase as a
  tie-break "to be safe": it is strictly coarser than action_seq and redundant.
