...
(`CNR3_CMS_Future_Investigations_and_Open_Questions_v7.9.md` pairs with CMS07.9).
...
FI-04  Scaffold getFrame registration parameters are passthrough-correct, not
       filter-correct — the K.1C live scaffold registers fmUnordered + a
       per-source rpStrictSpatial dependency, both correct for a 1:1 passthrough
       but WRONG to carry unchanged into real recursive filtering, where
       output[N] = f(source[N], output[N-1]) plus a bounded recovery range. The
       real-filtering phase must re-derive the dependency declaration (and
       re-confirm the filter mode). Correctness item, not efficiency. OPEN.
...
### FI-04 — Scaffold getFrame registration is passthrough-correct, not filter-correct

**Status:** OPEN — assessment deferred to the first real-filtering / real-output-store phase
(the phase that replaces the K.1C live passthrough scaffold with real CNR3 output generation).

**Trigger (when to assess this):** when the K.1C live getFrame passthrough scaffold is replaced
by a callback that produces real filtered output[N] — i.e. the first phase where
output[N] = f(source[N], output[N-1]) is actually computed in the live getFrame path. This must
be assessed BEFORE that phase's patch is accepted, because the registration parameters are part
of what changes when passthrough becomes real filtering.

**The parameters in question.** The CMS07-K.1C live scaffold (`src/vapoursynth-Cnr3.cpp`,
`cnr3_create_filter`) registers the filter via `createVideoFilter(...)` with:

- **filter mode `fmUnordered`** — correct for CNR3's inherently serial recursive algorithm (per
  the settled mode decision: `fmUnordered` is the correct current mode, `fmParallelRequests` the
  development target, `fmParallel` the eventual correctness goal). For a single-frame passthrough
  this is trivially fine.
- **dependency `{ source, rpStrictSpatial }`** — declares that output frame N depends only on
  input frame N (a strict 1:1 spatial dependency). For a passthrough that requests exactly
  source[N] to produce output[N], this is exactly correct.

**Why both are passthrough-correct but not filter-correct.** The scaffold's access pattern is a
genuine 1:1 map: one source frame in, the same frame out, no history. `rpStrictSpatial` describes
that truthfully. But real CNR3 is recursive and bounded-non-local:

- output[N] depends on **output[N-1]** (the previous *filtered* output, R-ARCH-06), not only on
  source[N]; and
- when the predecessor is absent, the bounded recovery (CMS §9.5) requests a **descending range**
  of source frames down to the floor `max(0, N - BACK_RADIUS)` — so a single output activation can
  legitimately touch many source frames, not one.

`rpStrictSpatial` asserts neither of those. Carried unchanged into real filtering it would be a
**false dependency declaration**: it tells VapourSynth the filter has a strict 1:1 spatial access
pattern when in fact it has a recursive + bounded-range pattern. That is not a cosmetic mislabel —
the dependency declaration informs the host's frame-management/caching assumptions, so an incorrect
one is a latent correctness hazard once the access pattern stops being 1:1.

**Why this is recorded now.** The scaffold *set* these values, and they are *correct for K.1C*, so
nothing in the K.1C code or tests flags them as provisional — they look permanent. This is exactly
the class of "correct now, must change later" detail that is easy to lose between the scaffold phase
and the real-filtering phase. FI-04 exists so the dependency declaration is re-derived deliberately
at the real-filtering phase rather than inherited by default.

**Disposition / what the real-filtering phase must do (non-normative, for that phase to design):**
- Re-derive the `VSFilterDependency` set for a recursive predecessor-using filter — at minimum the
  source dependency is no longer `rpStrictSpatial`; the correct request type and whether the
  predecessor (prior output) participates in the declared dependencies must be settled against the
  VapourSynth API4 dependency model and the multi-frame VS-LIFECYCLE-01 work owed at the
  recovery-wiring phase.
- Re-confirm (not necessarily change) the filter mode against the real multi-frame request
  behaviour; `fmUnordered` remains the expected current choice, but the confirmation should be
  explicit rather than inherited from the scaffold.

**Related owed items (carried elsewhere, listed here for context — not duplicated as FI):**
- real `VSFrame` cached-output return — the integration half of branch (b) (coder-tracked);
- multi-frame VS-LIFECYCLE-01 — holes-only request set meeting real arInitial (coder-tracked,
  recovery-wiring phase);
- a genuine once-per-stream `[KDT-SUMMARY]` would need an end-of-stream signal, since KDT emission
  at plugin load/create/free is forbidden (minor; real-filtering KDT design).
