...
**Version:** Document A v3.6 — RESUME-state regeneration generated from Production Spec **v2.12**
populated §3.2 (canonical context, reproduced verbatim) and §3A (Prevailing Rules Register, reproduced
in full). **v3.6 change over v3.5 (currency touch only; NO register-owned rule text changed):** (a) advances the
...
fmParallel validation phase. **Next phase: live cache-pressure WIRING (hot-zone obs @arInitial then prune)** → real-footage → diagnostics → fmParallel
