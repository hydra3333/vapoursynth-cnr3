# CNR3 — Development RESUME on the CMS07.10 cache + pixel architecture
*(Coder restart introduction (v6.0) — paste this at the start of a new memoryless chat,
...
committed through D.1. It is NOT a fresh start, it is NOT "early cache-core work," it is NOT
pixel-path work, and the keystone dispatch is NOT un-started or partial — read the state below
carefully. The immediate live work is branch-(d) generalisation: the next subphase is D.2
(exact-anchor MULTI-hole recovery).
v6.0 supersedes v5.0 (which was on the K.1D / CMS07.8 / K.1E-branch-(c) baseline — many phases
and three CMS versions out of date). v6.0 also carries the new Design Alignment and Escalation
Charter (section 0.0).)*

This chat **resumes** CNR3 development on the CMS07.10 architecture. The build is far advanced
...
cache-hit (K.1F), fresh-start (K.1D), predecessor-present (K.1E.2/E.3), and recovery (D.1, the
first live branch-(d)). Your job is branch-(d) GENERALISATION — the next subphase is **D.2
(exact-anchor MULTI-hole recovery)** — after first confirming the build state from the repository.
...
architecture is the CMS07.10 design, and it **completely supersedes** the previous CMS06.x
...
KEYSTONE:            LIVE DISPATCH COMPLETE (all four branches), committed through D.1:
                       ...
                       D.1   live branch-(d) exact-anchor SINGLE-hole recovery   (first live recovery)
Latest committed:    CMS07-D.1-live-exact-anchor-single-hole-recovery-proof
Controlling CMS:     CMS07.10 (R-LIFECYCLE correction to 9A.1.1, proven by K.1F; pre-compute
                     adopt-and-skip normative since CMS07.9)
Selftest count:      49/49 PASS (forced-fail 48/49 exit 1; verbose 49/49).
...
are live and proven both Debug+Release. The plugin produces correct recursive output through
live getFrame, including reconstructing a missing predecessor via single-hole recovery (D.1).
What is NOT yet done, and is your immediate forward work, is branch-(d) GENERALISATION — the
next subphase is **D.2 (exact-anchor MULTI-hole recovery)** — see section 0.1.
...
D.2 state (section 0.1) because that
state is newer than Document B. Read both; where they overlap, Document B and the repository
win on build state.**

---
## 0.1 THE LIVE TASK — D.2 (exact-anchor MULTI-hole recovery)
The live getFrame dispatch is COMPLETE (all four branches). Your immediate forward work is
branch-(d) GENERALISATION, one step at a time, each harness-proven Debug+Release:
```text
D.2  exact-anchor MULTI-hole recovery (k>=2 holes; multi-pin discharge load-bearing) +
     bounded-window refusal                                              <- NEXT
D.3  floor-fresh-start recovery (no anchor in window: copyFrame base + walk forward)
...
### What D.2 must do (the shape; the designer will deliver a full scope first)
```text
Marker name (expected):  CMS07-D.2-live-exact-anchor-multi-hole-recovery-proof
Standard case: request output[N] where output[N-1] AND output[N-2] are ABSENT and output[N-3]
  is PRESENT (the anchor at N-3, k=2 holes).
  arInitial:        A-safe-1 routing (unchanged from D.1): cache-hit pin -> N==0 fresh-start ->
                    predecessor pin -> recovery. Recovery plans+pins the anchor and catalogues
                    the holes under the H.2A AS1 atomic. Accept gate WIDENS to admit k>=2 holes
                    (anchor == N-(k+1)); source set DERIVED as {N} U {all hole sources}; carry
                    the whole recovery plan in frameData.
  arAllFramesReady: fill the holes ASCENDING, each from its now-present predecessor (hole h from
                    output[h-1]); pre-compute adopt-and-skip per hole; store each via the AS2
                    hole-store; then compute the target output[N] from the last filled hole.
                    The MULTI-PIN discharge (anchor + each filled hole) is the load-bearing new
                    proof: D.1 already discharges TWO pins, D.2 stresses a larger pin-list.
  BOUNDED-WINDOW REFUSAL: if the nearest anchor is beyond the back-radius -> clean refusal
                    (distinct from D.3 floor-fresh-start). This is a NEW proof D.2 adds.
This phase proves exact-anchor multi-hole (k>=2) + the bounded-window refusal ONLY.
```
...
4. P.11C scene-change is DEFERRED uniformly across all live branches (scene_change_deferred=1).
   Do NOT wire scene-change into D.2 alone; it is a shared owed item to be wired across
   branch-a/c/d together before any real-footage test (see the owed-items ledger).
5. fmParallel goal: D.2's design note must carry an explicit interleaving analysis (the D.1 scope
...
6. Multi-hole golden note: at threshold=255 the recursive output moves ~1 LSB frame-to-frame, so
   the ROBUST D.2 proof is the KDT mechanism (hole_count=k, all computed, balanced multi-pin
   discharge) PLUS direct hole-byte checks via cache-hit follow-up — not the target byte margin
   alone. The designer computes and supplies the verified golden chain.
```

### Where D.2 lands (post-K.1G source layout)
```text
src/cnr3_arInitial.cpp          recovery branch START (cnr3_start_live_recovery): widen the accept
                                gate to k>=2; the bounded-window refusal lives here.
src/cnr3_arAllFramesReady.cpp   recovery branch COMPLETION (cnr3_complete_live_recovery): the
                                ascending multi-hole fill loop already exists from D.1 (it loops
                                over hole_frame_numbers); D.2 exercises it with k>=2 and proves the
                                larger pin-list discharge.
src/cnr3_plugin_internal.h      recovery frameData already vector-shaped (holes / source-requests /
                                per-hole outcomes) — designed to take extra holes WITHOUT reshaping.
```
...
1. This introduction (orientation only):
   CNR3_Coder_Restart_Introduction_to_CMS07_RESUME_v6_0.md
2. Controlling design authority:
   cnr3_cache_manager_design_v7_10.md
   (CMS07.10 — supersedes all earlier CMS07.x and CMS06.x. Carries: 9.7 keystone
    ...
    recovery model that D.1 wired and D.2 generalises.)
...
4. Current work plan + BUILD STATE  ->  DOCUMENT B:
   Document_B_CNR3_Restart_Work_Plan_and_Current_State_v3_5.md  (or highest present)
   (the live build state through D.1, the working method, salvage inventory, next phases, and the
    do-not-implement list. READ THIS for where the build actually is. Its v3.5 UPDATE block carries
    the K.1E.2/E.3 -> D.1 detail. If a higher version exists, use it.)
5. Production Spec (canonical context master + section 3A rules):
   CNR3_Handover_Pack_Production_Spec_v2_10.md  (or highest present)
...
- CNR3_THIS_CHAT_DELTA_keystone_through_D1_v4_4.md  (or highest present)
  (the keystone delta companion to Document B — the fullest single narrative of the keystone
   through D.1: the commits, the settled Option-C/R-LIFECYCLE finding, the A-safe-1 routing design
   road, the owed-items ledger, and the D.1 -> D.5 arc. If included, read it for depth.)
```
NOT part of the durable implementation authority (do not treat as controlling):
```text
- CNR3_CMS_Future_Investigations_and_Open_Questions_v7_10.md
  (NON-NORMATIVE companion to CMS07.10; deferred tuning / fmParallel-phase questions only; ignore
   for implementation. FI-04 is RESOLVED into CMS 9.7.7.)
...
```
If **CMS07.10 itself is not attached**, stop and say so. You may comment on this introduction, but
...
3. Build + run the isolated cache-core selftest (it also carries the pixel-proof tests and the
   K.1A/K.1B keystone selftests) and confirm the four-way:
       Debug   normal                              -> 49/49 PASS, exit 0
       Release normal                              -> 49/49 PASS, exit 0
       Release --force-fail-for-harness-proof      -> 48/49 PASS, 1 FAIL, exit 1
       Release --verbose                           -> 49/49 PASS, exit 0
...
```text
If CMS07.10 conflicts with, or is merely unclear in alignment with, prior material:
    CMS07.10 wins unless the user explicitly says otherwise.
If CMS07.10 itself is silent, ambiguous, or incomplete on an implementation point:
...
the latest prevailing CMS — currently **CMS07.10**. Specific CMS section pointers are
version-specific and must be re-checked against CMS07.10. All pre-CMS07 cache code/design is
