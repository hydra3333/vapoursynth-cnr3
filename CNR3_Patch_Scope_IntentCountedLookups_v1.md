# CNR3 — PATCH SCOPE: intent-counted cache lookup metrics (replaces uniform lookup counting) — v1
...
**Authority for the rules:** `CNR3_Cache_Lookup_Taxonomy_Findings_v05.md` (the census + W3X Decisions 1 and 2).
...
| 3 | recovery walk (cache_core:3598–3631) | candidates N−1, N−2, … | **full for every candidate EXCEPT N−1**, which is **hit-only** (a walk-N−1 MISS merely re-confirms site 2's counted miss; a walk-N−1 HIT is new concurrency information — a peer produced it between the two lock holds). Per walk: anchor found at candidate c → probes N−2..c+1 are counted misses (plus N−1 nothing-or-hit), c is a counted hit. Floor fresh-start → all probed non-N−1 candidates are counted misses, no hit. |
| 4 | hole-catalogue scan (cache_core:3641–3658) | plan derivation | **never counted** (unchanged from today — do not add) |
...
3. **Designer harness, NEW oracle** (the old lookup oracle is retired):
   ...
   - **L2 (`-r 1`, shuffle8):** derived by hand-check at proof time against the run's own plan counts
     ...
     site-3 walk arithmetic consistent with recovery counts. At `-r 1`, all hit-only concurrency sites
     (1-hit, walk-N−1-hit, 8-adopt, 10a/10b-duplicate) must contribute **zero** — any nonzero there on
     `-r 1` is a defect finding.
   - **L1noR (threaded):** hit-only sites may now be nonzero — record, don't gate; this is the new signal
     working. `hits + misses == queries` must still hold.
