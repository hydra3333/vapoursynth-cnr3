...
are marked `[VERIFY]`. Ready for coder study and the isolated cache-core milestone.
...
**Algorithmic core (§A4 — `[VERIFY]` against actual code, see V1):**
...
- For chroma U/V: compare current source chroma vs previous *filtered* chroma, and
  ...
  response.
- Scene-change detection: on a true cut, copy current source chroma and skip
  recursive blending (avoids smearing across the cut).
- The source window required to compute output N is therefore **backward-only**:
  `[max(0, N-B), N]` (predecessor output + current source; no forward source peek).
  `[VERIFY V1]` confirm no forward reach in the actual algorithm.
...
**6.3 Establishment and retention (DISTINCT rules; carried forward, `[VERIFY V2/V3]`):**
...
- INSIDE one atomic: search the window `[anchor … N-1]`; pin each PRESENT cached
  output (including the anchor frame itself); catalogue the OUTPUT HOLES (missing
  cached outputs); record all pins to the `frameData` pin-list; update hot zone(s)
  for N (D15). One indivisible operation.
- OUTSIDE the atomic: request from VapourSynth the SOURCE frames needed to fill the
  output holes (and source N). Slow (may trigger upstream decode) — never inside the
  lock.
- Safe because present frames are pinned (protected) and holes are absent (cannot be
  pruned). The plan snapshot stays valid after lock release.
- The "anchor" is the nearest usable recovery start: nearest checkpoint at/above the
  lower bound, else the bounded lower-bound floor `max(0, N-B)`.
...
- For each output hole, in frame-number order from the anchor forward: compute the
  output OUTSIDE the lock; then a brief per-frame atomic { first-in-best-dressed
  check; store; pin; record }; unlock; proceed. Store-and-pin MUST be the same atomic
  (else a gap lets another thread prune the just-stored frame).
...
**9.5 Recovery / fallback (carried forward from H16.3/H16.4; H17 deferred).** When N's
predecessor is an output hole, recovery walks forward from the anchor, filling holes
(pinning each intermediate as produced, per 9.2). Current fallback is correctness-
first and conservative; the optimised "start from nearest anchor, fetch only genuine
holes" path is **H17, deferred** (Section 13.19 equivalent) and MUST NOT be mixed
...
OVERFLOW_FACTOR           = 1.1
CHECKPOINT_INTERVAL       = 10        (+ frame 0)
CHECKPOINT_MAX_RETAIN     = 32  (soft)
...
- **CR2** BACK_RADIUS ≥ bounded recovery search window B; ideally = B (currently
  both 50).
- **CR3** BACK_RADIUS ≈ 5 × CHECKPOINT_INTERVAL (a zone covers ~5 anchors); 50=5×10.
- **CR4** active_ceiling ≥ ~2 × max-protected set, where max-protected ≈
  MAX_HOT_ZONES × (BACK_RADIUS + FORWARD_RADIUS) + checkpoint pool ≈ 5×60 + 32 =
  ~332; 1000 ≫ 332. (If pruning can never reach target, this rule is violated.)
- **CR5** CHECKPOINT_MAX_RETAIN ≥ MAX_HOT_ZONES × (BACK_RADIUS / INTERVAL) = 25;
  32 > 25.
...
- **V1** algorithmic temporal reach backward-only (Section 1) — corroborated by
  Document A §A4; confirm vs actual code.
