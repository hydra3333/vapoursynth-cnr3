...
For clean y4m piping, use:
    vspipe --container y4m script.vpy -
or the equivalent for the installed vspipe version.
```
...
Current intended frame path after v0.2:
...
The chroma-plane helper is currently pass-through again:
...
Current temporary name is acceptable during scaffolding, but once real logic starts it should probably be renamed to something like:
...
## 20. Next intended coding step

The next step should not be the full cache manager.

The next step should be the first real weighted blend skeleton.

Suggested cautious sequence:

```text
1. Keep current pass-through output as known-good baseline.
2. Rename temporary helpers only when ready:
       process_cnr3_frame_passthrough_for_now -> process_cnr3_frame
       process_cnr3_chroma_plane_passthrough_for_now -> process_cnr3_chroma_plane
3. Implement an 8-bit-only chroma blend path first.
4. For non-8-bit clips, either:
       keep pass-through temporarily, or
       return a clear "8-bit only for first algorithm stage" error.
5. Once 8-bit behaviour is correct, generalise to 10/12/16-bit.
```

The first real chroma logic should probably do:
...
U/V:
    ...
    read previous filtered output chroma
    calculate an absolute chroma difference or guard-weight input
    use lookup-table-derived weight
    write blended chroma
```

Potential first simple formula for proof, not final Cnr2 fidelity:

```text
dst = (src * (256 - weight) + prev_output * weight + 128) >> 8
```

But before implementing the formula, re-check vscnr2/Cnr2 behaviour so parameter meaning remains faithful.

---

## 21. Important future algorithm considerations
...
## 22. Developer style requirements from the user
...
The next chat should not jump into full cache-manager implementation unless deliberately asked. The better next step is to start the actual chroma blend carefully under strict mode, while preserving the current `Cnr3CacheManager` shape for future Policy C.
