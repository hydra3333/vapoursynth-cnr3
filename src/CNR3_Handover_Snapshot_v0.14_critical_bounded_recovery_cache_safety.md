# CNR3 Handover Snapshot v0.14 - Phase 4 Critical Bounded Recovery and Cache Safety Addendum
...
CNR3_Handover_Snapshot_v0.10_cache_manager_phase_2B1.md
CNR3_Handover_Snapshot_v0.11_cache_manager_phase_2G2.md
CNR3_Handover_Snapshot_v0.12_cache_manager_phase_2H1.md
```

This v0.14 snapshot is intentionally detailed. Do not compress it aggressively in a future handover unless the relevant detail has been superseded by code or by a later explicit decision.

This v0.14 revision specifically adds a critical Phase 4 pre-implementation warning about the no-prior-checkpoint bounded recovery policy, checkpoint retain sizing, temporary cache expansion/safety modelling, and the absolute requirement that bounded recovery must never break cache-manager ownership, pruning, indexing, pinning, or frame-reference management.
...
## 0. v0.13 critical delta since v0.12

This section records the latest state after the cache-manager Phase 3A/3B/3C work and the memory-diagnostics insertion. It is deliberately explicit because the next chat must not accidentally lose the decisions and safeguards made during this work.

Current branch/context:

```text
- Development continues on the cache-manager workstream for vapoursynth-cnr3.
- The latest local build succeeded and was committed after Phase 3C.7a.
- The v005 cache manager is now owned by Cnr3Data, but it is still not used by cnr3_get_frame() for output generation.
- Current frame-processing behaviour is therefore still the earlier strict-streaming behaviour.
- The v005 cache manager is infrastructure-ready for the next runtime-wiring phases, but not yet live in the algorithm.
```

Latest important files in the source tree:
...
The latest uploaded versions in the closing audit were:

```text
cnr3_build_config(1).h
cnr3_cache_manager(5).h
cnr3_cache_manager(5).cpp
cnr3_common(3).h
cnr3_memory_diagnostics(1).h
cnr3_memory_diagnostics(1).cpp
vapoursynth-Cnr3(5).cpp
```

### 0.1 Build-diagnostic configuration added

A project-wide build configuration header now exists:

```text
cnr3_build_config.h
```

It defines compile-time diagnostic switches:

```text
CNR3_DEV_DIAGNOSTICS = true
CNR3_CACHE_MANAGER_DEV_DIAGNOSTICS = CNR3_DEV_DIAGNOSTICS
CNR3_MEMORY_DIAGNOSTICS = CNR3_DEV_DIAGNOSTICS
```

Important policy:

```text
- These are compile-time controls.
- Code guarded by if constexpr is compiled out when the relevant switch is false.
- Diagnostics must not be required for correctness.
- CNR3_CACHE_MANAGER_VALIDATE_AFTER_MUTATION remains in cnr3_cache_manager.h because it is a cache-manager-specific policy switch.
- It defaults to CNR3_CACHE_MANAGER_DEV_DIAGNOSTICS.
```

The user preferred explicit includes rather than relying on transitive/includes-by-accident. Therefore `cnr3_build_config.h` may be included widely across CNR3-owned headers and source files. That is intentional and acceptable. Vendor VapourSynth API headers should remain unmodified.

### 0.2 Cnr3Data now owns the v005 cache manager and memory diagnostics accumulator

`Cnr3Data` now contains:

```text
Cnr3CacheManager cache;                 // old strict-streaming recursive cache, still active
Cnr3CacheManagerV005 cache_manager_v005; // new future output/checkpoint cache manager, not live yet
Cnr3MemoryStats memory_stats;            // per-instance process/system memory diagnostic accumulator
```

Important state:

```text
- cache_manager_v005 is per-instance/per-source, not global.
- memory_stats is per-instance and diagnostic-only.
- memory_stats does not affect frame processing.
- The v005 cache manager still does not participate in cnr3_get_frame().
```

### 0.3 Phase 3C cache-manager audit/fixes completed

The final Phase 3C state includes:

```text
3C.1 complete:
    Passive single-lock cache-manager debug snapshot added.
    The CNR3 debug summary no longer calls mutating validation.
    The CNR3 debug summary no longer reads multiple mixed-time helper snapshots.

3C.2 complete:
    checkpoint find-and-pin / direct pin / direct unpin rollback fixed.
    If post-mutation validation fails after a pin/unpin mutation, the mutation is rolled back before returning false.
    Direct pin/unpin now classify null checkpoint frames as integrity/null-frame errors.

3C.3 complete:
    Post-mutation validation failure counters added for store/remove/prune/prune-after-store paths.
    Debug summary includes post-validation failure headline counters.

3C.4 complete:
    checkpoint_prune_no_eligible_frames accounting fixed.
    If an eligible checkpoint exists but removal fails, that is represented by checkpoint_prune_remove_failures, not by no_eligible.

3C.5 complete:
    Shared build diagnostic configuration added.
    Windows memory diagnostics infrastructure added.
    create/free memory sampling added.
    min/avg/max memory summary added.

3C.6 complete:
    Documentation/counter-semantics cleanup completed.

3C.7 and 3C.7a complete:
    Final static audit completed.
    Per-metric memory diagnostic sample counts added.
    Memory diagnostic averages now divide by each metric's own sample count, not by total snapshots.
    Metric sample counts are printed in the memory summary output.
```

### 0.4 Cache-manager debug snapshot semantics

The v005 cache manager now supports a passive debug snapshot:

```text
cnr3_cache_manager_get_debug_snapshot(cache, snapshot)
```

Semantics:

```text
- Locks cache.cache_mutex once.
- Copies counts, pin state, highest cached frame number, stats, and passive invariants_ok.
- Its invariant check is passive.
- It does not increment validation counters.
- It is for summary/status output.
- It is not a replacement for mutating development validation after cache mutations.
```

This was added because the previous debug summary called multiple independently locking helpers and a mutating validation helper. That was memory-safe but could produce mixed-time diagnostics under future threaded use. The new snapshot avoids that.

### 0.5 Important cache-manager statistics/counter semantics

The following counter meanings were agreed and documented:

```text
reset_stats():
    Resets resettable diagnostic counters only.
    Does not clear cached output frames.
    Does not clear checkpoint slots.
    Does not clear cache indexes.
    Does not clear pin counts.
    Does not release frame references.
    Integrity/error counters are currently part of the resettable statistics block.
    Lifetime/non-resettable counters can be added later if testing shows that persistent integrity history is useful.

checkpoint_prune_skipped_frame_zero:
checkpoint_prune_skipped_pinned:
    These are skip observations, not unique checkpoint counts.
    Because checkpoint pruning may restart from begin() after each successful removal, the same surviving frame-zero or pinned checkpoint may be observed and counted more than once across a prune run.

checkpoint_prune_no_eligible_frames:
    Means no eligible removable checkpoint was found in that pass.
    It should not be used for remove-failure cases.
    Remove-failure cases are represented by checkpoint_prune_remove_failures.

post_validation_failure counters:
    These distinguish ordinary operation failure from development validation failure detected after a mutation already happened.
    Do not conflate them with ordinary store/remove/prune failures.
```

### 0.6 Pin/unpin safety decisions

For checkpoint pinning and unpinning:

```text
- A helper returning false must not leave behind a new unreported pin/unpin mutation.
- find_and_pin only reports success after validation passes.
- If validation fails after incrementing pin_count, the increment is rolled back and checkpoint_frame_number is reset to -1.
- Direct pin rolls back its increment if validation fails.
- Direct unpin rolls back its decrement if validation fails.
```

Stats meaning after the fix:

```text
checkpoint_pin_successes:
    The pin is actually still in effect.

checkpoint_unpin_successes:
    The unpin is actually still in effect.

checkpoint_find_and_pin_successes:
    The checkpoint was found and remains pinned.
```

### 0.7 Memory diagnostics: user goals and agreed implementation

The user explicitly raised memory usage as one of the largest unknowns before runtime cache use:

```text
- VapourSynth has its own caching and management.
- CNR3 has the old strict-streaming previous-output cache.
- CNR3 now has a new v005 output/checkpoint cache manager.
- The interaction between those memory systems is uncertain.
- Unexpected memory consumption should be detected early.
- If memory consumption becomes unacceptable, the current cache scheme may need tuning, replacement, or abandonment.
```

The user's memory-diagnostic goals were:

```text
- Capture total/process memory usage, not just CNR3-owned memory.
- Capture physical RAM used/free and global memory status where possible.
- Make the output broadly relatable to Windows 11 Task Manager.
- Print early/late lifecycle memory samples.
- Eventually print around cache store/prune/checkpoint events.
- Shield the diagnostics behind a separate compile-time switch.
- If easy, calculate min/max/average memory usage across samples and print a summary at close/free.
```

Agreed pragmatic answer:

```text
- Do not try to split memory ownership between VapourSynth and CNR3.
- Measure process-level and system-level memory instead.
- Correlate these memory numbers with CNR3 cache-manager counts, pin counts, checkpoint counts, and store/prune activity.
- This is enough to diagnose trends and decide whether the cache scheme is viable.
```

Windows APIs used by memory diagnostics:

```text
GetProcessMemoryInfo(GetCurrentProcess(), PROCESS_MEMORY_COUNTERS_EX, ...)
GlobalMemoryStatusEx(...)
GetPerformanceInfo(...)
```

Current memory snapshot fields include:
...
Process memory:
    ...
    process_pagefile_usage_bytes
    process_peak_pagefile_usage_bytes

Global/system memory:
    system_memory_load_percent
    system_total_phys_bytes
    system_avail_phys_bytes
    system_used_phys_bytes
    system_total_pagefile_bytes
    system_avail_pagefile_bytes
    system_used_pagefile_bytes
    system_total_virtual_bytes
    system_avail_virtual_bytes
    system_used_virtual_bytes

Performance memory:
    performance_commit_total_bytes
    performance_commit_limit_bytes
    performance_commit_peak_bytes
    performance_physical_total_bytes
    performance_physical_available_bytes
    performance_physical_used_bytes
    performance_system_cache_bytes
    performance_kernel_total_bytes
    performance_kernel_paged_bytes
    performance_kernel_nonpaged_bytes
```

Current memory summary accumulator tracks min/avg/max for:
...
After Phase 3C.7a, each metric has its own sample count:

```text
process_working_set_sample_count
process_private_usage_sample_count
system_avail_phys_sample_count
system_used_phys_sample_count
commit_total_sample_count
```

This avoids misleading averages if process/global/performance memory queries succeed for different numbers of snapshots.

Current memory sample points:
...
cnr3_free:
    before Cnr3Data delete, print min/avg/max memory summary
```

Memory diagnostic output is debug-only and goes to stderr.

### 0.8 Deferred memory diagnostics work still agreed/planned

The following memory-diagnostics work remains planned for later, when the v005 cache manager starts participating in runtime frame generation:

```text
Add memory sampling around v005 cache activity:
    after successful output-frame store
    after prune_after_store
    before/after checkpoint prune if useful
    after checkpoint pin/unpin if useful
    after cache hit return if useful
    after cache miss / nearest-checkpoint recovery / recompute chain if useful

Add less noisy runtime sampling policies if needed:
    every N frames
    only when cache count changes by a threshold
    only when process private usage reaches a new high-water mark
    only when working set reaches a new high-water mark
    only when available physical memory drops below a threshold

Use memory output with cache-manager stats:
    correlate process private usage / working set / commit total with cached-frame count, checkpoint count, pin count, store count, prune count, and post-validation counters.

If memory use is unacceptable:
    tune cache capacities and checkpoint retain counts;
    change pruning policy;
    reduce checkpoint count;
    reduce retained output-frame window;
    add hard memory guard rails;
    or abandon/replace the current v005 cache scheme.
```

A separate verbose/full cache-manager stats dump remains deferred. Current `vapoursynth-Cnr3.cpp` summary intentionally prints only headline counters plus post-validation failure counters. A future full dump can be added once the runtime path shows which counters are useful.

### 0.9A CRITICAL PRIORITY BEFORE PHASE 4 IMPLEMENTATION - no-prior-checkpoint bounded recovery and cache-safety modelling

This is a critical late clarification made after the initial v0.13 handover was produced. It must be reviewed before any Phase 4 runtime wiring is implemented. Do not treat this as a settled code design until it has been modelled and explicitly confirmed in the next chat.

Problem/use case:

```text
VapourSynth may request output frame N when CNR3 has no cached output[N - 1]
and no usable nearest-prior checkpoint C < N.

A naive fallback of requesting source[0..N] can be catastrophically expensive
for large N.

A naive refusal may be too conservative and may prevent useful recovery.

The user recalls and wants the no-prior-checkpoint fallback to use a bounded
source-frame warm-up/recovery window based on the standard/default cache size,
not an unrelated hard-coded magic number.
```

Current constants that must be considered:
...
Proposed no-prior-checkpoint bounded recovery policy to model/confirm:

```text
bounded_recovery_window = CNR3_OUTPUT_CACHE_CAPACITY

For requested output frame N, when no usable cached output[N - 1] and no
nearest prior checkpoint/output anchor exists:

    start = max(0, N - CNR3_OUTPUT_CACHE_CAPACITY)
    request source[start..N]

This requests at most CNR3_OUTPUT_CACHE_CAPACITY + 1 source frames.
With the current default capacity, that is at most 101 source frames.
```

Important correctness caveat:

```text
This bounded-source-window recovery is not mathematically identical to full
recursive recovery from frame 0.

If start > 0 and no filtered output[start - 1] exists, the implementation must
explicitly initialise the recovery chain at start, probably using source-copy /
initial-copy semantics for output[start], then recursively compute
output[start + 1] through output[N].

This is a deliberate bounded warm-up/reset recovery path. It is expected to be
close enough in a tight no-anchor situation because the chroma-stabilisation
algorithm should settle after a bounded number of frames, but it is not exact.
```

Absolute cache-safety requirement:

```text
The bounded recovery window must never break the v005 cache manager.

The implementation must prove/guarantee that inserting up to at least
CNR3_OUTPUT_CACHE_CAPACITY + 1 generated frames during a recovery operation is
safe under the intended VapourSynth scheduling mode.

This guarantee must hold even under the multithreaded environment we are moving
toward; simple single-request arithmetic is not enough. The next chat must model
or reason about worst-case in-flight requests, pinned checkpoints, temporary
retention, prune timing, and memory pressure.

Every stored output frame must go through cnr3_cache_manager_store_output_frame().
Pruning must go through cnr3_cache_manager_prune_after_store() or a deliberately
designed stronger recovery-aware pruning/temporary-extension path.
Any store/prune/validation failure must be treated as a hard cache-manager
failure until a better policy is explicitly designed.

The cache manager must not leak frame references, corrupt cache_index ownership,
leave stale aliases, prune pinned checkpoints, lose needed predecessor frames,
or allow unbounded retained-frame growth.
```

Checkpoint retain sizing must be revisited:

```text
The current checkpoint retain constants were chosen before the no-prior-checkpoint
bounded-recovery and multithreaded predecessor-calculation safety problem was
fully surfaced.

CNR3_CHECKPOINT_MAX_RETAIN = 16 and CNR3_CHECKPOINT_MIN_RETAIN = 6 may be too
small once Phase 4 begins performing bounded recovery, checkpoint pinning,
chain filling, and later fmParallelRequests-style concurrent request scheduling.

Before runtime recovery is implemented, checkpoint retain sizing must be modelled
and probably increased. It should not be guessed from the simple single-window
calculation alone.

The sizing should consider at least:
    - CNR3_OUTPUT_CACHE_CAPACITY;
    - CNR3_CHECKPOINT_INTERVAL;
    - maximum bounded recovery source window size;
    - maximum expected in-flight requests under fmUnordered/fmParallelRequests;
    - pinned checkpoints that cannot be pruned;
    - temporary recovery frames required by predecessor-frame calculation;
    - whether store/prune occurs after every generated frame or after batches;
    - memory diagnostics observed during runtime proving.
```

Temporary extension / recovery staging is unresolved and must be designed deliberately:

```text
No separate temporary-extension mechanism is currently known to be implemented.
The current ordinary non-checkpoint policy is Option B:

    allow non_checkpoint_pool up to
        CNR3_OUTPUT_CACHE_CAPACITY * CNR3_OUTPUT_CACHE_OVERFLOW_FACTOR
    then prune back to CNR3_OUTPUT_CACHE_CAPACITY.

With current constants, that ordinary overflow limit is about 110 frames.

A no-prior-checkpoint bounded recovery can request/generate up to 101 frames for
a single recovery window using the current CNR3_OUTPUT_CACHE_CAPACITY. That may
fit the ordinary non-checkpoint overflow margin for a simple single request, but
that is not a sufficient guarantee for multithreaded or overlapping recovery.

The next chat must decide whether to add a deliberately named temporary recovery
extension/budget, and if so what it means. Possible concepts to model include:

    CNR3_OUTPUT_CACHE_TEMP_EXTENSION_CAPACITY
    CNR3_RECOVERY_TEMP_EXTENSION_CAPACITY
    CNR3_MAX_IN_FLIGHT_RECOVERY_WINDOWS

The exact names and design are not decided. The critical requirement is that the
predecessor-frame calculator must have access to all frames it needs while those
frames are needed, and pruning must not remove them prematurely.
```

Critical implementation warning:

```text
Do not implement Phase 4 no-prior-checkpoint recovery until this bounded-window
policy, checkpoint retain sizing, temporary-extension/staging policy, prune
cadence, and cache-safety guarantees are explicitly reviewed and confirmed.

Mark this as a problem requiring further detailed consideration and modelling in
the next chat.
```

### 0.9 Current v005 cache manager state

Important: despite the extensive infrastructure, the current runtime path is not yet a v005 runtime cache path.

Current behaviour:

```text
- fmUnordered remains in use.
- The old strict-streaming cache remains active.
- cnr3_get_frame() still requires serial order / previous output availability according to the old strict policy.
- Out-of-order requests are still rejected by the current runtime logic.
- cache_manager_v005 is constructed, can be cleared, summarized, and audited, but is not yet used for output generation.
```

Current v005 cache-manager capabilities now implemented as infrastructure include:

```text
- per-instance mutex-protected cache state;
- non-checkpoint output-frame pool;
- checkpoint pool with pin_count;
- cache_index alias map;
- highest_cached_frame_number tracking;
- store output frame with addFrameRef ownership;
- remove output frame with freeFrame release;
- clear all retained frames;
- reset/get stats;
- passive debug snapshot;
- mutating invariant validation;
- nearest prior checkpoint lookup;
- atomic find-and-pin nearest prior checkpoint;
- direct pin/unpin checkpoint;
- non-checkpoint pruning;
- checkpoint pruning respecting frame zero and pinned checkpoints;
- prune_after_store combining both prune passes under one lock;
- extensive stats and post-validation counters.
```

### 0.10 Future steps as currently understood

These future steps are subject to change, but this is the current best roadmap.

#### Immediate next step after handover

```text
1. In the new chat, upload or reference this v0.13 handover and the current source files.
2. Confirm the current branch, latest commit, and that Release x64 builds.
3. Run a basic debug smoke test to ensure the new memory diagnostics print at create/free and do not write to stdout.
4. Confirm that CNR3 behaviour is unchanged before runtime v005 cache wiring.
```

#### Phase 4A - pre-runtime wiring review

```text
- Re-read cnr3_get_frame() and current strict-streaming cache behaviour.
- Identify the safest first insertion point for v005 cache-manager runtime use.
- Confirm which helper gaps remain before runtime use, especially cached-output lookup with caller-owned addFrameRef.
- Decide whether first runtime phase should be store/prune-only instrumentation or actual cache-hit reuse.
- CRITICAL: before any no-prior-checkpoint recovery implementation, model and confirm the bounded recovery policy based on CNR3_OUTPUT_CACHE_CAPACITY, checkpoint retain sizing, temporary recovery extension/staging, prune cadence, and multithreaded in-flight safety.
```

Recommended caution:

```text
Do not jump directly to full out-of-order recovery or fmParallelRequests. First prove that v005 can store, prune, summarize, clear, and release retained frames during a normal serial run.
```

#### Phase 4B - store/prune-only runtime proving under strict/serial behaviour

Proposed first live runtime use:

```text
- After a frame output is produced by the existing strict-streaming path, store that output frame into cache_manager_v005.
- Call prune_after_store().
- Do not yet retrieve frames from v005 for output generation.
- Keep the old strict-streaming algorithm as the actual output authority.
- Add memory diagnostic samples around store/prune if useful.
```

Purpose:

```text
- Prove addFrameRef/freeFrame balance under real frames.
- Prove v005 store/prune counters.
- Prove checkpoint promotion frequency.
- Prove clear/free releases retained frames at teardown.
- Establish memory growth baseline while v005 is retaining frames.
```

This phase may temporarily increase memory because both the old strict cache and the v005 cache retain frames. That is acceptable for proving but should be measured.

#### Phase 4C - add cache lookup helper with caller-owned reference

Before returning cached frames from v005 after releasing the mutex, add or confirm a helper like:
...
```text
- locks internally;
- finds frame_number in cache_index;
- verifies owning pool consistency;
- takes a caller-owned temporary reference with vsapi->addFrameRef();
- returns that retained frame pointer to the caller;
- caller must freeFrame() it exactly once on every path after use.
```

Do not return a raw cached VSFrame pointer from a public helper after unlocking.

#### Phase 4D - v005 cache-hit path under fmUnordered

```text
- Before computing a requested frame, check whether v005 already has it.
- If found, return/copy from caller-retained cached output safely.
- Ensure stats and debug clearly distinguish cache hit, miss, store, and prune.
- Add memory samples only at useful points or thresholds to avoid excessive noise.
```

#### Phase 4E - nearest-checkpoint recovery / chain filling under fmUnordered

```text
- For an out-of-order request, find and pin nearest prior checkpoint atomically.
- Recompute/fill missing frames in ascending frame-number order from checkpoint+1 to requested frame.
- Store each newly produced output frame into v005.
- Prune after store, respecting pinned checkpoint(s).
- Unpin checkpoint on every success/error/early-exit path.
- Validate with development diagnostics enabled.
```

Important:

```text
- Chain filling must remain strictly frame-number ordered.
- pin_count protects checkpoint slots from pruning only; it is not a VSFrame reference.
- All failure paths must unpin exactly once if a checkpoint was pinned.
```

No-prior-checkpoint case:

```text
If no usable nearest-prior checkpoint/output anchor exists, do not blindly
rebuild from frame 0 for large N and do not implement a guessed policy.

The currently preferred policy to model/confirm is bounded warm-up recovery
based on CNR3_OUTPUT_CACHE_CAPACITY:

    start = max(0, N - CNR3_OUTPUT_CACHE_CAPACITY)
    request source[start..N]

This is approximate if start > 0, because output[start] must be initialised from
source[start] without exact output[start - 1]. The implementation must document
this trade-off and guarantee cache-manager safety before coding it.
```

#### Phase 4F - fmParallelRequests target

The accepted v005 direction targets `fmParallelRequests` before full `fmParallel`.

Key expected shape:

```text
- arInitial stage may have concurrent readers/pinners/request scheduling.
- arAllFramesReady writer/output stage remains serial enough for safer first implementation.
- Cache-manager metadata operations remain mutex protected.
- Need to respect VapourSynth request lifecycle and avoid unsafe separate find-then-pin sequences.
```

This phase requires careful design before coding.

#### Phase 4G - memory diagnostics expansion during runtime cache testing

Add memory diagnostics progressively around real cache activity:

```text
- store/prune points first;
- checkpoint recovery later;
- threshold/high-water sampling if per-frame output is too noisy;
- summary correlation with cache-manager stats.
```

Use memory diagnostics to answer:

```text
- Does process private usage grow with cached-frame count as expected?
- Does working set return/downshift after pruning/clear?
- Does system available physical memory drop dangerously?
- Do checkpoint retain counts or output cache capacity need tuning?
- Is the v005 scheme viable, or does it need redesign?
```

#### Phase 4H - full fmParallel only after fmParallelRequests is proven

Full `fmParallel` remains the hardest mode.

Expected additional needs:

```text
- active-computation state;
- condition variables or equivalent wait/notify logic;
- duplicate work suppression;
- handling multiple threads requesting overlapping chains;
- deadlock/livelock prevention;
- clear policy for speculative refusal vs waiting;
- careful memory guard rails.
```

Do not jump to full `fmParallel` until fmUnordered and fmParallelRequests behaviour is proven.

#### Later algorithm/quality tasks still relevant

These remain on the longer-term list and should be rechecked against current code before acting:

```text
- Compare CNR3 output against vscnr2/Cnr2 behaviour on representative real clips.
- Continue validating luma guard and scene-change behaviour.
- Confirm TV-range/full-range assumptions and whether first-frame/range property detection is needed.
- Validate 4:2:0, 4:2:2, and 4:4:4 integer formats at 8-16 bits.
- Continue to keep public threshold parameters in 8-bit domain.
- Keep mode semantics documented as response-curve width, not on/off.
- Revisit defaults only with evidence.
- Test Win10 x64 and Win11 x64 plugin loading/autoload.
- Keep GitHub Actions / local VS project aligned when new .cpp files are added.
```

### 0.11 Current risk register

```text
Runtime memory risk:
    Still unknown. Instrumentation now exists, but v005 is not live in runtime yet.

VapourSynth cache interaction risk:
    Still unknown. Process/system memory diagnostics will observe combined behaviour, not isolate ownership.

Reference-count risk:
    Store/remove/clear helpers are designed around addFrameRef/freeFrame ownership, but real runtime use must prove balance.

Pinned-checkpoint leak risk:
    Reduced by 3C.2 rollback fixes. Future runtime code must still unpin on every path.

Concurrency risk:
    Cache-manager metadata is mutex protected, but algorithmic correctness under fmParallelRequests/fmParallel is not yet proven.

Diagnostic overhead risk:
    Controlled by compile-time switches and debug-only output. Runtime memory sampling should still be used judiciously.

Handover/compression risk:
    This document is intentionally detailed. Future chats should not compress away cache/refcount/memory semantics.
```

### 0.12 Suggested first prompt for the next chat

A good starting prompt for a new chat is:

```text
I am continuing vapoursynth-cnr3 development from CNR3_Handover_Snapshot_v0.13_cache_manager_phase_3C_memory_diagnostics.md. Please read it carefully first. The latest source has Phase 3C complete: v005 cache-manager infrastructure, passive cache-manager debug snapshot, post-validation counters, memory diagnostics, and build-config switches. The v005 cache manager is owned by Cnr3Data but is not yet used by cnr3_get_frame(). I want to proceed carefully toward Phase 4 runtime wiring, starting with the safest store/prune-only proving step unless you identify a better first step. Please do not skip the memory-diagnostics goals or the VSFrame addFrameRef/freeFrame ownership rules.
```
...
## 30. v0.11 addendum - cache manager Phase 2G.2 checkpoint

This v0.11 update records cache-manager infrastructure progress on branch `dev_cache_manager` after the v0.10 Phase 2B.1 checkpoint.

The accepted cache-design basis remains:

```text
cnr3_cache_concept_v005.md
```

No new cache concept/spec version has been declared. The work since v0.10 is implementation of the v005 direction plus implementation-policy clarifications developed during coding.

Completed implementation checkpoints since v0.10:

```text
Phase 2C.1:
    Added Cnr3CacheManagerStats.
    Added stats member to Cnr3CacheManagerV005.
    Added thread-safe stats reset/snapshot helpers.

Phase 2C.2:
    Added checkpoint pin/unpin helpers.
    Added pinned-checkpoint inspection helpers.
    Added pin/unpin counters.
    pin_count remains a pruning-protection counter only; it is not a VSFrame reference.

Phase 2C.3:
    Added initial integrity counters.
    Changed nearest-prior-checkpoint lookup to a clearer reverse frame-number scan.
    The lookup now scans checkpoint_pool from highest frame number downward and selects the first checkpoint strictly lower than the requested frame.
    This was chosen for maintainability and clarity over the earlier lower_bound()/step-back idiom.

Phase 2D.1:
    Added cache store helper:
        cnr3_cache_manager_store_output_frame()
    Store takes one cache-owned VSFrame reference with vsapi->addFrameRef() on successful insertion.
    The retained frame is inserted into exactly one owning pool:
        non_checkpoint_pool or checkpoint_pool.
    cache_index receives a non-owning alias only.
    Store failure paths release a retained frame immediately if insertion later fails.

Phase 2D.2:
    Added cache remove helper:
        cnr3_cache_manager_remove_output_frame()
    Remove releases one cache-owned VSFrame reference with vsapi->freeFrame().
    Remove verifies that the frame exists in cache_index and exactly one owning pool.
    Remove refuses to remove pinned checkpoints.
    Remove updates remove-related diagnostic counters.

Phase 2E.1a:
    Refactored remove into:
        public internally locking wrapper;
        static _externally_locked implementation.
    This allows compound helpers to remove multiple frames under one cache_mutex lock without deadlock.

Phase 2E.1b:
    Added non-checkpoint pruning helper.
    Non-checkpoint pruning uses Option B:
        allow non_checkpoint_pool to grow to overflow limit;
        if above overflow limit, prune oldest non-checkpoint frames first until back to CNR3_OUTPUT_CACHE_CAPACITY.
    Actual frame release is delegated to the shared _externally_locked remove helper.

Phase 2E.2:
    Added checkpoint pruning helper.
    Checkpoint pruning prunes oldest eligible checkpoints first when checkpoint_pool exceeds CNR3_CHECKPOINT_MAX_RETAIN.
    It stops when checkpoint_pool is back to CNR3_CHECKPOINT_MIN_RETAIN, or when no eligible checkpoint can be removed.
    Frame 0 is never pruned.
    Checkpoints with pin_count > 0 are never pruned.
    Actual frame release is delegated to the shared _externally_locked remove helper.

Phase 2E.3a:
    Refactored non-checkpoint and checkpoint prune helpers into:
        public internally locking wrappers;
        static _externally_locked implementations.
    Static forward declarations were added near the top of cnr3_cache_manager.cpp.

Phase 2E.3b:
    Added combined prune-after-store helper:
        cnr3_cache_manager_prune_after_store()
    This public helper locks once and calls the non-checkpoint and checkpoint _externally_locked prune implementations.
    This gives later runtime code one clean post-store pruning entry point.

Phase 2F:
    Performed desk review of addFrameRef/freeFrame lifecycle, ownership invariants, pruning behaviour, thread-safety boundaries, and diagnostics.

Phase 2F.1:
    Fixed cnr3_cache_manager_clear() ownership safety.
    clear() now fails safely if asked to clear a non-empty cache with vsapi == nullptr.
    This prevents silently dropping cache-owned VSFrame references without calling freeFrame().
    clear-related diagnostic counters were added.
    Stale prune-helper wording was cleaned up.

Phase 2G.1:
    Added cache invariant validation helper.
    Validation checks cache_index/pool consistency, one-pool-only ownership, null frame pointers, negative pin_count values, and highest_cached_frame_number consistency.
    Validation/integrity counters were added.

Phase 2G.2:
    Refactored validation into:
        public internally locking wrapper;
        static _externally_locked implementation.
    This allows future compound helpers to validate invariants while holding one cache-manager critical section.
```

Current source-layout status:

```text
src/cnr3_cache_manager.h:
    v005 cache-manager data structures, constants, statistics fields, and public helper declarations.

src/cnr3_cache_manager.cpp:
    v005 cache-manager helper implementations.
    Includes static _externally_locked helper implementations for remove, prune, and validation compound-operation support.

src/cnr3_cache.cpp:
    Existing earlier cache/strict-streaming support remains separate.

src/cnr3_response_tables.cpp/.h:
    Response-table split remains separate.

src/vapoursynth-Cnr3.cpp:
    Runtime processing remains the current strict-streaming implementation.
```
...
Important cache-manager invariants now embodied in code and comments:

```text
Conceptual cache:
    The cache manager conceptually stores calculated output frames.

Implementation:
    Cached frames are owned by exactly one of:
        non_checkpoint_pool
        checkpoint_pool

cache_index:
    Provides fast exact lookup across both pools by frame number.
    cache_index is unordered and must never be used for ordered decisions.
    cache_index does not own VSFrame references.
    cache_index entries must alias the pointer owned by exactly one owning pool.

Frame-number ordering:
    All ordered decisions must use frame number only.
    non_checkpoint_pool and checkpoint_pool are std::map containers ordered by frame number.
    checkpoint selection uses reverse frame-number scan of checkpoint_pool.
    pruning removes oldest eligible frames first by frame number.

Frame reference ownership:
    Store takes one cache-owned reference using addFrameRef().
    Remove/prune/clear release cache-owned references using freeFrame().
    cache_index never calls freeFrame().
    pin_count never calls addFrameRef() or freeFrame().

Thread safety:
    Public cnr3_cache_manager_* helpers lock internally unless their name ends in _externally_locked.
    Static _externally_locked helpers assume the caller already holds cache.cache_mutex.
    Public helpers must not be called while the caller already holds cache.cache_mutex unless the helper is explicitly _externally_locked.
```

Diagnostic and review policy recorded during this work:

```text
- Error/integrity counters should be incremented where impossible or corrupt cache states are detected.
- A later review pass should add clearly marked stderr/debug error messages where error counters are incremented or where possible errors are detected.
- Future debug output should make it easy to collect and analyse cache behaviour from stderr.
- The addFrameRef/freeFrame lifecycle must be re-reviewed before runtime wiring.
- pin_count decrement/cleanup paths must be reviewed carefully when frameData/arInitial/arAllFramesReady wiring is implemented.
```

Known review notes / future cleanup candidates:

```text
- Some diagnostic counters may be noisy rather than exact event counts, especially checkpoint-prune skipped-frame-zero and skipped-pinned counters because checkpoint pruning restarts from begin() after a removal.
- If checkpoint removal fails, checkpoint_prune_no_eligible_frames may also be incremented in some paths where the root cause was removal failure rather than genuine lack of eligible frames. This is acceptable for now but should be reviewed before relying on the metric.
- Later debug logging should include clear tags for integrity errors, remove failures, prune failures, pin/unpin failures, and addFrameRef/freeFrame ownership anomalies.
```

Recommended next phase:

```text
Phase 2G.3 or Phase 2H:
    Decide whether to call cnr3_cache_manager_validate_invariants() from selected public helpers in development/debug-safe places, or defer automatic validation until runtime wiring/debug policy is clearer.

Before runtime wiring:
    Perform another desk review focused specifically on:
        addFrameRef/freeFrame paths;
        cache_index/owning-pool synchronization;
        pin_count cleanup and underflow paths;
        clear/prune/remove failure behaviour;
        whether validation should be enabled conditionally for development builds.
```


---

## 31. Cache-manager checkpoint: Phase 2G.3 and Phase 2H.1 complete

This section updates the cache-manager implementation checkpoint after completing development-only validation and atomic checkpoint find-and-pin support on branch `dev_cache_manager`.

Completed since v0.11:

```text
Phase 2G.3:
    Added development-only cache-manager diagnostics compile-time switches.
    Added optional post-mutation invariant validation behind if constexpr guards.
    Validation is enabled by default during development and maintenance.

Phase 2H.1:
    Added atomic find-and-pin nearest prior checkpoint helper.
    This prevents future runtime code from using an unsafe find/unlock/pin sequence.
```

Development diagnostics constants added:
...
The intent of these constants is:

```text
CNR3_CACHE_MANAGER_DEV_DIAGNOSTICS:
    Master compile-time switch for development/maintenance diagnostics.

CNR3_CACHE_MANAGER_VALIDATE_AFTER_MUTATION:
    Specific compile-time switch for post-mutation invariant validation.
    It defaults to CNR3_CACHE_MANAGER_DEV_DIAGNOSTICS, but may be set
    independently if validation needs to be enabled or disabled separately
    from other development diagnostics.
```

Important compile-time behaviour:

```text
Validation blocks are guarded with if constexpr.

When the validation constant is false, those diagnostic branches are not
compiled into generated executable code, although the source must still remain
syntactically valid C++.
```

Validation-after-mutation policy as of v0.12:

```text
Validation is deliberately development-focused and must not be required for
correctness.

Correctness must still come from:
    cache ownership rules;
    cache_mutex locking rules;
    one-owning-pool-only invariant;
    cache_index synchronisation;
    pin_count rules;
    addFrameRef/freeFrame lifecycle correctness.

Validation is used to detect breakage early during development and maintenance.
```

Validation is now called in selected mutation paths, including:

```text
cnr3_cache_manager_store_output_frame():
    validates after successful insertion.

cnr3_cache_manager_remove_output_frame():
    validates after successful public removal.

cnr3_cache_manager_prune_non_checkpoint_pool_externally_locked():
    validates after each successful individual non-checkpoint prune removal
    during development.

cnr3_cache_manager_prune_checkpoint_pool_externally_locked():
    validates after each successful individual checkpoint prune removal
    during development.

cnr3_cache_manager_prune_after_store():
    validates after the combined non-checkpoint/checkpoint prune pass succeeds.
```

The deliberate policy decision is:

```text
Avoid validation so aggressive that it obscures behaviour, but do not omit it
where it is useful during development.

Inner prune-loop validation is currently acceptable because it is behind the
development-only compile-time switch and helps identify exactly which individual
remove/prune operation corrupts the cache, if any.
```

Atomic checkpoint find-and-pin helper added in Phase 2H.1:
...
```text
Atomically find the nearest prior checkpoint and increment its pin_count while
holding cache.cache_mutex.

This prevents the unsafe future runtime sequence:
    find nearest prior checkpoint;
    unlock;
    later pin checkpoint.

That sequence is unsafe because another cache operation could prune the selected
checkpoint between the find and the pin.
```
...
- scans checkpoint_pool in reverse frame-number order;
- chooses the highest checkpoint frame number strictly less than the requested frame;
- checks that the checkpoint slot has a non-null frame pointer;
- increments pin_count before releasing cache.cache_mutex;
- returns only the checkpoint frame number;
- does not return a raw VSFrame pointer;
- does not take an extra addFrameRef();
- updates find-and-pin-specific counters and general pin counters.
```

Matching cleanup path:

```text
cnr3_cache_manager_unpin_checkpoint() remains the matching cleanup helper.

Future arAllFramesReady/error/cleanup paths should unpin the checkpoint selected
by find-and-pin on every success, error, and early-exit path.
```

Important future runtime rule:

```text
Future runtime code MUST NOT use a separate public find-then-pin sequence.

Preferred runtime pattern:
    arInitial-style path:
        cnr3_cache_manager_find_and_pin_nearest_prior_checkpoint(...)

    arAllFramesReady/error/cleanup path:
        cnr3_cache_manager_unpin_checkpoint(...)
```

Runtime status remains unchanged:

```text
- Cnr3CacheManagerV005 has not yet been added to Cnr3Data.
- cnr3_get_frame() has not been changed.
- VapourSynth filter mode has not been changed.
- The current strict-streaming cache remains active.
- The v005 cache manager remains infrastructure-only and is not yet used by the runtime path.
```
...
```text
Phase 3A:
    Add Cnr3CacheManagerV005 to Cnr3Data without changing runtime behaviour.

Phase 3A should include focused review of:
    where Cnr3Data is constructed and destroyed;
    whether the free/teardown callback has valid VSAPI access;
    where cnr3_cache_manager_clear() should be called;
    how clear() failure should be reported;
    whether old strict-streaming cache fields must remain untouched;
    ensuring cnr3_get_frame() behaviour remains unchanged during Phase 3A.
```

Before any actual runtime cache scheduling:

```text
Perform another desk review focused on:
    addFrameRef/freeFrame ownership paths;
    cache_index and owning-pool synchronisation;
    checkpoint pin/unpin cleanup paths;
    frameData lifetime and every arAllFramesReady/arError/early-exit path;
    development-only diagnostics and whether they should remain enabled.
```
