# CNR3 — PLAN/RESULT DIAGNOSTIC (D-SUM PLAN-TRACE) — VOCABULARY + ARCHITECTURE SPEC v2.3
...
and SPEC v2/v2.1 (superseded by this v2.2). The cross-check has happened
...
**Revision v2.3 (supersedes v2.2):** EMISSION/WRITER-LAYER refinement, specified after inspecting real 3c.1
output and given the A1 external-analysis commitment. Capture, timing, the tick/seq invariant, the windowed
buffer, observe-only fencing, and all O/R field CONTENT are UNCHANGED — this revises only how a buffered
record is RENDERED at dump time, and it SHRINKS the design. Accountability sits with the SPEC: the rationale
doc's single-line-sortability intent was never hardened into a "one physical line per record" requirement
through v1->v2->v2.2 (which chose multi-line labelled-lists for readability). Changes: (R1) each record is
ONE physical ASCII line, fixed schema, every field present even when empty (`anchor=[]`); (R2) zero-pad ONLY
the externally-sorted keys (enter_tick, exit_tick, seq, frame) — role-list/codes numbers are parsed, not
sorted, so unpadded; (R3) REMOVE the three sorted views and their sub-gates — the dump is ONE block in
natural capture order (action_seq ascending; no sort pass), because single-line sortable records + A1 make
in-plugin views redundant and strictly weaker than external `sort`/A1; (R4) the legend is printed ONCE at the
head of the single block. (R5) each instance's trace is bracketed by BEGIN/END markers (schema/window/record-
count/truncated), one block per instance, for self-description + truncation detection. Legend is column-
aligned ASCII and uses `O-item`/`R-item` (not "field", which is loaded in a video context). Vocabulary: Set 4 `L` display label -> `post_compute_discarded`; Set 4 `U`/`unpinned`
DROPPED from the R record (near-universal proven-safe pin-discharge — no thrash signal; pin accounting lives
in D-SUM-04/12, not the plan-trace; supersedes the B.3.1 "unpinned as per-item fact" half — coordinator-
approved under R-PROCESS-25). Sort keys L->R: instance (via the retained `CNR3[n]` host prefix), enter_tick,
seq, frame, exit_tick. ALL emission is ASCII-only. This supersedes the v2.2 §5 multi-line emission and the
§8 three-view design; §4 Set 4, §5, §6, §8, §9 are rewritten accordingly.

**Revision v2.2 (supersedes v2.1):** folds coordinator directive on plan-trace view (a) — clarification/
tightening of already-settled design (tick-not-datetime was closed in §6), NOT a design change, folded
without re-review per the directive. Three items, all in §8 (+ §3): (1) view (a) label tightened
"sort by enter_datetime" -> "sort by enter-tick (display column: UTC)" to prevent a string-sort misread;
(2) view (a) sort key stated as the ordered tuple (enter_tick ASC, action_seq ASC), with `phase` dropped as
a sort term (still displayed) — action_seq supersedes phase because it is the globally-unique deterministic
tie-break; (3) the (enter_tick outside-lock / action_seq inside-lock) capture INVARIANT stated with the
failure mode bound to each half (moving action_seq's increment outside the lock reintroduces the fmUnordered
collision). No finding or locked decision from v2/v2.1 reversed.
...
- action_seq         : monotonic per-instance counter stamped at record write (INSIDE the buffer lock —
                       globally unique per instance; the deterministic tie-break for view (a), §8) —
                       ZERO-PADDED — sortable
...
- plan/result body   : single-line fixed-schema fields + composed machine codes (§5)  [v2.3]
...
**`pinned` is a per-item FACT ONLY — no pin counts. [LOCKED — rationale B.3.1; `unpinned` dropped v2.3]**
- WHY / rejected TWICE: (i) a GLOBAL pin-count compared across a frame's O and R phases is *semantically
  ...
  COUNTS were dropped on the coordinator ruling: "this exercise is about SEEING PLANS, not pin counts." The
  O-item `pinned` fact remains (documents the plan's compute base). The R-item `unpinned`/`U` fact is REMOVED
  in v2.3 (near-universal proven-safe discharge, no signal — see §4 Set 4); this supersedes the "unpinned as
  per-item fact" half of B.3.1, coordinator-approved under R-PROCESS-25.
...
```text
C  computed                computed and stored into cache (won, or no race)     (Cnr3LiveRecoveryHoleOutcome::computed)
K  adopted_skipped         already present when checked; pinned and compute      (adopted_skipped)
                           skipped (bailed before computing)
L  post_compute_discarded  computed, then discarded as the duplicate that lost   (adopted_post_compute_loser)
                           the first-in-best-dressed race (winner adopted)
N  none                    nothing to do / default                               (none)
X  not_reached             planned item never reached (bailed before)  [DERIVED — needs bail-path write, 3c.2]
E  error_here              processing failed ON this item (bail cause) [DERIVED — needs bail-path write, 3c.2]

`U` / `unpinned` is REMOVED from the R record (v2.3). It was the per-plan pin-discharge, applied to nearly
every frame the plan pinned (base + holes), so it co-occurred with C/K/L (`CU`/`KU`/`LU`) and — being
near-universal and proven-safe — carried no distinguishing signal. Pin accounting lives in D-SUM-04 (cache
ref balance) and D-SUM-12 (plan lifecycle); the plan-trace is not a pin ledger. The O-item `pinned` role is
RETAINED (it documents the plan's compute base = plan structure). `L`'s underlying source enum
`adopted_post_compute_loser` is unchanged; only the display label/gloss becomes `post_compute_discarded`.
```
...
## 5. Emission format (single canonical ASCII line per record; fixed schema)

**DECISION [v2.3, R1] — each record is ONE physical line carrying every field; no indented sub-lines.** The
`CNR3[n] INFO DSUM-PLANTRACE:` host prefix stays (one per line, carries the instance for humans and A1); the
`[DSUM-PLANTRACE]` block tag follows it, then the phase token `O` or `R`, then the fields.
- WHY / supersedes: v2.2's multi-line labelled-list emission chose eye-alignment; real-output inspection plus
  the A1 external-analysis commitment made one-physical-line-per-record the requirement — it is what makes the
  A1 parser and plain `sort`/`awk` trivial. The rationale doc's single-line-sortability intent was the true
  parent; v2.3 hardens it.

**FIXED SCHEMA — every field present on every record of that phase, even when empty (`anchor=[] holes=[]`).**
Do NOT omit empty fields — fixed columns are what let column-oriented tools rely on position. Field ORDER is
fixed and identical across all records of a phase. ASCII only.

**Padding [v2.3, R2]:** zero-pad ONLY the externally-sorted keys — `enter_tick`, `exit_tick`, `seq`, `frame`
(existing widths; enter_tick/exit_tick fixed-20). Do NOT pad numbers inside `codes=[...]` or the role lists
(`target/predecessor/anchor/floor/holes/sources/pinned/computed/adopted_skipped/post_compute_discarded`) —
they are parsed, not sorted, so padding is noise: `codes=[6=PN,7=TS]`, `holes=[33,34,35]`. The `*`
checkpoint-grid glyph rides on the frame number inside role-lists/codes (e.g. `40*`), unpadded, and NEVER on a
padded sort key (a `*` in a sort column breaks lexical order).

**Field order — O phase:**
```text
CNR3[n] INFO DSUM-PLANTRACE: [DSUM-PLANTRACE] O enter_tick=<pad20> seq=<pad> frame=<pad> exit_tick=<pad20> strategy=<Set1> enter_utc=<iso> exit_utc=<iso> run_ms=<f> target=[...] predecessor=[...] anchor=[...] floor=[...] holes=[...] sources=[...] pinned=[...] codes=[frame=ROLES,...]
```
**Field order — R phase:**
```text
CNR3[n] INFO DSUM-PLANTRACE: [DSUM-PLANTRACE] R enter_tick=<pad20> seq=<pad> frame=<pad> exit_tick=<pad20> outcome=<Set3> enter_utc=<iso> exit_utc=<iso> run_ms=<f> computed=[...] adopted_skipped=[...] post_compute_discarded=[...] codes=[frame=ROLES,...]
```
Worked examples (ASCII; RECOVERY_EXACT and CACHE_HIT; note branch-specific `sources`, branch-derived
`pinned`, unpadded role/codes numbers, and NO `unpinned=`):
```text
CNR3[1] INFO DSUM-PLANTRACE: [DSUM-PLANTRACE] O enter_tick=00000197766114570100 seq=00000042 frame=00000123 exit_tick=00000197766114571900 strategy=RECOVERY_EXACT enter_utc=2026-07-06T07:46:55.492Z exit_utc=2026-07-06T07:46:55.494Z run_ms=1.830 target=[123] predecessor=[] anchor=[119*] floor=[] holes=[120,121,122] sources=[120,121,122,123] pinned=[119*] codes=[119=AP,120=HS,121=HS,122=HS,123=TS]
CNR3[1] INFO DSUM-PLANTRACE: [DSUM-PLANTRACE] R enter_tick=00000197766121030000 seq=00000191 frame=00000123 exit_tick=00000197766121104400 strategy=... outcome=RETURNED_RECOVERED enter_utc=2026-07-06T07:46:55.499Z exit_utc=2026-07-06T07:46:55.506Z run_ms=7.440 computed=[121,122,123] adopted_skipped=[120] post_compute_discarded=[] codes=[120=K,121=C,122=C,123=C]
CNR3[1] INFO DSUM-PLANTRACE: [DSUM-PLANTRACE] O enter_tick=00000197766100000000 seq=00000007 frame=00000045 exit_tick=00000197766100000200 strategy=CACHE_HIT enter_utc=... exit_utc=... run_ms=0.020 target=[45] predecessor=[] anchor=[] floor=[] holes=[] sources=[45] pinned=[45] codes=[45=TSN]
CNR3[1] INFO DSUM-PLANTRACE: [DSUM-PLANTRACE] R enter_tick=00000197766100050000 seq=00000009 frame=00000045 exit_tick=00000197766100050300 outcome=RETURNED_CACHE_HIT enter_utc=... exit_utc=... run_ms=0.050 computed=[] adopted_skipped=[] post_compute_discarded=[] codes=[]
```
(3c.2 will add to the R line, on FAILED only: `fail_reason=<Set5>` and `X`/`E` entries in `codes`.)
...
- PURE-OBSERVE (3c.1): O/R record capture reads plan/result state that already exists; it does not change
  what arInitial/arAllFramesReady decide, compute, pin, or return. Sets 1-3 + the C/K/L/N subset of Set 4 (U dropped v2.3).
...
**INVARIANT — the (enter_tick, action_seq) capture discipline is load-bearing; state the failure mode, not
just the rule, so it survives a well-meaning refactor. [v2.2]** A record's key pair is formed as:
```text
- enter_tick  : sampled OUTSIDE the diagnostics mutex (a cheap monotonic clock read), carried INTO the lock
                with the record.
                WHY NOT MOVE IT INSIDE: moving the clock read inside the lock needlessly serializes it behind
                the buffer mutex — contention for zero benefit.
- action_seq  : bumped INSIDE the diagnostics mutex, in the SAME critical section as the buffer append.
                ...
                collide, destroying action_seq's ONE JOB as the deterministic tie-break for view (a) (§8
                views). This is the load-bearing reason it is lock-protected; it is not incidental.
```
This is the WHY behind the capture sequence above: a coder who sees only "don't move this" may judge a move
safe; a coder who sees "moving it reintroduces the fmUnordered race" will not.
...
**DECISION [v2.3, R3] — ONE block in natural capture order; the three sorted views are REMOVED.**
```text
- The dump is a SINGLE block: the records walked from the buffer AS-BUILT (action_seq ascending — the buffer
  is already physically in action_seq order because the counter is bumped in the same locked critical section
  as the append). NO sort pass at all.
- The three views (VIEW_DATETIME / VIEW_FRAME / VIEW_PHASE) and their three sub-#ifdef gates are DELETED.
```
- WHY / supersedes rationale B.3.7: the three in-plugin views existed to hand a human pre-sorted perspectives
  without tooling. Two later commitments dissolve that: (i) single-line sortable records (§5, R1/R2) make any
  view one `sort -k` away; (ii) the A1 Python analysis track will parse records and produce arbitrary views
  far beyond three fixed sorts. Three hard-coded in-plugin sorts are now redundant with — and strictly weaker
  than — external `sort`/A1. Emitting the raw single block in capture order is the least code, the cleanest
  observe-only proof (one pass, one legend, zero sort passes), and loses NO information: every record carries
  instance/enter_tick/seq/frame/exit_tick, so every former view is an external re-sort.
- Sort keys (for external `sort`/A1), left to right: `instance` (via the retained `CNR3[n]` prefix, at line
  start), `enter_tick`, `seq`, `frame`, `exit_tick`. NOTE: the prefix instance number is not zero-padded, so a
  naive lexical `sort` orders `CNR3[10]` before `CNR3[2]`; A1 parses it numerically, and instance counts are
  small (1-2) in practice.
- The block BEGIN/END terminators and the clean-end DUMP END marker are RETAINED. The legend is printed ONCE
  at the head of the block (R4, below).

**DECISION [v2.3, R4] — the legend is MANDATORY, printed ONCE at the head of the block, ABOVE the records,
and COLUMN-ALIGNED for humans (ASCII spaces only, no tabs).** It is the parse key for `codes` and the item
glosses. Naming: `O-item` / `R-item` (NOT "field" — "field" is loaded in a video/interlacing context and is
avoided; `O-item`/`R-item` matches the Set 2 / Set 4 "ITEM-LEVEL" naming). Name column padded to the widest
name (`post_compute_discarded`, 22) so all glosses align; code columns aligned the same way. This alignment
is LEGEND-ONLY — it does NOT apply to record lines (see below).
```text
...
```
(`codes` compose honestly — a frame carries every role/outcome letter it holds. `U`/`unpinned` is NOT emitted
(removed v2.3); `X`/`not_reached` and `E`/`error_here` are 3c.2.)
**Record lines are NOT column-aligned** — they stay `key=value` single-line with no inter-field alignment.
Aligning columns across variable-length role-lists (`holes=[120,121,122]` vs `holes=[]`) would break the
fixed-schema `sort`/`awk` parsing and bloat every line. Legend = human-aligned table; records = machine-clean.

**DECISION [v2.3, R5] — each instance's trace is bracketed by a BEGIN line and an END line.** One
`BEGIN .. END` block PER INSTANCE (under fmParallel each buffer dumps its own block at teardown; do NOT merge
across instances — that would need a cross-buffer lock and defeats the per-instance design). Order within a
block: `BEGIN` -> legend (once, R4) -> records (natural capture order) -> `END`.
```text
[DSUM-PLANTRACE] BEGIN schema=3c1v1 instance=<n> window=[<from>,<to>] records=<count>
   ... legend (R4) ...
   ... records (single-line, action_seq order) ...
[DSUM-PLANTRACE] END   schema=3c1v1 instance=<n> records=<count> truncated=<0|1>
```
- `schema=3c1v1` — format-version token; lets A1 branch cleanly when 3c.2 adds `fail_reason`/X/E to the R line.
- `window=[from,to]` on BEGIN — the block is self-describing (what was/wasn't captured) without the build config.
- `records=` on BEGIN (expected) vs `records=` on END (actual) — a mismatch means the dump was cut off.
- `truncated=` on END — set to 1 if the buffer `reserve_failed` tripwire fired (§8 buffer); explicit honesty
  so an incomplete block self-flags rather than silently shortens.
- WHY both markers: crash-survival. Per-line flush (R-PROCESS-24) gets bytes out, but if VS tears down
  mid-dump, a matched BEGIN/END with record counts is how a human OR A1 detects a missing tail; BEGIN alone
  cannot.
- BEGIN/END/legend are STRUCTURAL markers, not records: a consumer dispatches on the `O`/`R` phase token, so
  markers are separated before any record re-sort and never participate in it. (Intended re-sort = parse/grep
  the `O`/`R` lines then `sort`; a naive `sort wholefile` only cosmetically interleaves the marker types —
...
paired with the print gate. Nested sub-flags (numbers INSIDE their feature #if):
  - the window from/to compile-time bounds (§2).
  [v2.3] The three sort-view toggles (VIEW_DATETIME / VIEW_FRAME / VIEW_PHASE) are REMOVED — there is one
  natural-order block, no views (§8, R3). No view sub-gates remain.
...
```text
1. Build default config, master gate ON: four-way 56/56 / 56/56 / 55/56 exit 1 / 56/56; the plan-trace emits
   ONE natural-order block (single-line records, §5) with the legend printed ONCE at its head; window
   respected; Sets 1-3 + C/K/L/N (no U; X/E are 3c.2).
...
3c.1  BUFFERED PLAN/RESULT CAPTURE + CLEAN-END DUMP   — OBSERVE-ONLY, R-PROCESS-19
  In scope:
    ...
    - Set 1 (strategy), Set 2 (roles, incl. branch-specific sources + branch-derived pinned), Set 3
      (outcome), Set 4 subset C/K/L/N (§4; U dropped v2.3)
    - the single natural-order block + single-line emission + one legend (§5, §8)
...
Carried-forward LOCKED decisions (rationale Part B; §5-§8 here): blocks-not-streaming; pinned (O-item) as
per-item facts only (pin counts rejected twice); dump-on-clean-end-OR-bail with once-guard; flush-always
(R-PROCESS-24); windowed buffer, no ring/no saturation; steady-clock ticks + UTC-at-dump. [v2.3 SUPERSEDED:
the three legend-headed views -> ONE natural-order block; multi-line human-lists -> single canonical line;
`unpinned`/`U` dropped. See the v2.3 revision note.] Any further change is a PROPOSAL (R-PROCESS-25).
