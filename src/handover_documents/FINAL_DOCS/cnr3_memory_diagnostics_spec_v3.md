# CNR3 Memory Diagnostics -- Formatted Output Specification v3

**Supersedes v2.** Read in conjunction with the CMS (v7.15+) and the DIAG.4 scope/decisions. Changes from v2:
...
DYNAMIC (min/avg/max accumulated):
  ...
  commit_total               perf_physical_avail        perf_physical_used
...
STATIC TOTALS (stored once from baseline, no accumulation -- they do not move within a run):
  system_total_phys          system_total_pagefile      system_total_virtual
  commit_limit               perf_physical_total        perf_kernel_total
```
