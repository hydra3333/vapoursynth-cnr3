...
**Supersedes:** CMS06.9 supersedes CMS06.8, CMS06.7, CMS06.6, CMS06.5, CMS06.4, CMS06.3, CMS06.2, CMS06.1, CMS06, CMS05.2b, CMS05.2, CMS05.1, CMS05, CMS04, CMS03, CMS02, CMS01
...
reduction (CMS06.8):** H15.5 proves that eligible sequential frames can
return through the sequential fast path (Section 4.10). It does not by
itself prove that `arInitial` can stop requesting the bounded-warmup
source window. Request-plan reduction must be handled separately because
of VS-LIFECYCLE-01. The safe sequence is:
- H15.6: probe/log the intended `arInitial` request-plan reduction without
  changing the requested frame set;
- H15.7: actively request only source frame N for eligible sequential
  fast-path candidates, while retaining fallback request behaviour for
  frame 0, holes, non-sequential requests, and failed eligibility.
...
reduction (H15.6B) must not precede the fallback proof, because
...
reference. H15.6 is the next recommended phase.
...
#### CMS02-H / SubPhase H15.6B / active source-plan reduction (fast-path-eligible only)
**Status: Pending H15.6A evidence and H16.3 fallback proof.**

Behaviour change, narrow. For frames classified as sequential fast-path
eligible (class B): request only source N. For all other classes: keep the
bounded-warmup source window.

**Precondition:** H15.6B must not begin until the predecessor-missing
fallback case is proven (H16.3), because reducing source requests before
fallback eligibility is fully sorted could starve a fallback path
(VS-LIFECYCLE-01).
...
| H15.6B | active source-plan reduction (fast-path-eligible only) | Pending | Requires H15.6A evidence and H16.3 |
...
| H16.3 | predecessor-missing fallback return-transfer proof | Pending | Key fallback coverage; gates H15.6B |
