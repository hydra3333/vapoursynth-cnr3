...
**Version:** v10.0 (supersedes v9.0). v10.0 (2026-07-18) closes PQ-1, PQ-2 and PQ-6, records the shipped
state, and leaves the reservation table as the single remaining big arc.

*** v10.0 ADVANCE (2026-07-18) ***

## CLOSED SINCE v9.0

### PQ-1 — option error messages — **CLOSED** (`CMS07-RIDER.option-error-messages`)
Implemented, gated 17/17, committed. Range errors echo the received value; type errors state the type
problem plus the SAME expectation text (shared builder, anti-drift). Also DISCHARGED the option-parser
commit's deferred gate items 8 (invalid-option throws) and 9 (threshold==0 SUCCEEDS -> `y=0/192/wide`).
Byte-identical frames confirmed message text touched no pixels.

### PQ-2 — README / user documentation — **CLOSED** (`CMS07-DOC.cnr2-descriptive-options-readme`)
README.md shipped: option docs, cnr2->CNR3 equivalence table, the Y-is-not-filtered semantic, progressive
+ interlaced usage with the proven field-split helpers (Appendix A), metadata-poor `.avi` warning, errors
section, mechanism-by-mechanism cache section, measured mode table.

### PQ-6 — compile-time guard for R-ARCH-08 — **ABSORBED into reservation Step A**
The `#error` guard (plan-retry without fmParallel) lands with the planner-selector work, alongside a
second guard (plan-retry with the reservation planner). Not a standalone item any more.

### PQ-7 — folded into PQ-1 at v9.0 — **CLOSED with it.**

## NEW: SHIPPED STATE (not an investigation — context for everything below)

`CMS07-RELEASE.production-config` on `main`. Ship = fmParallelRequests + HALF-500 + plan-retry OFF +
`CNR3_DIAG_MASTER_PERMIT_DIAGS` OFF (silent log, 56/56). Dev = master ON (provenance + D-SUM, 57/57).
CI/release pipeline live and proven. Vendored R78 headers, API 4.0 profile.

## THE LIVE QUEUE (in priority order)

### PQ-3 — RESERVATION TABLE — **THE LAST BIG ARC (designed, parked)**
Full design banked in **CNR3_PROPOSAL_Reservation_Table_v1_0.md** (self-contained pickup document; a
coder-annotated **v1_1** also exists in FINAL_DOCS and should be read alongside it). Summary:
- **Step A** (proven by BYTE-IDENTITY): `CNR3_PLANNER_CLASSIC`/`CNR3_PLANNER_RESERVATION` exactly-one
  selector; hookify the shared per-hole compute engine (classic passes inlined no-ops -> zero codegen
  change); move the classic planner + plan-retry intact behind the selector; land both `#error` guards.
- **Step B** (proven by BEHAVIOUR + benchmark): registry >= 2x numThreads; INTENT-MARK at arInitial vs
  HARD CLAIM at compute-start; collapse-at-F planning (an in-flight frame is a future checkpoint, so
  earlier holes become irrelevant); fetch avoidance for holes <= F; 10s bounded awaits that LOUD
  CLIP-FAIL on timeout (D-R5); RAII deregistration on EVERY exit path (the crux — a leaked entry makes
  waiters burn full deadlines on ghosts); both bails retained; counters from day one
  (holes_in_flight/absent, await_success/timeout, source_fetches_avoided).
  Acceptance: x=0, invariants 0, **L -> ~0**, duplicates -> ~0, then the real-footage benchmark against
  the 337 fps ship figure decides SHIP or PARK-WITH-DATA.
- **Scope A's FIRST mandatory deliverable: cold-verify arInitial's ACTUAL pass/mutex structure**
  (R-PROCESS-28: recollection doesn't cut patches).
- **Open question Q4** (settle in Scope B): with fetch avoidance, what if the awaited F's activation exits
  WITHOUT publishing? Likely resolution: await F's PUBLISH via the cache re-check already in the wait
  loop — F deregistering without publishing means F failed, so the clip is failing anyway.

### PQ-4 — Residual chroma desaturation on hard-cut / flashing content — **LOW PRIORITY**
3 frames in 957 (0.3%) on real interlaced footage. **Users self-serve with `scene_chroma=True`**, and the
README now documents exactly that use case. Revisit only if it proves to matter in practice.

### PQ-5 — A1 plan-trace analysis tool / A3 real-footage campaign — **PARKED**
Note A1's inputs now require a **master-ON dev build** (PLANTRACE is gated off in ship). A2 is effectively
discharged by the fmParallel diagnosis + plan-retry sweep + mode comparison (A2_first_findings_v3).

### PQ-8 — NEW: first Release not yet cut
The `release: types: [created]` trigger path is the one pipeline branch not yet exercised. Cutting the
first Release proves it (expect the zip attached automatically alongside GitHub's source-code links).

*** end v10.0 advance ***


**Version:** v9.0 (supersedes v8.0). v9.0 (2026-07-15) banks the outcomes of five committed arcs, RESOLVES
FI-16, ADVANCES FI-03 and FI-11 with hard data, and records the live parked queue.

*** v9.0 ADVANCE (2026-07-15) ***

## RESOLVED SINCE v8.0

### FI-16 — run-log marker emission — **RESOLVED**
The marker was never a regression: verify-cold + bisecting old GitHub trees (by MARKER, not filename)
established it had been **selftest-only since CMS07-G.6A** — a latent OMISSION. Fixed in
CMS07-SCAFFOLD.filter-mode-selector: the plugin create path now emits `edit_version=` and `filter_mode=`.
The emission-presence gate is R-PROCESS-27 (ratified). A THIRD provenance line has since joined them:
`response_config:` (the 11 resolved live option values). **New standing rule from a later failure:
R-PROCESS-30 — verify the marker LANDED after every commit** (CMS07-FIX.operational-response-defaults shipped
without its marker bump and under-claimed for a whole commit).

### FI-03 — CR4 tension at the active_ceiling lower clamp — **ADVANCED (exercised, not closed)**
The HALF-500 profile put CR4 under direct pressure and the tension resolved cleanly WITHOUT relaxing the rule:
at 5 zones, max-protected = 5*(50+10)+48 = 348 -> CR4 wants ceiling >= ~696, so a 500 ceiling VIOLATES it. W3X
chose to reduce HALF's zone count to 3 -> 228 protected -> CR4 wants >= 456 -> **500 fits with the 2x factor
intact**. This validates the FI-03 observation that CR4 is zone-count-dependent, and demonstrates the
preferred resolution: **size the protected set to the ceiling, do not weaken the assert.** A 1.4x relaxation
was considered and rejected. Derived values move with the zone count (MAX_PROTECTED 348->228, GRID_FLOOR
25->15) — both must be re-derived, never hardcoded, whenever a profile changes zones.

### FI-11 — recovery-path re-churn — **ADVANCED (the counter did its job)**
`frames_recently_evicted_then_re_requested` is now PROVEN as the decisive instrument for the
tunable-vs-inherent distinction it was designed for. The HALF-500 A/B: re-churn = **0 at BOTH 500 and 1000**
on a 3000-frame workload -> the extra evictions at 500 are entirely COLD frames, so halving the cache costs
nothing. Any future cache-size question should be settled the same way: sweep the ceiling, watch re-churn,
and take the smallest ceiling that holds it at 0.

## THE LIVE PARKED QUEUE (in priority order)

### PQ-1 — CNR3_RIDER.option-error-messages — **SCOPE APPROVED, NO PATCH**
Message-text-only improvement: range errors echo the received value; type errors read "incorrect value type"
plus the same expectation text, built ONCE per option kind and shared by both paths. The coder chat approved
the scope, then degraded at its limit (emitted an inline "patch", retracted it as unvalidated draft). A fresh
coder chat implements from CNR3_Rider_Scope_OptionErrorMessages_v2.md.
**Carries with it: the option parser's DEFERRED gate items 8 (invalid-option throws) and 9 (threshold==0 must
SUCCEED) — the same runs discharge both.** If any invalid value turns out to be ACCEPTED -> parser bugfix,
not the rider.

### PQ-2 — CMS07-DOC.cnr2-descriptive-options-readme — **OWED**
The user-documentation half of the option surface (the in-code maintainer half shipped with the parser).
Draft banked: CNR3_README_draft_user_options_v1.md; the same text is in
CNR3_cnr2_option_mapping_and_spec_v6.md. Must carry: the cnr2<->cnr3 name-equivalence table, per-option
behavioural effects, the **Y-is-not-filtered** note (the Y response is a luma-change guard gating chroma via
cross-plane weighting), threshold-zero semantics, curve semantics (j*j vs j), and scene_chroma's use case.

### PQ-3 — RESERVATION TABLE — **THE REAL fmParallel FIX (designed, parked)**
Research-ranked #1: an in-flight registry / memo-future (singleflight specialised to the recursive chain):
claim frame N at compute-start with a waitable placeholder; late consumers await/adopt instead of
recomputing. Closest analogue = ffmpeg frame-threading progress (report/await_progress). **Deadlock caution:
bounded-await + compute-fallback are MANDATORY (finite VS worker pool).** Parallel-prefix is inapplicable
(the blend is nonlinear).
**W3X CONSTRAINT (must carry into the scope): the reservation table, CNR3_ENABLE_PLAN_RETRY_BIAS, and the
cache-profile gating must all COMPOSE — work separately or together.**
**W3X's parked Stage-1.5 idea:** an arInitial-entry in-flight registry (per-instance mutex, RAII-removed at
EVERY arAllFramesReady exit path) so a wait loop can distinguish in-flight holes from genuinely-absent ones —
the cheapest slice that fixes plan-retry's one structural blind spot.
Note the current ship mode (fmParallelRequests) does NOT need this; it is what would make full fmParallel
shippable.

### PQ-4 — Residual chroma desaturation on hard-cut / flashing content — **LOW PRIORITY**
3 frames in 957 on real interlaced footage (0.3%); worse on a flashing-lights music video (a pathological
case for any temporal filter). **Users can now self-serve with `scene_chroma=True`** (chroma-inclusive scene
detection), which is exactly the knob for colour-only transitions. Revisit only if it proves to matter in
practice.

### PQ-5 — A1 plan-trace analysis tool / A3 real-footage 576p50 campaign — **UNCHANGED, PARKED**
A1's seed question-set is banked (Provenance). A2 is effectively DISCHARGED by the fmParallel diagnosis +
plan-retry sweep + mode comparison (see A2_first_findings_v3.md) — the C1-ownership-under-race question was
answered: ownership/invariants held (0/0) across ~95 cells and every mode.

### PQ-6 — Compile-time guard for R-ARCH-08 — **NEW, SMALL**
Consider `#error` when `CNR3_ENABLE_PLAN_RETRY_BIAS` is defined without `CNR3_FILTER_MODE_PARALLEL`. The rule
is now documented and ratified, but a guard would make the 2.7x-throttle mistake impossible rather than merely
discouraged.

### PQ-7 — Error-message value echo, type-error expectation text — **FOLDED INTO PQ-1**
(Was raised separately; now the whole content of the rider.)
