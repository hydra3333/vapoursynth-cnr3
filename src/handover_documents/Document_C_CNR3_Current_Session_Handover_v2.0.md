...
**Version:** v2.0  
**Date:** 2026-06-11  
**Status:** Current-session handover updated from v1.9 after CMS06.11 superseded CMS06.10; ready for H15.6B.1 coding only after reviewing CMS06.11.  
...
1. `CNR3_Handover_Pack_v2.0_MANIFEST.md`
2. `Document_A_CNR3_Project_Context_and_Rules_v2.0.md`
3. `Document_B_CNR3_Decision_Log_v2.0.md`
4. `Document_C_CNR3_Current_Session_Handover_v2.0.md`
5. `cnr3_cache_manager_design_v6_11.md`
6. `CNR3_CMS06_11_Handover_Reconciliation_Notes_20260611.md`
7. `CNR3_Handover_Pack_Production_Spec_v1.5.md` when regenerating or updating handovers
8. current relevant source files from the user's repository
9. latest build/test logs if the task depends on evidence

Rules for the new chat:

```text
Treat CMS06.11 as the current detailed cache-manager design authority.
Treat this Document C v2.0 as the source of truth for current implementation status.
Treat Document B v2.0 as the source of truth for settled decisions.
...
Do not claim fmParallelRequests or fmParallel readiness.
...
## C2. Current exact status

### C2.1 Completed/PASS status relevant to this handover
...
Pre-H15.6B checkpoint commit                                  completed
```

### C2.2 Current controlling design after CMS06.11

CMS06.11 supersedes CMS06.10 and is the active design authority. CMS06.11 is a correctness/proof-audit tightening pass over CMS06.10. It does not reverse the CMS06.10 predecessor-reservation model; it makes it clearer and safer to code.

The key CMS06.11 changes that affect the next coding phase are:

```text
- stale H15.6/H15.7 body wording fixed;
- H15.6B.3 source reduction requires a held reserved predecessor ref;
- named/null-guarded frameData release helper specified;
- H15.6B.2 PASS evidence fields strengthened;
- reserved predecessor refs tied into existing lookup-ref accounting;
- reserved predecessor refs clarified as lookup-addref, not pinning;
- H17 sparse-hole repair recorded as deferred;
- fail-closed-only H15.6B draft warning mirrored into Section 8.
```

---

## C3. Immediate next coding task
...
Prove that `arInitial` can atomically acquire a caller-owned reserved predecessor ref for `output_cache[N-1]`, store it in per-invocation frameData, carry it across `arInitial -> arAllFramesReady`, and release it safely on every tested path, without reducing source requests.
...
```text
- add frameData fields for a reserved predecessor ref and related diagnostics;
- acquire the ref in arInitial using cnr3_output_cache_find_frame_and_add_ref(N-1);
- store the owned ref in frameData only after successful acquisition;
- leave source requests conservative;
- add or prove a named/null-guarded release helper or currently named equivalent;
- prove cleanup release and zero lookup-ref balance;
- prove no checkpoint pin or non-checkpoint pin misuse;
- leave output authority behaviour otherwise unchanged;
- leave old strict state quarantined and non-authoritative.
```

H15.6B.1 must not:

```text
- request only source N;
- change active source-request behaviour;
- consume the carried predecessor as the fast-path predecessor for final output authority;
- implement H17 sparse-hole/minimal fallback optimisation;
- claim fmParallelRequests or fmParallel readiness.
```

### C3.3 H15.6B.1 required proof evidence

At minimum, H15.6B.1 evidence should include:
...
reserved_predecessor_field_null_after_cleanup
lookup_owned_ref_acquired_total
lookup_owned_ref_released_total
lookup_owned_ref_transferred_total
lookup_ref_balance=0
checkpoint_pin_underflow=0 or equivalent
checkpoint_unpin_failures=0 or equivalent
total_pin_count=0 at cleanup
...
If H15.6B.1 includes a forced cleanup/error path, it should prove that the release helper is idempotent and does not double-release when called more than once after the field is nulled.

---

## C4. Next subphases after H15.6B.1

### C4.1 H15.6B.2 / reserved-predecessor fast-path consumption proof

Only after H15.6B.1 PASS, H15.6B.2 should prove that the carried reserved predecessor ref is actually used as the fast-path predecessor in `arAllFramesReady`, replacing the second lookup of `N-1`.

H15.6B.2 still keeps conservative source requests.
...
reserved_predecessor_consumed=1 on consumed paths
reserved_predecessor_released_by_cleanup=0 for consumed refs
reserved_predecessor_released_by_cleanup=1 on unconsumed/error paths
reserved_predecessor_field_null_after_cleanup=1 on cleanup paths
lookup_ref_balance=0
source_request_reduction=0
```

### C4.2 H15.6B.3 / active sequential source-request reduction

Only after H15.6B.1 and H15.6B.2 PASS, H15.6B.3 may reduce source requests for safe class-B sequential fast-path candidates.

CMS06.11 eligibility rule:

```text
A frame is source-reduction eligible only if arInitial successfully acquired
and stored a reserved predecessor ref for output_cache[N-1].
```

A cache-contains check or non-owning observation that `N-1` exists is insufficient. If no reserved predecessor ref is held, the request must keep conservative bounded-warmup source requests.

---

## C5. Ownership-proof standing rule

Every cache-affecting proof or validation phase must explicitly prove both ownership systems where relevant.

### C5.1 Lookup-addref ownership

For ordinary cached-frame use, including reserved predecessor refs:
...
Reserved predecessor refs are expected to be released, not transferred, in H15.6B.1 and H15.6B.2 because they are input predecessor refs, not returned frames.

### C5.2 Checkpoint pin ownership

For checkpoint/recovery anchors:

```text
- pin/unpin counts;
- underflow/failure checks;
- total_pin_count=0 at cleanup;
- has_pinned_checkpoints=0 at cleanup.
```

Reserved predecessor refs are not checkpoint pins and must not introduce checkpoint pin activity for ordinary cached predecessors.

---

## C6. Deferred H17 sparse-hole/minimal fallback recovery optimisation

H17 is now recorded in CMS06.11 as a deferred design item. It remains important but must not be mixed with H15.6B.

Current fallback remains correctness-first and conservative. It may retrieve/recompute a bounded lower frame even when that output is already cached, relying on first-in-best-dressed duplicate handling to discard duplicate output safely.

Future H17-style work should start from the nearest suitable cached predecessor or checkpoint inside the bounded recovery window, retrieve only missing source frames needed to walk forward, compute/store only missing outputs plus the requested output, and return through output-cache authority.

---

## C7. Repository/code context
...
The user's current committed state is after the pre-H15.6B checkpoint commit. That checkpoint records H16.3 and H16.4 as complete, records that H15.6B will be restructured, and records the future H17 sparse-hole/minimal fallback recovery optimisation for later handover/CMS updates.

---

## C8. Do not implement next unless explicitly chosen

Do not implement the following during H15.6B.1 unless the user explicitly changes scope:

```text
- H15.6B.2 consumption behaviour;
- H15.6B.3 source-request reduction;
- H17 sparse-hole/minimal fallback optimisation;
- non-checkpoint pinning;
- fmParallelRequests or fmParallel mode changes;
- old strict-state retirement;
- broad diagnostic cleanup unrelated to H15.6B.1 proof.
```
...
Before marking H15.6B.1 PASS or offering a commit message, check:

```text
- build succeeds;
- source requests remain conservative;
- reserved predecessor acquisition/release counters are balanced;
- lookup_ref_balance=0;
- no checkpoint pin misuse;
- no source-frame leaks;
- no local-output leaks;
- no old strict-state mutation;
- proof-only code is gated;
- normal operation path is not accidentally dependent on debug-only proof code.
```

When the user marks a development/test step as PASS, provide a suitable commit message.

---

## C10. Current new-chat starter will be reconsidered separately

The v1.9 starter prompt should not be reused unchanged, because it names CMS06.10 as controlling. A revised starter prompt should be produced separately after this v2.0 pack is reviewed.

---

## Appendix C - Full v1.9 Document C retained for continuity

The following is the full v1.9 Document C content. It remains useful history and detailed continuity, but any CMS06.10-as-current wording inside it is superseded by this v2.0 current layer and by CMS06.11.
