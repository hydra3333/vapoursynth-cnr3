...
**Version:** v4.10 (DELTA through D.4 — adopt-skip + first-in-best-dressed primitives proven; selftest 51/51)
**Date:** 2026-06-26
...
```text
Committed/pushed through:  CMS07-D.4-adopt-skip-primitive-selftest-proof
...
Selftest count:            51/51 PASS  (forced-fail 50/51 exit 1; verbose 51/51)  [D.4 added TWO
                           cache-core selftest cases: present-frame adopt-skip primitive + first-in-
                           best-dressed duplicate; 49->51. D.1-D.3 were plugin-only (no cache-core
                           change); D.4 is selftest-only (no production change)]
...
## 6. OWED-ITEMS LEDGER (none blocking D.5)

- **D.4 ADOPT-SKIP + FIRST-IN-BEST-DRESSED PRIMITIVES (committed; selftest-only).** D.4 proved, at the
  cache-core selftest level, the ownership/concurrency primitives the D.3 floor/hole adopt-skip paths
  rely on, for the case where the race OUTCOME has occurred (frame present at fill time). Two cases:
  (1) present-frame adopt-skip — lookup_frame_and_record_pin adopts a present frame, records the
  consumer pin with NO addFrameRef/freeFrame side effects, the pinned frame is eviction-protected
  (remove_unpinned_frame -> lifecycle_violation), AS4 discharge balances, clear releases exactly once;
  (2) first-in-best-dressed duplicate — a duplicate store preserves the existing winner (authoritative
  by frame identity), releases the incoming loser exactly once, reports duplicate_existing_slot /
  incoming_frame_rejected / pin_recorded, balances. The adopt-skip primitive is CONTEXT-FREE (floor vs
  hole is a live-plugin concern already proven in D.3), so it is proven ONCE. Selftest-only: no live
  plugin change, no cache-core production change, no test-injection seam, no helper extraction, no pixel
  goldens (proof is status/ownership). Selftest 49 -> 51. **Real fmParallel-scheduling reachability of
  the race is EXPLICITLY DEFERRED to D.5 / fmParallel validation** — D.4 proves the primitive behaves
  correctly WHEN the race outcome occurs, not that live scheduling reaches it. Proven both configs:
  51/51 normal, 50/51 forced-fail (expected invariant_violation, exit 1), 51/51 verbose.
...
- **TEST ARTIFACTS / GOLDEN PROVENANCE (housekeeping; add to repo alongside the existing harnesses).**
  ...
  regression bases for later phases (D.3+ must keep D.1 and D.2 green):
    ...
    * D.4 has NO separate harness or golden script: its proof is the TWO cache-core selftest cases
      (cnr3_cache_core_selftest: present-frame adopt-skip primitive + first-in-best-dressed duplicate),
      which ship in the selftest target itself (count 49->51). Status/ownership proof, not pixels.
