...
**Date:** 2026-06-30 (CMS07.15; supersedes CMS07.14 dated 2026-06-29, which superseded CMS07.13 dated 2026-06-27, which superseded CMS07.12 dated 2026-06-27, which superseded CMS07.11 dated 2026-06-27, which superseded CMS07.10 dated 2026-06-25, which superseded CMS07.9 dated 2026-06-24, which superseded CMS07.8 dated 2026-06-23, which superseded
...
CMS07.15 is an ADDITIVE CLARIFICATION over CMS07.14: it records, in §7.5, the store-status RETURN contract
of the combined helper — the production-vs-AS2-consumer asymmetry on an already-present frame number —
exactly as surfaced and proven by the W.3 implementation (committed CMS07-W.3-combined-live-store-prune-helper;
four-way selftest 55/55 both configs, forced-fail 54/55 exit 1; designer eviction-proof live A/B harness PASS:
byte-identical output under live eviction, with the §7.2 capacity AND §7.4 checkpoint triggers proven to fire
and detach victims). It adds NO rule, constant, AS-scope, or section-number change and changes NO behaviour (W.3
already ships this); it documents an existing contract so a future consumer reads the correct field. This paragraph
also SUPERSEDES the 'implementation owed' tail of the implementation-state note above: the live cache-pressure
wiring arc W.1 (§7.4 trigger) → W.2 (§7.6 arInitial observation) → W.3 (§7.5 combined helper) is now COMPLETE
and committed; the design text of §7.4-§7.6 is unchanged.
...
**Store-status return contract (recorded CMS07.15; surfaced and proven by W.3).** The helper's two store
flavours report an already-present frame number ASYMMETRICALLY, and a consumer must read the right field:
- A **production store** (step 1 for a branch target / frame-0) returns the raw store status: `duplicate`
  (not `ok`) when the frame number is already present, per first-in-best-dressed (§9.3). Production callers
  do not pin and do not consult a record summary; the `duplicate` return is the signal.
- An **AS2-consumer pinned store** (step 1+3 for a cache-hit / predecessor / recovery anchor / floor / hole)
  NORMALIZES the return to `ok` and reports the pre-existing slot OUT-OF-BAND via the record summary's
  `duplicate_existing_slot` field (`Cnr3CacheAs2StoreRecordSummary`). The return is `ok` precisely because the
  consumer's contract is "a usable, pinned slot now exists" — satisfied whether this activation's frame won
  the store or an existing winner did. A consumer that must KNOW it hit a duplicate reads `duplicate_existing_slot`,
  NOT the return code; reading the return alone would never observe an AS2 duplicate.

In both flavours the EXISTING winner is authoritative (first-in-best-dressed §9.3); the computed loser is freed
after unlock (§7.5 step 6); and exactly ONE pin is recorded for the activation (against the winning slot — never
a second pin for the discarded loser). The checkpoint *flag* under a duplicate is handled separately and is
monotonic (§6.6). This records the existing contract W.3 wired and proved; it adds no rule or behaviour.
