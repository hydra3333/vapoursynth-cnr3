...
```text
Committed/pushed through:  CMS07-DIAG-tinycache-scaffold  (tiny-cache diagnostic scaffold + profile marker + protection test; on top of CMS07-W.3)
Selftest count:            56/56 PASS  (forced-fail 55/56 exit 1; verbose 56/56)  [was 55/55; +1 = profile-agnostic protection-under-eviction test, runs in BOTH profiles]
...
Controlling CMS:           CMS07.15  (cnr3_cache_manager_design_v7_15.md)  [UNCHANGED by the scaffold — no design/invariant change; scaffold is behind a compile guard]
...
- **NEW — PROFILING FINDING BANKED (FI-10, investigation only, NOT actioned): native<->scalar plane marshalling
  ...
  Candidate lever = the typed-row-pointer pixel-path rewrite the P.11B selftest comment already deferred; est.
  ~1.5-2x on this path, but a SEPARATE R-PROCESS-21 arc touching proven P.7A-P.11B code — scoped only if/when opened.
  Recorded in full in Future Investigations v7.15 as FI-10. Also banked there: the ~3 fps concern was the TINY-100
  pruning cadence, NOT the cache architecture (normal build ~46 fps sequential) — so nobody re-opens that worry.
