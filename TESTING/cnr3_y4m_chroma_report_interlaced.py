#!/usr/bin/env python3
"""
cnr3_y4m_chroma_report_interlaced.py
...
This interlaced-aware variant keeps the original whole-frame analysis and
also emits per-field delta metrics. It does not deinterlace; it splits stored
top/bottom field rows for reporting so field-local chroma damage can be seen.
For C420 Y4M, per-field chroma rows assume a field-weaved VapourSynth-style
stream. If the file is frame-coded 4:2:0 instead, field chroma rows are an
approximation; whole-frame metrics remain exact.
"""
...
def resolve_field_order(interlace_tag: str, requested: str) -> tuple[str, str]:
    """Return (order, note). order is tff or bff."""
    if requested in ("tff", "bff"):
        return requested, "explicit"

    tag = (interlace_tag or "").lower()
    if tag == "t":
        return "tff", "from Y4M It"
    if tag == "b":
        return "bff", "from Y4M Ib"
    if tag == "p":
        return "tff", "Y4M says progressive; using tff only to label row parity"
    if tag == "m":
        return "tff", "Y4M says mixed; using tff unless --field-order overrides"
    return "tff", f"unknown Y4M interlace tag I{interlace_tag}; using tff unless --field-order overrides"


def field_temporal_label(spatial_field: str, field_order: str) -> str:
    if field_order == "bff":
        return "first" if spatial_field == "bottom" else "second"
    return "first" if spatial_field == "top" else "second"


def field_slice_for_plane(plane: np.ndarray, spatial_field: str) -> np.ndarray:
    start = 0 if spatial_field == "top" else 1
    if plane.shape[0] <= start:
        return plane[0:0, :]
    return plane[start::2, :]


def analyze_delta_only(
    frame_index: int,
    spatial_field: str,
    temporal_field: str,
    in_y: np.ndarray,
    ...
    chroma_mag_mean_input = float(mag_in.mean()) if mag_in.size else 0.0
    chroma_mag_mean_output = float(mag_out.mean()) if mag_out.size else 0.0
    chroma_mag_mean_delta = float(mag_delta.mean()) if mag_delta.size else 0.0
    ...
    classification, notes = classify_frame(
        ...
        red_brown_mean_mag_delta=red_brown_mean_mag_delta,
        exact_all=exact_all,
        cnr3_default_scene_candidate=None,
        likely_reset_passthrough=exact_all or near_all,
    )
...
def analyze_interlaced_fields(
    ...
    rows: list[FieldStats] = []
    for spatial_field in ("top", "bottom"):
        temporal_field = field_temporal_label(spatial_field, field_order)
        rows.append(
            analyze_delta_only(
                frame_index=frame_index,
                spatial_field=spatial_field,
                temporal_field=temporal_field,
                in_y=field_slice_for_plane(in_y, spatial_field),
                in_u=field_slice_for_plane(in_u, spatial_field),
                in_v=field_slice_for_plane(in_v, spatial_field),
                out_y=field_slice_for_plane(out_y, spatial_field),
                out_u=field_slice_for_plane(out_u, spatial_field),
                out_v=field_slice_for_plane(out_v, spatial_field),
                near_mad_threshold=near_mad_threshold,
                chroma_strong_threshold=chroma_strong_threshold,
                red_brown_v_threshold=red_brown_v_threshold,
            )
        )
    return rows


def write_field_csv(path: str, rows: list[FieldStats]) -> None:
    fieldnames = list(FieldStats.__dataclass_fields__.keys())
...
def make_field_summary(rows: list[FieldStats]) -> dict[str, object]:
    classes = [
        "UNCHANGED",
        "NEAR_RESET",
        "MINOR",
        "MODERATE",
        "SUBSTANTIVE_CHROMA_CHANGE",
        "SUBSTANTIVE_DESATURATION",
        "LIKELY_DESATURATION_BUG",
    ]
    by_field: dict[str, dict[str, int]] = {}
    for spatial_field in ("top", "bottom"):
        field_rows = [r for r in rows if r.spatial_field == spatial_field]
        by_field[spatial_field] = {name: sum(1 for r in field_rows if r.classification == name) for name in classes}
        by_field[spatial_field]["rows"] = len(field_rows)

    by_temporal: dict[str, dict[str, int]] = {}
    for temporal_field in ("first", "second"):
        field_rows = [r for r in rows if r.temporal_field == temporal_field]
        by_temporal[temporal_field] = {name: sum(1 for r in field_rows if r.classification == name) for name in classes}
        by_temporal[temporal_field]["rows"] = len(field_rows)

    worst_chroma = sorted(rows, key=lambda r: max(r.u_mad, r.v_mad), reverse=True)[:20]
    worst_desat = sorted(rows, key=lambda r: r.chroma_mag_mean_delta)[:20]

    return {
        "field_rows_analyzed": len(rows),
        "likely_desaturation_field_rows": sum(1 for r in rows if r.classification == "LIKELY_DESATURATION_BUG"),
        "substantive_field_rows": sum(
            1 for r in rows
            if r.classification in ("LIKELY_DESATURATION_BUG", "SUBSTANTIVE_DESATURATION", "SUBSTANTIVE_CHROMA_CHANGE")
        ),
        "by_spatial_field": by_field,
        "by_temporal_field": by_temporal,
        "worst_fields_by_chroma_mad": [
            {"frame": r.frame, "spatial_field": r.spatial_field, "temporal_field": r.temporal_field, "classification": r.classification}
            for r in worst_chroma
        ],
        "worst_fields_by_desaturation": [
            {"frame": r.frame, "spatial_field": r.spatial_field, "temporal_field": r.temporal_field, "classification": r.classification}
            for r in worst_desat
        ],
    }
...
def append_interlaced_md(
    path: str,
    input_info: Y4MInfo,
    field_order: str,
    field_order_note: str,
    field_rows: list[FieldStats],
) -> None:
    field_summary = make_field_summary(field_rows)
    worst_chroma = sorted(field_rows, key=lambda r: max(r.u_mad, r.v_mad), reverse=True)[:20]
    worst_desat = sorted(field_rows, key=lambda r: r.chroma_mag_mean_delta)[:20]

    with open(path, "a", encoding="utf-8") as f:
        f.write("\n\n## Interlaced / field-aware analysis\n\n")
        f.write(f"- Y4M interlace tag: `I{input_info.interlace}`\n")
        f.write(f"- Field order used for temporal labels: `{field_order}` ({field_order_note})\n")
        f.write("- Spatial field mapping: top = even stored rows, bottom = odd stored rows.\n")
        f.write("- C420 chroma caveat: per-field U/V rows assume a field-weaved VapourSynth-style stream. ")
        f.write("If the file is frame-coded 4:2:0, field U/V rows are approximate; whole-frame U/V metrics remain exact.\n\n")

        f.write("### Field totals\n\n")
        f.write("| Field grouping | Rows | Substantive rows | Likely desaturation rows |\n")
        f.write("|---|---:|---:|---:|\n")
        for key, label in (("top", "top spatial"), ("bottom", "bottom spatial")):
            item = field_summary["by_spatial_field"][key]  # type: ignore[index]
            substantive = (
                item["SUBSTANTIVE_CHROMA_CHANGE"]
                + item["SUBSTANTIVE_DESATURATION"]
                + item["LIKELY_DESATURATION_BUG"]
            )
            f.write(f"| {label} | {item['rows']} | {substantive} | {item['LIKELY_DESATURATION_BUG']} |\n")
        for key, label in (("first", "first temporal"), ("second", "second temporal")):
            item = field_summary["by_temporal_field"][key]  # type: ignore[index]
            substantive = (
                item["SUBSTANTIVE_CHROMA_CHANGE"]
                + item["SUBSTANTIVE_DESATURATION"]
                + item["LIKELY_DESATURATION_BUG"]
        ...
        f.write("### Worst field rows by chroma mean absolute delta\n\n")
        f.write("| Frame | Spatial field | Temporal field | Class | Y MAD | U MAD | V MAD | U changed % | V changed % | Chroma mag delta | Notes |\n")
        f.write("|---:|---|---|---|---:|---:|---:|---:|---:|---:|---|\n")
        for r in worst_chroma:
            f.write(
                f"| {r.frame} | {r.spatial_field} | {r.temporal_field} | {r.classification} | "
                f"{fmt_float(r.y_mad)} | {fmt_float(r.u_mad)} | {fmt_float(r.v_mad)} | "
                f"{fmt_float(r.u_changed_pct)} | {fmt_float(r.v_changed_pct)} | {fmt_float(r.chroma_mag_mean_delta)} | {r.notes} |\n"
            )
        f.write("\n")

        f.write("### Worst field rows by chroma magnitude loss\n\n")
        f.write("| Frame | Spatial field | Temporal field | Class | Chroma mag in | Chroma mag out | Chroma mag delta | Strong toward neutral % | Red/brown samples | Red/brown mag delta |\n")
        f.write("|---:|---|---|---|---:|---:|---:|---:|---:|---:|\n")
        for r in worst_desat:
            f.write(
                f"| {r.frame} | {r.spatial_field} | {r.temporal_field} | {r.classification} | "
                f"{fmt_float(r.chroma_mag_mean_input)} | {fmt_float(r.chroma_mag_mean_output)} | "
                f"{fmt_float(r.chroma_mag_mean_delta)} | {fmt_float(r.chroma_strong_toward_neutral_pct)} | "
                f"{r.red_brown_samples} | {fmt_float(r.red_brown_mean_mag_delta)} |\n"
            )


def write_interlaced_json(
    path: str,
    summary: Summary,
    input_info: Y4MInfo,
    field_order: str,
    ...
    payload = {
        "frame_summary": asdict(summary),
        "interlaced": {
            "input_interlace_tag": input_info.interlace,
            "field_order": field_order,
            "field_order_note": field_order_note,
            "field_chroma_assumption": "C420/C422/C444 U/V rows are split by stored row parity; exact for field-weaved streams, approximate for frame-coded C420 chroma.",
            "field_summary": make_field_summary(field_rows),
        },
    }
    with open(path, "w", encoding="utf-8") as f:
...
def analyze(args: argparse.Namespace) -> tuple[Summary, list[FrameStats], list[FieldStats], str, str, str, str]:
    rows: list[FrameStats] = []
    field_rows: list[FieldStats] = []
    ...
    with Y4MReader(args.input) as input_reader, Y4MReader(args.output) as output_reader:
        ...
        field_order, field_order_note = resolve_field_order(input_info.interlace, args.field_order)
    ...
    field_csv_path = args.field_csv or f"{out_prefix}_per_field.csv"
    md_path = args.md or f"{out_prefix}_report.md"
    json_path = args.json or f"{out_prefix}_summary.json"

    if not args.no_csv:
        write_csv(csv_path, rows)
    if not args.no_field_csv:
        write_field_csv(field_csv_path, field_rows)
    if not args.no_md:
        write_md(md_path, summary, rows)
        append_interlaced_md(md_path, input_info, field_order, field_order_note, field_rows)
    if not args.no_json:
        write_interlaced_json(json_path, summary, input_info, field_order, field_order_note, field_rows)

    return summary, rows, field_rows, csv_path, field_csv_path, md_path, json_path


def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Compare selected-input and CNR3-output Y4M clips for chroma attenuation/desaturation, with interlaced field reporting."
    )
    ...
    p.add_argument("--csv", default=None, help="Explicit per-frame CSV output path.")
    p.add_argument("--field-csv", default=None, help="Explicit per-field CSV output path.")
    ...
    p.add_argument("--no-csv", action="store_true", help="Do not write per-frame CSV.")
    p.add_argument("--no-field-csv", action="store_true", help="Do not write per-field CSV.")
    ...
    p.add_argument("--field-order", choices=("auto", "tff", "bff"), default="auto", help="Field order for temporal labels. Default: auto from Y4M I tag.")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        summary, rows, field_rows, csv_path, field_csv_path, md_path, json_path = analyze(args)
    ...
    field_likely_desat = sum(1 for r in field_rows if r.classification == "LIKELY_DESATURATION_BUG")
    field_substantive = sum(
    ...
    print(f"field_rows_analyzed={len(field_rows)}")
    ...
    print(f"substantive_field_rows={field_substantive}")
    print(f"likely_desaturation_field_rows={field_likely_desat}")
