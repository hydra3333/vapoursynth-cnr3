# CNR3 — THIS-CHAT DELTA: current-state companion (SLIMMED, through marshalling levers 0A/0B/3a.1/3b.1/3a.2 + validation policy + A-lite + C1) — v4.21

**Version:** v4.21 (SLIMMED). Supersedes v4.20. Extends the marshalling-optimisation arc with the typed-pointer levers, the validation policy, Lever A-lite, and Lever C1 (buffer elimination). All steps value-preserving (56/56 four-way, P-series unchanged) and profiled; CMS UNCHANGED (still 07.15 — implementation optimisation only, no design/invariant change). Cumulative vs pre-lever baseline: **93,914 -> ~29,250 samples (~-69%)**.
...
- **Ahead (post-C1 landscape — top leaves shifted):** the unpack/downsample memory-elimination story is now largely DONE (bridge gone ~1.2k; unpack ~1.6k). The new TOP leaf is **F/3c = the BLEND** (`cnr3_process_chroma_plane_from_downsampled_luma` ~6,600, now ~22% of total — it did not grow, everything around it shrank): mixed arithmetic + the data-dependent response-table GATHER. Scope 3c as a scalar production-fast path + Tier-2/Tier-3 validation-policy application BEFORE any SIMD gather (vpshufb 256-entry LUT is a later maybe, needs multi-stage construction). Second leaf: **repack** (`cnr3_stage_scalar_plane_to_native_bytes` ~4,700, memory). Then **B** (allocation pooling as a lifetime/thread-safety proof under fmUnordered — churn is now a smaller share). **D** (EXACT SIMD downsample — NOT the biased VPAVGB) and **E** (scene-change local accumulator) remain cheap-but-small, deferred for cumulative fps. Parked: diagnostics D-SUM menu, selftest skip-pass fix.
...
```text
Committed/pushed through:  CNR3-OPT-LeverC1-native-luma-downsample-bridge  (marshalling arc: AVX2 + 0A + 0B + 3a.1 + 3b.1 + 3a.2 + A-lite + C1)
Selftest count:            56/56 PASS  (forced-fail 55/56 exit 1; verbose 56/56)  [unchanged by any lever — all value-preserving]
...
Marshalling arc status:    0A -28% | 0B flat | 3a.1 -37% | 3b.1 flat | 3a.2 flat-on-8bit | A-lite -7.4% | C1 -24.5% (BUFFER ELIMINATION, biggest single win). cumulative ~-69% (93,914 -> ~29,250). C1 fused the native-luma downsample bridge (read taps directly, no full-res int buffer); bridge leaf ~9.5k -> ~1.2k; value-identical (P.4A/P.9A proven). POST-C1: unpack/downsample memory-elimination largely DONE. NEW TOP LEAF = the BLEND (cnr3_process_chroma_plane_from_downsampled_luma ~6,600 ~22%), then repack (cnr3_stage_scalar_plane_to_native_bytes ~4,700). NEXT: F/3c (blend scalar-fast + Tier-2/Tier-3 policy, before SIMD gather). Then B (pooling), D/E cheap-but-small later.
Validation posture:        RECORDED (CNR3_Validation_Policy_recorded_v1): defend-at-source(T1)/trust-downstream(T2)/bounded-by-construction(T3)/final-clamp-always. Governs 3a.2/3b.2/3c.
...
Controlling CMS:           CMS07.15  (cnr3_cache_manager_design_v7_15.md)  [UNCHANGED by any lever — implementation optimisation only, no design/invariant change]
