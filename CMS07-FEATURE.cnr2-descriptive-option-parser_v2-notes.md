# CMS07-FEATURE.cnr2-descriptive-option-parser v2 patch notes
...
V2 correction after designer cold review: only `src/cnr3_build_config.h` marker/provenance wording changed from v1. The parser implementation, README documentation, registration, validation, emission, and application logic are otherwise unchanged.
...
- Bumps the edit marker to the clean change marker:

```text
CMS07-FEATURE.cnr2-descriptive-option-parser
```

- Adds the required provenance/debt note for the inherited marker issue: the prior commit (`CMS07-FIX.operational-response-defaults`) shipped without its marker bump, so its logs show the earlier snapshot marker while containing the operational-defaults fix; the `response_config` line is the tell. Marker provenance is corrected as of this commit.

- Does not label the `scene_threshold` CMS07-equivalent formula as debt. That remains a documented deliberate divergence/statement, not a marker/debt annotation.
...
| marker/provenance note required | bump clean marker and add prior-commit marker-provenance note in `cnr3_build_config.h` |
...
```bat
git status --short
git apply --ignore-whitespace --check CMS07-FEATURE.cnr2-descriptive-option-parser_v2.patch
git apply --ignore-whitespace CMS07-FEATURE.cnr2-descriptive-option-parser_v2.patch
git diff --check
git status --short
```
...
```bat
patch -p1 --binary < CMS07-FEATURE.cnr2-descriptive-option-parser_v2.patch
```
...
5. Default `.vpy` no-options `response_config` equals committed interim defaults plus scene fields, with the corrected clean marker visible in the same startup/provenance run.
