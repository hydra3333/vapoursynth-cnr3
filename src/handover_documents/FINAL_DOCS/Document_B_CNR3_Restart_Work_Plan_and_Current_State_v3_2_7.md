...
**Version:** v3.2.7 (RESUME-state work plan; focused status update. The version label is
kept at the "3.2" generation to stay aligned with Document A v3.2; the `.7` patch level
marks this update. Records the **scalar→native bridge as proven/committed** — P.7A source-luma
downsample traversal, P.8A native byte-plane access, and P.9A native luma downsample bridge —
on top of the complete scalar pixel pipeline (P.1A–P.6A) and the C.14A cache-core milestone;
selftest count is now **41/41**. The whole scalar path AND the native byte-buffer access layer
are now proven; the next phases are the **real VapourSynth frame-memory adaptation, scene-change,
and getFrame/cache integration** (where the proven cache core meets the proven pixel chain). See
§8. Earlier sections carry forward except the version pointers below.)  
**Date:** 2026-06-19  
...
NEXT — the scalar pixel pipeline AND the native byte-buffer access/bridge are complete; the
remaining phases cross into REAL VapourSynth machinery (proof surface = real/synthetic frame buffers
and VS-integration proofs, not scalar reference vectors):
    1. REAL VAPOURSYNTH FRAME-MEMORY ADAPTATION — wire the proven native byte-plane layer (P.8A) to
       actual VS frame memory: getReadPtr/getWritePtr/getStride, actual per-plane dimensions
       (getFrameWidth/getFrameHeight, plane-dependent due to chroma subsampling), and the decision
       on typed row pointers (reinterpret_cast<const uint16_t*>) vs the P.8A memcpy approach against
       the real frame-memory contract and its alignment guarantees. P.8A/P.9A proved the byte-stride
       LOGIC on synthetic buffers; this binds it to real frames. Validate real subsampled source
       dimensions here (P.7A's ceil was harness-only).
    2. SCENE-CHANGE accumulation (the deferred original "P.6A" content) — diff_total accumulates
       abs(diff_y << (subw+subh)) (+ chroma diffs if scenechroma) DURING the chroma pass; if it
       exceeds diff_max (scaled by scdthr and resolution) the frame is returned as SOURCE UNCHANGED
       (no recursive blend). Intrinsic to the compute step, not a separate pre-pass (CNR2 source).
    3. VapourSynth getFrame integration (arInitial/arAllFramesReady), source request/retrieve
       lifecycle, return-transfer — where the proven CACHE CORE (through C.14A) finally connects to
       the proven PIXEL CHAIN (through P.9A). The predecessor is the explicit previous FILTERED
       output supplied by the cache/recovery layer, NEVER source[N-1] (R-ARCH-06). The Category-B
       developer-alert (CMS §9.6.4) belongs here, at integration/error-mapping time: it is the
       EMISSION half of what the C.13B guard DETECTS — map the hard status to clean filter failure
       plus a bounded one-shot stderr developer-alert outside locks; expected Category-A
       duplicate/adopt stays silent. The VS-LIFECYCLE-01 rule (source frames retrieved in
       arAllFramesReady must have been requested in arInitial of the same activation) becomes binding.
    4. cnr3_frame_processing.cpp membership in the cnr3 PLUGIN project — it is in the selftest
       project (from P.3A); the real-frame phase (item 1) is the FIRST to need it in the plugin
       build. Confirm BEFORE item 1, via the VS2026 GUI (Add Existing Item), to avoid the link error
       the selftest hit at P.3A.
    5. Visual Studio 2026 project wiring for the full plugin build.

    NOTE on the arc shift: the remaining phases leave the isolated cache-core selftest harness and
    move into real VapourSynth frame memory and live interaction. The PDAP delivery process,
    read-first review for load-bearing work, genuine-failure-mode tests, and explicit-known expected
    values all still apply, but the proof surface changes. PROCESS NOTE: P.1A was reviewed read-first
    POST-COMMIT (sound — low-risk pure maths); every phase since (P.2A–P.9A) was reviewed read-first
    BEFORE apply, with every reference vector independently recomputed by the designer and (from P.3A)
    cross-checked against the fetched vsCnr2.cpp source. From here, and for anything touching
    cache-core/lifecycle/real-frame-memory, read-first-BEFORE-apply remains firm. See the
    Designer/Reviewer Role Handover (v1.4) for the full review discipline, the pixel-arc reference,
    the accuracy rule, and the review checklist (the byte-plane/stride items are now done; the live
    hunting list is real-frame memory, scene-change, and getFrame/cache integration).
```
