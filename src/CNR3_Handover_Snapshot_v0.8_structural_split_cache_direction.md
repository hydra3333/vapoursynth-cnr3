# CNR3 Handover Snapshot v0.8 - Structural Split Checkpoint and Cache Direction
...
This v0.8 snapshot is intentionally detailed. Do not compress it aggressively in a future handover unless the relevant detail has been superseded by code or by a later explicit decision.
...
cl ^
  ...
  /MD ^
  /DNOMINMAX ^
  /I third_partyapoursynth\include ^
  /Fo:build\ ^
  srcapoursynth-Cnr3.cpp ^
  src\cnr3_cache.cpp ^
  src\cnr3_response_tables.cpp ^
  /Fe:build\cnr3.dll ^
  /link ^
...
### Cache manager is the major future work - interim direction/specification

Important correction for future continuation:

```text
The planned cache-manager direction is not merely undecided "strict now, maybe cache later".
The current code is strict-streaming only, but the previously discussed future
cache-manager direction is a per-instance checkpoint/back-cache/speculative-refusal design.
```

This subsection records the agreed/planned direction as an **interim cache-manager specification**. It should be treated as a contribution to the next design discussion, not as fully finalised implementation detail. A future chat should review, clarify, confirm, and refine it before coding. It is recorded here specifically so it is not forgotten.
...
Settled future cache-manager direction to discuss/confirm:

```text
Per-instance cache manager:
    Each CNR3 filter instance owns its own cache.
    No global cache.
    No cross-instance cache sharing.
    No hidden dependence on worker-thread identity.

Recent output back-cache:
    Keep a bounded cache of recently produced filtered output frames.
    A practical starting size discussed was around 50 frames.
    Purpose:
        - satisfy small backward/nearby requests;
        - absorb minor scheduler/request jitter;
        - avoid unnecessary recomputation;
        - preserve recursive correctness by storing filtered outputs, not merely source frames.

Checkpointing:
    Keep checkpoint output frames at chosen intervals.
    Use checkpoints as safe restart anchors for recomputation.
    Keep the number of checkpoints bounded.
    Evict old checkpoints by an explicit policy.
    Consider extra checkpoint anchors around scene changes, because scene-change frames reset chroma history by copying current source chroma.

Jump-distance assessment:
    When a frame is requested, classify it relative to:
        - cache.next_needed;
        - the recent output back-cache;
        - the nearest available checkpoint;
        - configured maximum forward/backward/recompute distances;
        - whether the request looks like small scheduler jitter, a cascade artefact, or a larger seek.

Speculative refusal / "not available yet":
    For awkward out-of-order or near-future requests, do not always hard-fail immediately.
    A controlled speculative refusal / temporary-not-available response was discussed
    as a way to let VapourSynth's request cascade drain and possibly request the missing
    earlier frames first.

Cascade-drain behaviour:
    The cache manager should avoid fighting VapourSynth's scheduler unnecessarily.
    Some request patterns may be transient cascade effects rather than true random seeks.
    The design should allow a cascade to settle before choosing expensive recompute or final refusal.

Recovery/recompute:
    For recoverable jumps, recompute from the nearest suitable checkpoint.
    For small gaps, continue forward from current state if safe.
    For requests satisfied by recent output back-cache, return a frame reference directly.
    For very large jumps beyond policy limits, choose a clear refusal/recovery path rather than silently doing unbounded work.

Cache clearing and invalidation:
    Define explicit rules for clearing or partially clearing cache state.
    Required cases include:
        - filter instance cleanup;
        - unrecoverable discontinuity;
        - major seek/recovery reset if implemented;
        - bounded-cache eviction;
        - checkpoint eviction.
    Avoid accidental clearing that breaks a still-valid recursive chain.
    Avoid keeping stale frames after a reset/recompute policy decision.

Diagnostics:
    Add cache-state diagnostics that are useful but not too noisy.
    Useful fields may include:
        ...
        - checkpoint chosen;
        - jump distance;
        - speculative refusal count/state;
        - recompute start/end frames;
        - cache clear/eviction decisions.
```
...
- The first implementation should be phased; do not attempt a fully general random-access cache in one patch.
- Keep vspipe -r 1 strict-streaming behaviour working throughout development.
```

Recommended phased implementation direction:
...
Phase 1:
    Refactor current cache state into a clearer Cnr3CacheManager interface,
    with no intended behaviour change.

Phase 2:
    Add bounded recent output back-cache, probably starting around 50 frames.
    Use it for small backward/nearby requests if safe.

Phase 3:
    Add checkpoint storage and recomputation from checkpoint.

Phase 4:
    Add jump-distance classification and explicit policy decisions.

Phase 5:
    Add speculative refusal / cascade-drain handling if VapourSynth API4 behaviour
    and testing demonstrate it is useful and safe.

Phase 6:
    Tune eviction, cache clearing, and diagnostics based on real scheduler behaviour.
```

This cache-manager direction should be revisited in the next chat before coding. The user specifically requested that this interim cache specification be retained because it was already discussed and should not be lost.
...
The previous v0.7 recommended sequence included low-risk file splitting before cache-manager work. That structural work is now DONE.

Recommended next sequence from this v0.8 checkpoint:

```text
1. Confirm the current v0.8 source/build checkpoint is committed and pushed.
2. Confirm GitHub Actions passes with all current .cpp files:
       srcapoursynth-Cnr3.cpp
       src\cnr3_cache.cpp
       src\cnr3_response_tables.cpp
3. Optionally tag the current stable structural checkpoint.
4. Re-open the interim cache-manager specification in Section 22.
5. Review the draft cache-manager direction before coding.
6. Confirm/refine:
       per-instance ownership;
       recent output back-cache;
       checkpointing;
       jump-distance policy;
       cascade-drain behaviour;
       speculative refusal / not-available-yet handling;
       recompute/recovery;
       cache clearing and eviction;
       diagnostics;
       edge cases and failure policy.
7. Decide a phased cache-manager implementation plan.
8. Only then begin cache-manager implementation, one small tested phase at a time.
```

Do not begin functional cache-manager coding merely because the structural split is complete. The cache-manager design still needs explicit review and confirmation.

Suggested commit message for the v0.8 structural checkpoints already completed:
...
Please read CNR3_Handover_Snapshot_v0.8_structural_split_cache_direction.md and continue from there. The current implementation has real recursive chroma blending, shared luma buffers, vscnr2-style scene-change detection, VS2026 local build, verified GitHub Actions build, and two completed no-program-logic-change structural splits:

- common/cache split;
- response-table helper split.

Strict streaming still requires vspipe -r 1.

Next intended step: re-open and confirm the interim per-instance cache-manager direction recorded in Section 22: recent output back-cache, checkpointing, jump-distance assessment, speculative refusal / not-available-yet handling, cascade-drain behaviour, recovery/recompute, cache-clearing/eviction policy, and diagnostics. Do not start cache-manager coding until the design is explicitly confirmed. Please preserve the user's patch style: explain what changes and why, show prior/replacement blocks with enough context, avoid unnecessary changes, and keep comments maintainable.
```
