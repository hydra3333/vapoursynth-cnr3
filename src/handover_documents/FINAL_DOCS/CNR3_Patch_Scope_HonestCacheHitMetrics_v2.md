# CNR3 — PATCH SCOPE v2: HONEST CACHE-HIT METRICS — per-frame branch rows + true lookup hit-rate

**From:** designer/reviewer (W3D), via coordinator (W3X), to coder (W3C).
**Status (v2, 2026-07-09):** The v1 scope was IMPLEMENTED and the delivered patch DESIGNER-REVIEWED and APPROVED,
EXCEPT it omitted the derived `cache_lookup_misses` emission row (section 2.3). v2 promotes that row to an explicit
REQUIRED deliverable and records the review outcome. A resuming (memoryless) coder should: apply the delivered
patch `CMS07-DIAG_honest-cache-hit-metrics.patch`, ADD the missing misses row per section 2.3, run the canonical
4-way (R-PROCESS-26; expect 56/56 unchanged), and return the diff for review before the coordinator commits. Verify
every claim COLD against live `src/` (file:line); never trust the patch's own claims.
...
```
cache_lookup_queries_total     <value>     [DELIVERED — present in the approved patch]
cache_lookup_hits              <value>     [DELIVERED — present in the approved patch]
cache_lookup_misses            <queries - hits>     [*** OUTSTANDING — the ONE required addition in v2 ***]
```
*** OUTSTANDING ITEM (v2) — add the misses row immediately AFTER the cache_lookup_hits row in the D-SUM-04
emission (cnr3_cache_diagnostics.cpp), mirroring the existing write_uint64_row rows exactly. Derived, NO new
stored counter, underflow-guarded:
```cpp
cnr3_cache_diag_write_uint64_row(
    ...
    stats.cache_lookup_queries_total >= stats.cache_lookup_hits
        ? (stats.cache_lookup_queries_total - stats.cache_lookup_hits)
        : 0U);   // derived; the else-branch is unreachable if the two increments are placed correctly
                 // (query++ before the find, hit++ on the found branch) — reaching it signals a placement bug.
```
Do NOT add a third stored counter; do NOT modify the two increments in cnr3_cache_core.cpp (reviewed correct);
do NOT touch the three per-frame health rows or the hit-rate row (reviewed correct). This one row makes the
section 2.4 invariant a PRINTED line.
```
