...
branch-a/c/d (P.11C.3/.4/.5), and **live cache-pressure WIRING (hot-zone obs @arInitial, then prune) is the
next phase**, then first real-footage validation. The only
...
```text
CMS (design authority)     cnr3_cache_manager_design_v7_13.md   (CMS07.13; §0A charter; §9.5 materialized-floor invariant)
...
Document A                Document_A_CNR3_Project_Context_and_Standing_Rules_v3_5.md  (project context + standing rules)
Diagnostics spec          cnr3_diagnostics_specification_v1_5.md       (§2.8 = the temporary keystone KDT, removed post-K.1G)
Companion (non-normative)  CNR3_CMS_Future_Investigations_and_Open_Questions_v7_13.md   (FI-01..08; pairs with CMS in lockstep; NOT in the coder pack)
Role/Reviewer Handover    CNR3_Designer_Reviewer_Role_Handover_v1_10.md  (the role + disposition depth; charter §D0)
Current-state             Document_B_CNR3_Restart_Work_Plan_and_Current_State_v3_5_1.md  (top UPDATE block authoritative)
This-chat delta           CNR3_THIS_CHAT_DELTA_current_state_SLIMMED_v4_12.md   (companion to Document B; NEWEST state; phase-index + active-phase detail)
This document             CNR3_Handover_Introduction_to_new_reviewer_chat_v3_3.md
```
...
Next phase:                live cache-pressure WIRING — hot-zone observation (CMS §5.7: at arInitial) then pruning into
                           the live getFrame path. Prune/hot-zone LOGIC built+selftest-proven but ZERO live callers; the
                           live prune-TRIGGER contract needs a designer+coder review first (single-activation now; concurrent
                           prune revisited in fmParallel). THEN real-footage validation -> diagnostics (condensed 4-phase) ->
                           fmParallel. See the slimmed DELTA §4/§5 + owed-items.
```
