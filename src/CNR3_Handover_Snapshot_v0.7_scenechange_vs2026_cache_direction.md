# CNR3 Handover Snapshot v0.7 - Scene Change, Visual Studio 2026, GitHub Actions, Cache Direction
...
This v0.7 snapshot is intentionally detailed. Do not compress it aggressively in a future handover unless the relevant detail has been superseded by code or by a later explicit decision.

The major additions since v0.5 and v0.6 are:
...
```text
1. Load this v0.7 handover and the current source.
2. Confirm current Git status and latest commit.
3. Ensure scene-change/VS2026/cache-direction checkpoint is committed and pushed.
...
11. Re-open the interim cache-manager specification in Section 22.
12. Confirm/refine per-instance recent back-cache, checkpointing, jump-distance, cascade-drain, speculative-refusal, recompute, and cache-clearing policies.
13. Only then begin phased cache-manager implementation.
```
...
Please read CNR3_Handover_Snapshot_v0.7_scenechange_vs2026_cache_direction.md and continue from there. The current implementation has real recursive chroma blending, shared luma buffers, vscnr2-style scene-change detection, VS2026 local build, and verified GitHub Actions build. Strict streaming still requires vspipe -r 1.

Next intended step: review whether to checkpoint/tag, then plan low-risk file splitting. Before implementing cache-manager work, re-open and confirm the interim per-instance cache-manager direction recorded in Section 22: recent output back-cache, checkpointing, jump-distance assessment, speculative refusal / not-available-yet handling, cascade-drain behaviour, recovery/recompute, and cache-clearing policy. Please preserve the user's patch style: explain what changes and why, show prior/replacement blocks with enough context, avoid unnecessary changes, and keep comments maintainable.
```
