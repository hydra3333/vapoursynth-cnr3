...
**Version:** v4.8 (DELTA through D.2; CMS-GAP candidate RESOLVED into CMS07.12)
**Date:** 2026-06-26
...
```text
Committed/pushed through:  CMS07-D.2-live-exact-anchor-multihole-recovery-refusal-proof
...
## 6. OWED-ITEMS LEDGER (none blocking D.3)
...
- **TEST ARTIFACTS / GOLDEN PROVENANCE (housekeeping; add to repo alongside the existing harnesses).**
  ...
  regression bases for later phases (D.3+ must keep D.1 and D.2 green):
    * test_D1_once_only_harness_AB.vpy/.bat (D.1 single-hole) and test_D2_once_only_harness_AB.vpy/.bat
      (D.2 multi-hole RUN A + filled-hole cache-hit RUN B + bounded-window refusal RUN C + D.1 k==1
      regression RUN D + negative control RUN E + passthrough RUN F). Both reuse
      test_K1F_check_y4m_constant_plane.py.
    * D1_golden_chain_derivation.py and D2_golden_chain_derivation.py are the GOLDENS PROVENANCE:
      ...
      and self-check by reproducing the known K.1E.3 161/95 & 163/93. They are the "answer key with
      its working shown" that proves the harness goldens are independently correct, not circular.
