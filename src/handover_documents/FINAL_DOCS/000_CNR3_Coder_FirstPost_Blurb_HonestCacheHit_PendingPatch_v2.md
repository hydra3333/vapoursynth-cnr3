...
Attachments (latest versions), in read order:
  ...
  6. CNR3_Patch_Scope_HonestCacheHitMetrics_v2.md                  (the APPROVED patch's scope; your first task = its section 2.3 misses row)
 ...
 12. CMS07-DIAG_honest-cache-hit-metrics.patch                     (the APPROVED delivered diff — apply, then add the 2.3 misses row)
 13. CMS07-DIAG_honest-cache-hit-metrics-patch-notes.md            (predecessor's patch notes)
...
(scope: CNR3_Patch_Scope_HonestCacheHitMetrics_v2.md).

That patch HAS NOW BEEN DESIGNER-REVIEWED and is APPROVED, with ONE required addition before commit
(the predecessor chat omitted it when it hit the limit). So your FIRST CONCRETE TASK is well-defined:

  ADD the derived `cache_lookup_misses` row to the D-SUM-04 summary emission (scope section 2.3).
  It is DERIVED (no new counter): misses = cache_lookup_queries_total - cache_lookup_hits, with an
  underflow guard. Mirror the existing D-SUM-04 write_uint64_row rows exactly. Suggested form:
      ...
      cnr3_cache_diag_write_uint64_row(
          ...
          stats.cache_lookup_queries_total >= stats.cache_lookup_hits
              ? (stats.cache_lookup_queries_total - stats.cache_lookup_hits)
              : 0U);   // derived; the guard's else-branch would signal a hit/query placement bug

  Place it immediately after the cache_lookup_hits row. This makes the integrity invariant
  (cache_lookup_hits + cache_lookup_misses == cache_lookup_queries_total) a PRINTED line rather than
  mental arithmetic. Do NOT add a third stored counter; do NOT touch the two increments in
  cnr3_cache_core.cpp (they were reviewed correct). Then run the canonical 4-way (R-PROCESS-26);
  expect 56/56 UNCHANGED (this is additive observe-only emission). Report the diff for review before
  the coordinator commits.

The rest of the patch (three per-frame health rows pred_returned_from_cache_percent /
current_frame_returned_from_cache_percent / cache_hit_percent; the two D-SUM-04 counters
cache_lookup_queries_total + cache_lookup_hits at both lookup entry points; the
cache_lookup_hit_rate_percent health row; edit_version -> CMS07-DIAG.honest-cache-hit-metrics) was
reviewed CORRECT and is not to be changed. If you are asked to re-examine any part, verify claims cold
against live src/ (file:line), never from memory or the patch's own claims.

After that commits, forward work is the external ANALYSIS TRACK only (A1 plan-trace tool / A2 fmParallel
C1-under-race gate / A3 real 576p50) plus designer-driven L-series production-case testing; there are no
remaining in-plugin DIAG steps.

---

## Maintainer notes (NOT part of the pasted block)

VERSION POINTERS current as of this blurb (2026-07-09) — bump on each future handover.
