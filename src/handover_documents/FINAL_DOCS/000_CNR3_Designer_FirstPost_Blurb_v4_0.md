...
3. Then the parked reservation-table work (the real fix for the fmParallel predecessor-in-flight
   ...
   gating must all COMPOSE — separately or together.## WHERE WE ARE (precise)

**THE PLUGIN IS SHIPPABLE. The deliverable pipeline is complete.**

Committed marker: **`CMS07-RELEASE.production-config`** (the production/ship configuration).
Branch: **`main`** — `dev_cache_manager` has been MERGED and RETIRED. All work continues on main
(open a fresh feature branch for the next big arc).

**SHIP CONFIG (settled, triple-confirmed, W3X verbatim sign-off):**
`fmParallelRequests` + `CNR3_CACHE_PROFILE_HALF` (500/3 zones) + `CNR3_ENABLE_PLAN_RETRY_BIAS` **OFF**
= 337 fps null / 274 fps end-to-end encode (5.5x realtime), duplicates ~0, re-churn 0.
R-ARCH-08: plan-retry is valid ONLY under fmParallel (it throttles fmParallelRequests 2.7x).

**THE MASTER DIAGNOSTICS GATE (new, and it changes what a run looks like):**
`CNR3_DIAG_MASTER_PERMIT_DIAGS` in cnr3_build_config.h is the single production switch. Every
dev-instrumentation gate is INDIVIDUALLY wrapped in `#if defined(CNR3_DIAG_MASTER_PERMIT_DIAGS)`
(per-item, deliberately not one long region: the pattern is visible at every site so nothing can be
added outside it by accident; there is NO central #undef list to drift). Ship = master commented out:
all 15 D-SUM families, PLANTRACE, the keystone scaffolds, AND the startup provenance emission compile
out. Dev = uncomment ONE line and the whole instrumented world returns.
**CONSEQUENCE: a SHIP build's log is SILENT** — no `edit_version=`, no `filter_mode=`, no
`response_config:`, no D-SUM. W3X ruling: ship quiet like other DLLs. Those lines return in dev builds.
**CANONICAL SELFTEST COUNT IS CONFIG-DEPENDENT: 56/56 with the master OFF** (forced-fail 55/56 e1);
**57/57 with the master ON** (56 base + the PLANTRACE-gated diag3c2 test; forced-fail 56/57 e1).
A changed count must be LOCATED IN CODE, never hand-waved.

**THE USER SURFACE IS COMPLETE:** eleven descriptive options (y/u/v_threshold, y/u/v_strength,
y/u/v_curve, scene_threshold, scene_chroma), cnr2-equivalent defaults, strict validation, and
self-explanatory one-line errors that state what was received AND what is acceptable:
```
...
```
README.md is SHIPPED (option docs, cnr2->CNR3 equivalence table, progressive + interlaced usage with the
proven field-split helpers in Appendix A, and the technical/cache section).

**CI/RELEASE PIPELINE IS LIVE:** `.github/workflows/build-windows-x64-release.yml` — triggers on
workflow_dispatch and on Release creation; builds Release x64 with an explicit `cl` command mirroring the
vcxproj (a FLAG MAP comment ties every flag to its project setting); wildcard source list minus
`cnr3_cache_core_selftest_main.cpp` (cannot fossilize); hard-fails if `VapourSynthPluginInit2` is not
exported; emits BUILD_INFO.txt (marker/commit/UTC/sha256); zips [cnr3.dll, README_CNR3.md,
SHA256SUMS.txt, BUILD_INFO.txt]; PDB as a separate artifact; attaches the zip to the Release on a
release trigger. First workflow_dispatch run PASSED (40s, VS2026 Enterprise on windows-latest).
No selftest in CI by W3X decision (maintainer's responsibility).

## NEXT STEPS IN ORDER

1. **Cut the first Release** (if not already done): tag, create a GitHub Release, confirm the zip
   auto-attaches. The pipeline is proven bar this trigger path.
2. **RESERVATION TABLE — the last big architectural arc** (designed, parked, self-contained pickup doc:
   CNR3_PROPOSAL_Reservation_Table_v1_0.md; a coder-annotated v1_1 also exists in FINAL_DOCS).
   Two BIG steps by W3X ruling: **Step A** = planner selector + engine hookification + #error guards,
   proven by BYTE-IDENTITY; **Step B** = the reservation path (registry, intent-mark at arInitial, hard
   claim at compute-start, collapse-at-F, 10s loud-fail awaits, fetch avoidance), proven by BEHAVIOUR
   ...
   **Scope A's FIRST mandatory deliverable: cold-verify arInitial's actual pass/mutex structure** —
   the whole design rests on a recollection of it, and R-PROCESS-28 says recollection doesn't cut patches.
3. **Residual desaturation** (3/957 frames on hard-cut/flashing content) — users self-serve with
   `scene_chroma=True`. Revisit only if it proves to matter.
4. **A1 plan-trace tool / A3 real-footage campaign** — parked, unchanged.

After reading, confirm your understanding of: the committed state (CMS07-RELEASE.production-config on
main; the plugin is shippable and the CI pipeline is live); the ship config (fmParallelRequests + HALF-500
+ plan-retry OFF) and R-ARCH-08; the MASTER diagnostics gate and its consequence (ship logs are silent;
56/56 master-off vs 57/57 master-on, config-dependent); that the option surface and README are COMPLETE;
the parked reservation-table arc and its two-step structure; and the R-PROCESS rules, especially
gap-analysis-before-patch (R-PROCESS-31), the-patch-is-the-arbiter (R-PROCESS-32), and the quality-gate
rule for pixel changes (R-PROCESS-33). Then I will hand you the next artifact.
