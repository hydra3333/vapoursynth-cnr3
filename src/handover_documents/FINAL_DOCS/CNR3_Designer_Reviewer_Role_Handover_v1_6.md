...
**Version:** v1.6
**Date:** 2026-06-23
**Status:** Handover with the **keystone UNDER WAY**. v1.6 supersedes v1.5: the cache↔pixel /
...
**v7.8** (now contains **FI-04**), diagnostics **v1.5**, Document B **v3.2.9**, this document **v1.6**;
...
```
What K.1A–K.1D delivered (the keystone's opening — NEW in v1.6; the P.10A–P.11C and earlier
...
consumed-and-released, never transferred). Dependency declaration changes `rpStrictSpatial` →
...
KEYSTONE — cache<->pixel / getFrame integration  [K.1A-K.1D DONE; K.1E IN FLIGHT; rest OWED]:
  ...
  [ ] K.1E branch (c) predecessor-present frame-1 compute (IN FLIGHT, pre-patch): acquire cached
      ...
      predecessor (acquired=1/released=1/transferred=0/balance=0 — OPPOSITE tail to K.1B), store/
      return output[1]. rpStrictSpatial->rpGeneral (FI-04). REVIEW BARS: P.11B-call scope = thin
      exposure only, P.11C UNTOUCHED (watch hardest, per D16/Example F); ownership balance; KDT
...
— End of CNR3 Designer / Reviewer Role Handover v1.6
