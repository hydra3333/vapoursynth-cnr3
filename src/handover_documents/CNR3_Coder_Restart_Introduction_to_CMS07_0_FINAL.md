...
controlling design authority. It is the **final and complete controlling design for
this restart, unless explicitly revised**: all verify items
...
Being a new design/development, Phase/SubPhase numbering will restart. Propose a
new numbering/naming convention before coding, using the existing expanded
Phase/SubPhase style unless I approve a different convention.
...
**Do not copy old `.txt` code into new `.h`/`.cpp` files during this first
cache-core milestone unless I explicitly approve that specific salvage.** Salvage
is the second step (below), after the new core ownership/pinning/eviction
...
introduction, surface it as a CANDIDATE rule and clearly mark it as
remembered/prior-context-derived — it is not controlling until I confirm it.
