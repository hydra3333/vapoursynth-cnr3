# CNR3 A1 — Plan-Trace Log Analysis Tool — SPECIFICATION v0.1
...
**Status:** v0.1 seed. The log-format section is verified cold against `src/` at marker
`CMS07-DIAG.honest-cache-hit-metrics`. Two design decisions (§10) are left open for W3X to settle before
implementation.
...
Use `dataclasses` (frozen where practical) and `enum`. Stdlib only (see §10 open decision on pandas).
...
### 7.1 `query_arinitial_received_order`  (Dave (a); Q-B)
...
### 7.4 `query_open_holes_stats`  (Dave (d))
...
(recovery plans vs pred/cache-hit). This quantifies bubbling depth.

### 7.5 `query_recovery_hole_fill`  (Dave (e); feeds Q-A)
...
### 7.6 `query_execution_disorder_vs_dsum01`  (Q-B, explicit)
...
### 7.7 `query_lookup_accounting_reconcile`  (Q-A, explicit)
...
### 7.8 `query_lifecycle_spans`  (supporting)
...
**Non-goals (v0.1):** no plugin interaction; no live capture; no plotting (numbers/tables only — a plotting
layer can come later); does not re-derive frame pixels; not bound by any plugin proof discipline.

**Open decisions for W3X to settle before implementation:**
1. **Dependencies:** stdlib-only (portable, zero-install, recommended) vs allow `pandas` for the
   sort/group/histogram queries (faster to write, adds a dependency). Spec is written to be satisfiable
   stdlib-only.
2. **Scope of this first cut:** implement only Q-A + Q-B + Dave's five queries (7.1–7.7) now, and stub the
   banked Provenance queries (7.x tail) as named no-ops to be filled at A3/real-footage time — or build the
   full banked set up front. Recommend the former (answer the live disputes first, expand against real
   footage).
