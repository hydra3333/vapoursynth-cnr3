...
**Version:** v1.5
**Date:** 2026-06-20
**Status:** CLEAN-SEAM handover, **immediately before the keystone**. v1.5 supersedes v1.4: the
**entire pixel path is now proven on CALLER-SUPPLIED frames** — P.1A–P.6A (scalar pixel chain),
P.7A–P.9A (scalar→native bridge), and **P.10A (VapourSynth plane-view adapter), P.11A (caller-supplied
frame-triplet validation), P.11B (caller-supplied real-frame pixel composition), P.11C (caller-supplied
scene-change/reset)** — so this is a clean-seam handover with **no phase in flight**. The next work is
**the cache↔pixel / getFrame keystone — the hard designer gate** — where the proven cache core meets
the proven pixel chain and frame sourcing / lifecycle / ownership become load-bearing. It has not yet
...
designer/reviewer chat ends, a new chat (and Dave) can re-establish not just the factual state but the
*way the work has been done* — the review disciplines, the decision heuristics, and the reflexes that
produced the quality so far. **This refresh was made deliberately at the keystone's doorstep so a fresh
chat would inherit accurate state at the riskiest transition.**

**v1.5 change:** (1) State pointer advanced from P.9A to **P.11C done (45/45)**; roadmap marks
P.1A–P.11C done; in-flight marker "none — the cache↔pixel/getFrame keystone is next (hard gate)".
...
Latest committed phase:     CMS07-P.11C-caller-supplied-scene-change-reset-proof (thirteenth pixel phase)
...
R-PROCESS-19 culmination). **The ENTIRE PIXEL PATH is now proven on CALLER-SUPPLIED frames, on top
of the milestone: P.1A (response tables) → P.2A (config/geometry) → P.3A (int64 blend) → P.4A
(downsampled-luma) → P.5A (signed-difference bridge) → P.6A (chroma-plane traversal) → P.7A
(source-luma downsample traversal) → P.8A (native byte-plane access) → P.9A (native luma downsample
bridge) → P.10A (VapourSynth plane-view adapter) → P.11A (caller-supplied frame-triplet validation)
→ P.11B (caller-supplied real-frame pixel composition) → P.11C (caller-supplied scene-change/reset).**

Phase history (recent): ... → **C.14A (cache-core milestone)** → P.1A → P.2A → P.3A → P.4A → P.5A →
P.6A (chroma traversal) → P.7A → P.8A → P.9A (scalar→native bridge) → **P.10A** (VS plane adapter) →
**P.11A** (triplet validation) → **P.11B** (real-frame composition) → **P.11C** (scene-change/reset).

### MOST RECENT PHASE — P.11C (DONE); the NEXT work is the cache↔pixel/getFrame KEYSTONE (hard gate, not yet proposed)

The **entire pixel path is proven on caller-supplied frames.** Thirteen pixel phases are proven,
committed, and pushed; the most recent is **P.11C**:

```text
PHASE:    CMS07-P.11C — caller-supplied scene-change/reset proof
STATUS:   DONE. Committed and pushed. Baseline marker:
          CMS07-P.11C-caller-supplied-scene-change-reset-proof
RESULT:   Debug 45/45 exit 0; Release 45/45 exit 0; forced-fail 44/45 exit 1; verbose 45/45.
          No D-SUM change (no macro-off run needed).
IN FLIGHT NOW:  NONE. The next work is THE CACHE<->PIXEL / getFrame KEYSTONE — the HARD DESIGNER
          GATE. Send proposal at minimum; proposal-plus-read-first preferred; do NOT apply without
          designer review. It connects the proven CACHE CORE (C.14A) to the proven PIXEL CHAIN
          (P.11C) inside VS getFrame scheduling. See Part 6B for the keystone hunting list.
NOTE:     The pixel path is fully proven on CALLER-SUPPLIED frames (P.11A/P.11B/P.11C), so the
          keystone reasons ONLY about frame SOURCING / lifecycle / ownership over an already-proven
          pixel path. Scene-change was deliberately placed BEFORE the keystone (it is a pixel
          decision), keeping the keystone's surface purely lifecycle/sourcing.
```

What P.10A–P.11C delivered (the real-frame pixel path on caller-supplied frames — added since v1.4;
the P.7A–P.9A and P.1A–P.6A summaries follow below and are unchanged):
...
P.11C caller-supplied scene-change / reset proof                  <-- DONE (committed)
      ** P.6A is chroma-plane traversal, NOT the original "scene-change." P.7A–P.9A built the
         scalar→native bridge; P.10A–P.11C built the real-frame pixel path on CALLER-SUPPLIED
         frames (adapter → triplet validation → composition → scene-change). The ENTIRE pixel
         path is COMPLETE on caller-supplied frames at P.11C. cnr3_frame_processing.cpp is now
         in BOTH the selftest and cnr3 plugin projects. **
--- complete through P.11C on caller-supplied frames; below is the KEYSTONE + post-keystone ---
KEYSTONE  CACHE<->PIXEL / getFrame integration — THE HARD DESIGNER GATE (proposal-plus-read-first;
        do NOT apply without designer review). Connects the proven CACHE CORE (C.14A) to the proven
        PIXEL CHAIN (P.11C) inside VS getFrame scheduling. Expected split: arInitial request planning;
        arAllFramesReady retrieval + release; predecessor acquisition from cache/recovery ONLY (NEVER
        source[N-1], R-ARCH-06); pixel processing via the proven P.11B/P.11C caller-supplied helpers;
        store + return-transfer ownership; fallback/error cleanup. Load-bearing: VS-LIFECYCLE-01;
        cache-lookup addref released/transferred exactly once; source-frame refs released on every
        path; checkpoint/recovery pin balance; AS1-AS7 + lock disciplines live again; Category-B
        developer-alert (CMS §9.6.4) is the EMISSION half of what C.13B DETECTS (hard status →
        setFilterError + bounded one-shot stderr OUTSIDE locks; Category-A duplicate/adopt silent).
...
Document B v3.2.8              Document_B_..._Current_State_v3_2_8.md  — current build state + work plan (P.11C proven)
Companion v7.3                CNR3_CMS_Future_Investigations_..._v7_3.md — NON-NORMATIVE, NOT in coder pack
This document v1.5            CNR3_Designer_Reviewer_Role_Handover_v1_5.md — the role/disposition handover
```
...
## PART 6B — THE PIXEL-ARC REVIEW CHECKLIST (P.3A–P.11C done; the getFrame/cache KEYSTONE is live)
...
================  LIVE HUNTING LIST — the cache↔pixel / getFrame KEYSTONE (NOT yet done)  ================

This is the hard designer gate. The pixel path is fully proven on caller-supplied frames, so the
keystone is about frame SOURCING / lifecycle / ownership over an already-proven pixel path.

KEYSTONE — getFrame / cache integration:
  [ ] predecessor acquisition from cache/recovery ONLY — uses the proven caller-supplied helpers
      (P.11B/P.11C) with the previous filtered OUTPUT from cache; NEVER source[N-1] (R-ARCH-06).
      This is the keystone's single most important property.
  [ ] VS-LIFECYCLE-01: every source frame retrieved in arAllFramesReady was requested in arInitial
      of the SAME activation. arInitial does request planning; arAllFramesReady does retrieval.
  ...
  [ ] Policy A strict next_needed boundaries: frame 0 (no prev_output), seeks / random access, and
      the LAST frame of a clip.
