...
*** v4.0 STATE + METHOD ADVANCE (2026-07-18) — READ FIRST; supersedes all older notes below where they differ ***
...
### METHOD ADDITIONS PROVEN THIS CYCLE (fold into how you work)

11. **AN ENRICHED, SELF-CONTAINED SCOPE IS WORTH THE EFFORT.** The rider scope was rewritten as a 10-part
    document (verified code facts with file:line, 8 named TRAPs, the due-process wrapper, the proof gate,
    "what done looks like"). A fresh memoryless coder implemented it flawlessly AND caught three designer
    defects. The traps were the load-bearing part: each was a real hazard found by cold inspection, and
    the one that mattered most (TRAP 1 — `scene_chroma` delegates to the int parser) was found only
    because the designer re-read the source instead of trusting the earlier scope.
12. **THE CONFIRM PASS CAN AND SHOULD OVERTURN THE DESIGNER.** D1/D2/D3: `%g` defeated its own stated
    rationale (verify numerically, not by reading); `%.32s` was an out-of-bounds READ on user-controlled
    data (the coder proved it with AddressSanitizer, and noticed the parser's OWN matcher documents that
    map data may be non-NUL-terminated); raw echo contradicted the one-line requirement. **Two of the
    three would have SHIPPED A DEFECT if the coder had complied.** When a coder pushes back with probe
    evidence, that is the process working — rule on it, don't defend the scope.
13. **HARNESS DETOURS ARE THE SLOWEST PART OF A GATE.** The 17/17 run took six attempts, none of them
    about the plugin: a mangled copy-paste of the .bat, a wrong vspipe path (exit 9009 = command not
    found), an over-generous FAIL_ANY criterion that scored launch failures as passes, and finally the
    .vpy having been RECYCLED during a folder tidy. **The log file is the arbiter every time** — one
    `type <log>` ended it. Do not theorise about a failing harness; read its output.
14. **THE BINDING LAYER PRECEDES THE PLUGIN.** VapourSynth's Python `typedDictToMap` rejects int/float/
    bool TYPE mismatches before the plugin's create runs (`rangeToIntFilter` ValueError), so the plugin's
    own int/float/bool type-error branches are unreachable from Python and remain as C-API defence.
    Data-typed params (the curves) DO reach the plugin. Gate criteria must expect either layer.
15. **VERIFY BY CONTENT, NOT BY IDE.** VS2026 needed a restart before it acknowledged externally-edited
    project settings — `findstr /n "NOMINMAX" cnr3.vcxproj` was the arbiter, not the dialog.

*** end v4.0 STATE + METHOD ADVANCE note ***


**Version:** v3.0 (2026-07-15) — CURRENCY ADVANCE over v2.0. The ROLE/METHOD content below (PART 1, PART 3,
PART 4, PART 5, and especially PART 10 — the operating playbook) remains CORRECT and is the reason this
process works; read it in full. This note adds what the last session proved, and PART 2's pointer is
superseded by the v3.0 state block here.

*** v3.0 STATE + METHOD ADVANCE (2026-07-15) — READ FIRST ***

**COMMITTED: `CMS07-FEATURE.cnr2-descriptive-option-parser`.** Canonical 4-way **57/57** (config-dependent:
56 base + the PLANTRACE-gated diag3c2 test; forced-fail 56/57 e1). CMS UNCHANGED at 07.15.
**SHIP CONFIG:** fmParallelRequests + CNR3_CACHE_PROFILE_HALF (500/3 zones) + CNR3_ENABLE_PLAN_RETRY_BIAS OFF.
**IN FLIGHT:** CNR3_RIDER.option-error-messages — scope approved, NO PATCH (coder chat degraded at its limit).

### METHOD ADDITIONS PROVEN LAST SESSION (fold these into how you work)

1. **GAP ANALYSIS BEFORE PATCH.** For surface/parameter work, require the coder to deliver — before any diff —
   a per-item table (parse / default / validate / apply, each EXISTS-with-file:line / MISSING / PARTIAL) plus
   definitive answers to the named unknowns, ending in a FINAL PATCH SHAPE statement. Then: **the diff must
   map one-to-one onto the reported gaps; diff content with no corresponding gap is a review finding.** This
   worked outstandingly — it front-loaded the investigation, killed three unknowns cheaply (curve already
   wired; threshold-zero already safe; scene_chroma plumbing-only), and made the review mechanical.

2. **THE PATCH IS THE ARBITER, NOT ITS NOTES.** The coder's notes were wrong about their own patch THREE
   times in one round (described the wrong debt; listed a README the patch did not contain; called a
   standalone patch a "delta"). An apply-test settles such questions in seconds. Never reason about a patch
   from its description when you can apply it to a copy of the real tree and look.

3. **DIFF BEFORE DIAGNOSING.** When an apply fails, DIFF the target file against the patch's expected context
   FIRST. I twice asserted a cause (CRLF; a 3-line offset) without diffing; both were wrong, and the real
   cause (the coordinator's committed tree carried deliberate experiment edits the coder's reconstructed
   baseline lacked) fell out of the diff instantly. Theorising before diffing cost hours.

4. **VERIFY THE MARKER LANDED — AFTER the commit.** CMS07-FIX.operational-response-defaults shipped WITHOUT
   its marker bump: its logs under-claimed (showed the prior snapshot marker while containing the fix) for a
   whole commit, because the FIX's proof instrument was a chroma analyser that never surfaces edit_version.
   RULE: every proof gate must surface the marker somewhere, whatever the patch's primary instrument is.

5. **STRUCTURAL GATES CANNOT SEE QUALITY.** The chroma desaturation (K.1E.2 proof placeholders live in the
   shipping path) passed every gate we own — selftests, byte-identical, ownership balances — for many commits,
   because those gates compare placeholder-build against placeholder-build. It took W3X's eyeball on a
   stackhorizontal to catch brown going grey. A Python chroma-delta analyser now exists as a standing
   QUALITY gate: **any pixel-affecting change gets the 10-frame analyser + a stackhorizontal look.** Treat it
   as the third gate class: selftests gate structure, byte-identical gates inertness, the analyser gates the
   picture.

6. **A "TEMPORARY" PLACEHOLDER IN A LIVE PATH IS A DEFECT WITH A TIMER ON IT.** K.1E.2's 255/255 defaults were
   honest, documented, and deferred — and outlived their welcome by months because every subsequent arc was
   correctly disciplined about not touching proven code. When you see a deferral comment in a live path,
   record it as an owed item somewhere that decays loudly.

7. **INSTRUMENTS CLASSIFY; FOOTAGE ARBITRATES.** The chroma analyser labelled frame 8 a "chroma-inclusive
   scene candidate" and both the coder and I read the label as the phenomenon. W3X looked at the frames:
   there was no scene change. The label was a symptom of the same over-blending, not an independent cut. When
   a tool's category and the primary evidence disagree, the evidence wins.

8. **THE COORDINATOR'S SKEPTICISM IS A PRIMARY INSTRUMENT.** Every significant correction last session started
   with W3X questioning a number: "bigger cache should help, not hurt"; "who knows what ms"; "the FULL run
   also says 500?"; "this patch is smaller on disk"; "with fmParallelRequests there is no benefit to
   plan-retry?" — that last one overturned my own "leave it on, it's free" advice and exposed a 2.7x
   throttle. Do not defend a position against W3X's instinct; go and measure.

9. **CODER-CHAT DEGRADATION IS A KNOWN FAILURE MODE.** Both coder chats so far degraded near their limits
   (mangled run instructions; an inline unvalidated "patch" then retracted). Watch for: self-contradiction
   about their own deliverables, inability to produce files, drifting from the scope. Escalate scrutiny and
   recommend a fresh chat early.

10. **NEVER `git stash`** (standing project rule — I violated this last session by suggesting it for a
    reference build; W3X's route was safer). Use `git switch -c wip-name`, or `git checkout -- <files>` plus
    re-applying the patch file.

*** end v3.0 note ***


**Version:** v2.0 (2026-07-12) — CURRENCY ADVANCE over v1.17. The ROLE/METHOD content below (PART 1, PART 3,
PART 4, PART 5, and especially PART 10 — the operating playbook: confirm-before-patch, verify-cold against
source at file:line, whole-patch deletion scan, verify-invocation-not-just-definition, caller-map
re-derivation, R-PROCESS-24/25/26, harness ownership) is CURRENT and UNCHANGED — read it in full; it is why
this project works. Only the STATE pointers were stale. Current state as of v2.0:

>> THE IN-PLUGIN DIAGNOSTICS ARC IS CLOSED. An ANALYSIS/INSTRUMENTATION SUB-ARC then ran to make the cache's
behaviour under out-of-order requests fully legible. FOUR patches committed this session, all through the full
gate (canonical 4-way 56/56; R-PROCESS-19 macro-off byte-identical; L1/L2 oracles; CMS UNCHANGED at 07.15): <<
  1. CMS07-DIAG.intent-counted-lookups  — intent counting replaces uniform lookup counting; old L1 oracle
     (66.664) retired; new L1 exact 7279/7279/0 = 100.000.
  2. CMS07-DIAG.lookup-site-breakdown   — 11 per-site D-SUM-04 counters + legend + purpose lines + print-only
     self-check (per-site sums == merged totals; never wired to selftest).
...
IN FLIGHT (reviewed-APPROVED, NOT committed — HELD): CMS07-SCAFFOLD.filter-mode-selector. Compile-time mode
selector at the top of cnr3_build_config.h (uncomment one of fmUnordered/fmParallelRequests/fmParallel;
exactly-one #error guard; mode suffixed onto CNR3_EDIT_VERSION -> marker reads `...filter-mode-selector:
fmUnordered`; `filter_mode=<token>` provenance line at filter creation). Default fmUnordered = identical
behaviour. 4-way PASS; byte-identical satisfied BY INSPECTION (both change sites traced cold — token resolves
to the same fmUnordered, the fprintf is stderr-only with no pixel reach). HELD pending the marker fix below.

OPEN THREAD (why the commit is held): a RUN-LOG MARKER REGRESSION. The plugin used to print its edit_version/
CMS07 marker to the RUN LOG; it no longer does. Verified cold: CNR3_EDIT_VERSION appears in EXACTLY ONE place
in the committed tree (the selftest summary) — no plugin-run-path emission, no orphaned scaffolding — and it
was already absent at the earliest available snapshot (honest-cache-hit-metrics). So the regression PREDATES
this arc and is NOT caused by any of the four patches. It went unnoticed because the proof gate checks the
SELFTEST and FRAME BYTES but never asserts RUN-LOG EMISSION content. Task in progress: bisect older GitHub
trees (verify each by MARKER, not filename) to pin the removing commit and capture the exact original emission
line; then a small scoped patch restores the run-path emission (home: cnr3_create_filter, beside the
filter_mode= line) AND adds an emission-presence check to the gate. The selector commits with/after it.

FORWARD: pin+restore marker -> commit selector -> sample runs (200-frame TINY-100 baseline; shuffled variant;
first fmParallel whirl — proof runs NORMAL, experiment runs TINY) -> A1 plan-trace tool (spec v0.4 handoff-
ready; Q-B = reconstruct true order from ticks and explain D-SUM-01 out_of_order=0, the ORIGINAL driving
question) -> A2 (fmParallel churn = C1-under-race gate) -> A3 (real 576p50 via A1).

CURRENT DOC SET (authoritative versions): DELTA v5.0 / Document B v4.0 / Document A v4.0 / Provenance v2.0 /
FI v8.0 / Taxonomy Findings v06 / A1 spec v0.4 / Reviewer Intro v4.0 / CMS design v7.15 (UNCHANGED). The
repository, Document B, and the THIS-CHAT DELTA remain authoritative for build state over any wording below.
STANDING: caller-map re-derivation is the highest-yield check on counter patches; objective backstops
(4-way + byte-identical) do NOT cover run-log emission content (the marker lesson) — emission checks are now
a REQUIRED gate addition; verify every uploaded src.zip by its marker on arrival.

Everything from the "**Version:** v1.17 ..." banner down is retained as history and for the role/method content.


**Version:** v1.17 (2026-07-09) — CURRENCY ADVANCE over v1.16. The ROLE/METHOD content below (esp. PART 10 —
the operating playbook, confirm-before-patch, verify-cold, whole-patch deletion scan, verify-invocation,
R-PROCESS-24/25, harness ownership) is CURRENT and unchanged — read it in full; it is why this project works.
Only the STATE pointers were stale (v1.16 froze at the marshalling-optimisation arc). Current state as of v1.17:

>> THE MARSHALLING ARC AND THE ENTIRE IN-PLUGIN DIAGNOSTICS ARC ARE BOTH COMPLETE AND COMMITTED. <<
The DIAGNOSTICS arc ran the marshalling arc's successor: DIAG.1 -> 2a -> 2b -> 3a -> 3b -> 3c.1 -> 3c.2 -> 4,
CLOSED at DIAG.4 (memory D-SUM-02 + arc-close). All 14 D-SUM families are live + [DSUM-PLANTRACE] plan-trace +
[DSUM-HEALTH] derived-ratios block. Since arc-close, two ANALYSIS-TRACK diagnostics also committed
(prune-rechurn recency-gate; derived-health-ratios). Latest commit marker CMS07-DIAG.derived-health-ratios;
selftest 56/56 (forced-fail 55/56 exit 1); NORMAL baseline; CMS DESIGN UNCHANGED at 07.15.

IN FLIGHT: CMS07-DIAG.honest-cache-hit-metrics (fixes a misleading health ratio surfaced by L-series
production-case testing; adds true cache-lookup hit/miss/query counters). Delivered by the prior (now
limit-reached) coder chat, designer-APPROVED bar one derived misses row; a NEW ChatGPT coder chat is producing
the replacement patch. Scope authority: CNR3_Patch_Scope_HonestCacheHitMetrics_v3.md.

FORWARD (post-commit): external ANALYSIS TRACK only — L1noR/L2noR threading runs, then B-series as needed, then
A1 (plan-trace analysis tool) / A2 (fmParallel = C1-under-race gate) / A3 (real 576p50 via A1). NO in-plugin DIAG
steps remain.

CURRENT DOC SET (authoritative versions): DELTA v4.30 / Document B v3.20 / Document A v3.14 (R-PROCESS register
incl. R-PROCESS-26 canonical 4-way) / Provenance v1.8 / Condensed Plan v1.10 / FI v7.18 / Reviewer Intro v3.11 /
Coder Intro v6.10 / CMS design v7.15. The repository, Document B, and the THIS-CHAT DELTA remain authoritative
for build state over any wording below. STANDING: evaluate coder responses against known-good baselines (a prior
coder chat twice mangled run instructions -> R-PROCESS-26); the .vpy/.bat harnesses are the DESIGNER's deliverable.

Everything from the "**Version:** v1.16 ..." banner down is retained as history and for the role/method content.
