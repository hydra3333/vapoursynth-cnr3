# CNR3 — THIS-CHAT DELTA: current-state companion (SLIMMED, through marshalling levers 0A/0B/3a.1/3b.1 + validation policy + 3a.2) — v4.19

**Version:** v4.19 (SLIMMED). Supersedes v4.18. Extends the marshalling-optimisation arc with the typed-pointer levers and the validation policy. All steps value-preserving (56/56 four-way, P-series unchanged) and profiled; CMS UNCHANGED (still 07.15 — implementation optimisation only, no design/invariant change). Cumulative vs pre-lever baseline: **93,914 -> ~42,400 samples (~-55%)**.
...
```text
Committed/pushed through:  CNR3-OPT-Lever3a2-hoist-source-range-check  (marshalling arc: AVX2 + 0A + 0B + 3a.1 + 3b.1 + 3a.2)
...
Marshalling arc status:    0A -28% | 0B flat | 3a.1 -37% | 3b.1 flat | 3a.2 flat ON 8-BIT (confirmed YUV420P8). cumulative ~-55% (93,914 -> ~41,850). PRODUCTION REALITY: the vast majority of VHS/analogue captures are 8-BIT, so the 8-bit path IS the production hot path. 3a.2's vectorised loop is the 16-BIT path (secondary/edge case, unmeasured). On 8-bit, 3a.1 already took the compute (call-elimination) win; what remains in the 8-bit unpack is the COPY ITSELF (native byte -> int buffer -> read back) + per-frame allocation. VERDICT: further 8-bit marshalling wins require attacking the COPY/ALLOCATION, not the arithmetic -> the real 8-bit levers are (a) buffer ELIMINATION (native->native / buffer-free / pass fusion so bytes are touched fewer times), (b) allocation POOLING (kill the per-frame resize/alloc churn, ~1,500-2,200 samples/run). 16-bit SIMD is a real-but-secondary open question (profile a P16 clip when convenient). 3c (blend) ahead regardless.
