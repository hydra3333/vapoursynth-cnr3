# CNR3 — THIS-CHAT DELTA: current-state companion (SLIMMED, DIAG.3c.2 COMMITTED; DIAG.3c plan-trace family CLOSED; DIAG.4 memory is the last in-plugin step) — v4.29
...
```text
Committed/pushed through:  CNR3-OPT-LeverE-scenechange-local-accumulator  (marshalling arc COMPLETE: AVX2 + 0A + 0B + 3a.1 + 3b.1 + 3a.2 + A-lite + C1 + Repack + F/3c + Staging + E)
...
Marshalling arc status:    COMPLETE at ~-80%. 0A -28% | 0B flat | 3a.1 -37% | 3b.1 flat | 3a.2 flat-on-8bit | A-lite -7.4% | C1 -24.5% | Repack -4% | F/3c -15.7% | Staging -17.5% | E -4%. cumulative ~-80% (93,914 -> ~18,660). Tier-2 chroma fusion DECLINED (Path C, scene-detection-coupled); Lever D DECLINED (PATH-B-only). Remaining: Lever B (pooling ~587, needs fmUnordered lifetime proof) is the only headroom — coordinator call vs pivoting to parked diagnostics.
