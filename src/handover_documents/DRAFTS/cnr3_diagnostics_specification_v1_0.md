# CNR3 Diagnostics Specification v1.0

**Status:** Draft v1.0 for review and iteration, separate numbering convention to CMS
**Date:** 2026-06-15  
**Design epoch:** CMS07.0 restart  
**Requested filename:** `cnr3_diagnostics_specification_v1_0.md`

Filename note: the filename preserves the requested spelling. The document text uses
"diagnostics".
...
are confirmed in the implementation layout, but the pattern is:

```cpp
// ---------------------------------------------------------------------------------------------
// NOTE:    Comment out one or both 'if defined' lines to
...
Note: re the zero-row suppression vs the many "_store_failures: 0 → FAIL if >0" fields: 
...
Note that all columns always line up and
all values within columns also always vertically align.
...
IMPORTANT NOTE:
1.  In 2.3 above, there is provision for a comment block description.
...
Not that this table uses delta_1 / delta_2_to_5 / delta_negative / delta_0_or_duplicate,
...
D-SUM-04 example lists a pin_list_balance row in the table but the field-definitions
list has pin_list_records / pin_list_discharges without explicitly defining
pin_list_balance as records - discharges. Trivial — worth one line defining it,
parallel to how pin_balance and lookup_ref_balance are defined,
so the example field has a definition
...
store_failures:
    Store attempts that failed due to error.
...
- store failures must be explained.
```
...
```text
FAIL:
    store_failures > 0 unless explicitly handled and documented.
...
????? seeking clarification:
D-SUM-08 has the same latent ambiguity as D-SUM-14.
This is the kind of consistency point worth catching now:
D-SUM-08 (cache store / first-in-best-dressed) has store_failures > 0 → FAIL with
the same potential confusion. 
It's slightly better guarded there ("unless explicitly handled and documented," 
and it separates duplicate_skipped_already_cached as its own counter), 
so it's probably fine — but the word "store_failures" carries the identical 
risk of a coder conflating error-failure with race-loss. 
we are nailing the definition in D-SUM-14, consider appling the
same error-vs-duplicate-skip distinction to D-SUM-08's store_failures
so the two summaries use the term identically.
Consistency across the catalogue is exactly what stops a future reader (or coder)
misreading one by analogy with the other.
...
scene_change_checkpoint_store_successes:
    Scene-change checkpoint stores that succeeded.

scene_change_checkpoint_store_failures:
    Scene-change checkpoint stores that failed.
...
CNR3 scene-reset: instance=1, summary (frames_processed=200)
  ...
  scene_change_checkpoint_store_failures        0
...
???????? seeking clarification:
this lists scene_change_checkpoint_store_failures > 0 as FAIL — good — but a scene-cut
frame is also subject to normal first-in-best-dressed
(a duplicate store losing the race is not a failure, per §9.3).
So "store failure" needs a precise definition that distinguishes
a genuine store error (FAIL) from first-in-best-dressed losing the
race (normal, INFO). As written, a future coder could miscount a
benign race-loss as a FAIL. 
It doesn't yet distinguish two (even three) completely different events:
1. First-in-best-dressed race-loss — a duplicate store arrives,
    the frame is already cached, the duplicate is correctly discarded.
    Per §9.3 this is normal, especially under out-of-order/parallel paths. 
    Not a failure.
2. A genuine store error — the store path tried to store and something
    went wrong (allocation failure, integrity breach, ownership imbalance).
    That is a FAIL.
3. There's actually a third category specific to scene-cuts that neither
    of the above covers, and it's the one that genuinely matters for D-SUM-14;
    A cut frame that should have become a checkpoint but didn't get the flag set.
    This is the real integration failure D-SUM-14 exists to catch:
    pixel layer detected a cut, the frame got stored, but the checkpoint flag was not applied.
    That's not a "store failure" at all (the store succeeded) - it's a promotion failure.
    And it's already correctly captured by the other FAIL line
    ("eligible reset frames are not checkpoint-promoted once integration is active").
Worth at elast a clarification in that field's definition.
This is the kind of precise-definition point that matters.
Perhaps consider something like:
-   scene_change_checkpoint_store_errors (renamed from _failures) — genuine store errors
    only (allocation, integrity, ownership). FAIL if > 0.
-   scene_change_checkpoint_store_duplicate_skips — cut-frame stores that lost
    first-in-best-dressed (frame already present).
    INFO, expected under stress — explicitly not a failure.
-   The existing promotion-mismatch FAIL line stays as-is (eligible-reset-not-promoted),
    since that's the actual checkpoint-integration gate
and maybe 1-liner
    "A store 'error' is a genuine failure of the store operation
    (allocation, integrity, ownership). A first-in-best-dressed
    duplicate skip — the cut frame's output was already cached
    and the duplicate was correctly discarded (§9.3) — is NOT a
    store error and is counted separately as INFO."

NOTE: it appears that D-SUM-08 has the same latent ambiguity.
