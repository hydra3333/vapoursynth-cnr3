# CNR3 vs cnr2 — option/default mapping + option-parsing spec — v6

(v6 = v5 + coder editorial cleanups (version alignment, retired-mode wording, parity phrasing, emission
sentence). v5 = full reconciliation pass per coder response 4: cnr2-exact names are REFERENCE-ONLY everywhere; the
...
1. **Option-parsing patch** (the core follow-up): add the ELEVEN CNR3 descriptive params per the
   ...
   per split rule), with cnr2-identical defaults and validation-throw ranges. Extend the response_config emission to print the resolved values. This gives cnr3 functional/default parity with cnr2 while using the W3X descriptive surface.
   - PRE-REQUISITE within this patch: resolve the curve-wiring open item (does Cnr3ResponseCurveKind actually
     select j*j vs j?). If not wired, wire it — otherwise y_curve/u_curve/v_curve parsing is a silent no-op.
