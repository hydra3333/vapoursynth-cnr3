...
**Version:** v02 — adds Item 6 (locking granularity) CLOSED and the bounded-prune
scheme (Section 8 rewritten from framing to decision; sizing starting-points
added). v01 covered foundation, Position B, pinning, arInitial/arAllFramesReady
atomics, the new hot-zone scheme, checkpoints, Q1/Q2.
...
1. **Item 6 — locking granularity: CLOSED** (Section 8). One cache-wide lock held
   minimally; bounded prune (K=8 start); fine-grain deferred to measured. Listed
   here only to mark it done.
2. **Item 3 (residual) — hot-zone SIZING under fmParallel:** zone width, decay
   margin, MAX_HOT_ZONES, vs thread spread. Entangled with item 7. OPEN.
3. **Item 7 — parameter-coherence / derived sizing:** cache capacity, BACK/FORWARD
   ...
   meaningless; search window wider than BACK_RADIUS can't find what it needs.) OPEN.
