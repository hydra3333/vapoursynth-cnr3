# A2 — first findings: fmParallel, plan-retry, HALF-500, and the mode decision (v3.0, 2026-07-15)
...
---
## HALF-500 cache profile RESULT (2026-07-14, true 1000-vs-500 comparison)

Committed CMS07-FEATURE.half-cache-profile-and-retry-rename (gated HALF profile: ceiling 500 + 3 hot zones,
CR4-preserving; rename EXPERIMENT->ENABLE_PLAN_RETRY_BIAS; default SLEEP_MS=50). Gate items 1-7 passed.

CACHE-SIZE SWEEP (3000-frame fmParallel, plan-retry on S=50, distinct markers TEST-NORMAL vs TEST-HALF):

  metric                                    NORMAL-1000   HALF-500
  active_ceiling                            1000          500
  frames_recently_evicted_then_re_requested 0             0        <- DECISIVE: neither re-churns
  frames_re_requested_repeatedly            0             0
  slot_count_max                            1101          551      (HALF ~half memory, as intended)
  frames_evicted                            1904          2456     (HALF evicts ~29% more -- all COLD)
  duplicates_seen                           2174          2625     (HALF +21%; in-flight race, not eviction)
  recovery_span_mean                        1.75          1.89
  fps                                       125.5         126.4    <- IDENTICAL throughput
  invariants / plan_balance / x             0/0/0         0/0/0

VERDICT: conjecture "can 500 do as good a job as 1000?" CONFIRMED. Re-churn = 0 at BOTH sizes: the extra
eviction at 500 is entirely cold frames (nothing evicted came back), so the working set fits in 500. fps
identical. Correctness intact. HALF-500 halves cache memory footprint for no measurable throughput cost --
suitable as a low-memory profile for end-user PCs across a range of AVX2 machines.
Minor: HALF shows ~21% more duplicates (in-flight predecessor race, slightly likelier with less headroom),
but wasted COMPUTE not wrong output, small, and already largely handled by the plan-retry mitigation.
Note: this workload ran low-contention (span ~1.8, dup ~2000) vs the plan-retry sweep clip (span ~23) --
the 500-vs-1000 comparison is valid (same workload both sides); it simply is not the heavy-stress scenario.

STATUS: HALF-500 arc COMPLETE and validated. Reproducible across 3 prior HALF runs + this controlled A/B.
Reservation table remains the designed real fix for the fmParallel in-flight duplicate race (parked).

---
## CRITICAL FINDING: plan-retry is ACTIVELY HARMFUL to non-fmParallel modes (2026-07-14)

While selecting the ship mode (HALF-500), tested fmParallelRequests with plan-retry ON vs OFF:

  fmParallelRequests + HALF     plan-retry ON      plan-retry OFF
  fps                           122-124            337          <- 2.7x FASTER with it OFF
  frames_computed               ~3130              3001
  duplicates_seen               119-139            1            <- CLEANER too
  re-churn / invariants / x     0/0/0              0/0/0

ROOT CAUSE: plan-retry dumps hole-heavy plans and sleeps 50ms betting a predecessor will publish during
the sleep. Under fmParallel that works (predecessor genuinely in-flight -> adoption). Under
fmParallelRequests compute is SERIAL, so the missing predecessors are NOT in-flight-about-to-arrive --
the sleep waits for things that never come. Evidence (fmParallelRequests, plan-retry ON):
plans_dumped_total=8903, retry_sleeps_total=8903, plans_kept_on_attempt_3plus=2993 (nearly all plans hit
the retry cap), dumped_plan_holes 136043 vs kept_plan_holes 4852 (holes did NOT shrink on re-plan --
throttle-vs-adopt guard fires NEGATIVE). The 8903 useless 50ms sleeps throttle the mode to ~1/3 speed.

RULE: CNR3_ENABLE_PLAN_RETRY_BIAS must ONLY be enabled when compiling CNR3_FILTER_MODE_PARALLEL. It
actively throttles fmParallelRequests (~3x) and fmUnordered. Recommend a config comment to that effect,
and consider a compile-time guard (#error if plan-retry enabled without fmParallel) as a future safety.

SHIP DECISION (revised, evidence-backed): fmParallelRequests + HALF-500 + plan-retry OFF.
  337 fps, 1 duplicate, re-churn 0, correct. 3.5x faster than fmUnordered (95fps), 2.7x faster than
  fmParallel (126fps), and as clean as fmUnordered. Dominant on every axis. This is the PyPI ship config.
  (fmParallel + plan-retry remains the path the reservation table will later improve; not the ship mode.)
