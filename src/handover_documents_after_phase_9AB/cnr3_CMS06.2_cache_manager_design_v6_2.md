...
**Supersedes:** CMS06.2 supersedes CMS06.1, CMS06, CMS05.2b, CMS05.2, CMS05.1, CMS05, CMS04, CMS03, CMS02, CMS01
...
**Current state:** Phases CMS02-A through CMS02-E are complete. CMS02-G
proof sub-phases G.7A through G.9AB are complete (all proof paths disabled
in normal committed state). See Section 14 for exact completion status.
...
**Status: Not started.** Note: CMS02-G proof scaffolding (G.7–G.9AB)
has proceeded ahead of CMS02-F as disabled proof-only skeleton work.
CMS02-F full implementation remains a prerequisite before CMS02-G
becomes output-authoritative.
...
| CMS02-G.8D | Complete | Pre-store decision/walk proof showed current frame `would_compute=1`; disabled. |
| CMS02-G.9AB | Complete | Local source-frame-set acquire/release proof passed; disabled state restored. |
| CMS02-G.10ABC | **Next** | Dry-run compute orchestration only — see below. |
| CMS02-G.10D+ | Planned | First actual local recovered-frame computation — see below. |
...
#### Phase CMS02-G.10ABC — Dry-run compute orchestration skeleton
**Status: Next recommended phase after CMS02-G.9AB.**
...
#### Phase CMS02-G.10D — First actual local recovered-frame computation
**Status: Planned. Precondition not yet satisfied.**

Purpose: compute recovered frames locally, then immediately release them.
Prove the processing boundary works without affecting strict-streaming state.

**Hard precondition before G.10D begins:**
See Section 13.4 — a safe processing boundary must exist that accepts an
explicit predecessor VSFrame* without depending on or mutating
`d->old_strict_cache.prev_output` or `d->old_strict_cache.next_needed`.
...
The review must verify that the resulting execution paths follow CMS06.2,
...
**Status: Open design requirement. Must be satisfied before CMS02-G.10D.**
...
### 13.7 Comment/label drift — do not mix with G.10ABC
...
**Rule:** Do not combine broad CMS05/CMS06 wording cleanup with CMS02-G.10ABC.
...
Handle wording cleanup as a separate dedicated pass after G.10ABC is complete.
...
| CMS02-F (cache-hit reuse) | Not started | Proof scaffolding of G preceded F; F full impl still required |
| CMS02-G.7A (source-request-plan skeleton) | Complete | Proof disabled in committed state |
| CMS02-G.7B (lifecycle proof) | Complete | Proof disabled in committed state |
| CMS02-G.7C (widened source-request proof) | Complete | Proof disabled in committed state |
| CMS02-G.8A (decision/walk skeleton) | Complete | Proof disabled in committed state |
| CMS02-G.8B (checkpoint selection/ref/rollover) | Complete | Proof disabled in committed state |
| CMS02-G.8C (pre-store probe) | Complete | Proof disabled in committed state |
| CMS02-G.8D (would_compute detection) | Complete | Proof disabled in committed state |
| CMS02-G.9AB (source-frame-set acquire/release) | Complete | Proof disabled in committed state |
| CMS02-G.10ABC (dry-run compute orchestration) | **Next** | No real frames; no output authority change |
| CMS02-G.10D+ (first actual recovery computation) | Planned | Precondition: safe processing boundary (Section 13.4) |
...
**Output authority:** The old strict-streaming cache (`old_strict_cache`)
remains the source of truth for all returned output frames. Recovery
scaffolding is proof-only and disabled in normal committed state. Do not
describe recovery as implemented until recovered outputs are computed,
stored, returned, and proven.
...
- `cnr3_output_cache_find_frame_and_add_ref()` lookup helper
  (Phase CMS02-F — full implementation, not proof scaffolding).
- Actual recovered-frame computation (Phase CMS02-G.10D+). Proof
  scaffolding through G.9AB is complete but all proof paths disabled.
...
- Caller-side lookup ref balance counters (not yet needed — no
  caller-owned lookup references issued yet).
- Safe processing boundary for recovery (Section 13.4 — open design
  requirement; must be resolved before G.10D).
...
source of truth for all returned output frames. The output cache stores
and prunes for proving only. Recovery scaffolding (G.7–G.9AB) is
disabled in the normal committed state and does not affect returned output.

This changes when Phase CMS02-F is complete and the output cache begins
serving cache hits, followed by CMS02-G recovery walks becoming
output-authoritative. Both require their Design Compliance Reviews before
any change to output authority.
