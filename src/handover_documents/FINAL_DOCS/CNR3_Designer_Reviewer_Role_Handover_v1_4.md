...
**Version:** v1.4
**Date:** 2026-06-20
**Status:** CLEAN-SEAM handover. v1.4 supersedes v1.3: the **scalar pixel pipeline AND the native
byte-buffer access/bridge layer are now proven, committed, and pushed** — P.1A–P.6A (scalar pixel
chain) plus **P.7A (source-luma downsample traversal), P.8A (native byte-plane access), P.9A (native
luma downsample bridge)** — so this is again a clean-seam handover with **no phase in flight**. The
next work crosses into **real VapourSynth machinery** (real frame-memory adaptation, scene-change,
getFrame/cache integration where the proven cache core meets the proven pixel chain) and has not yet
been proposed (see Part 2). This is the companion to Document B: **Document B says WHAT is done; this
document says HOW the design/review role is performed.** It exists so that if the current
designer/reviewer chat ends, a new chat (and Dave) can re-establish not just the factual state but
the *way the work has been done* — the review disciplines, the decision heuristics, and the reflexes
that produced the quality so far.

**v1.4 change:** (1) State pointer advanced from P.6A to **P.9A done (41/41)**; roadmap marks
P.1A–P.9A done; in-flight marker "none — real frame-memory / scene-change / getFrame-cache
integration next". (2) Part 2 now summarises P.7A–P.9A and the key proven properties (the
`x*storage_bytes` two-byte column addressing; the memcpy-not-reinterpret_cast choice; the
8-bit→1-byte / 9..16-bit→2-byte storage boundary; the native-endian = little-endian = VS-matching
note). (3) Part 6B checklist: the byte-plane/stride items are now marked DONE-and-proven; the live
hunting list is re-pointed at real frame-memory adaptation (getReadPtr/getWritePtr/getStride, real
per-plane dims, typed-row-pointer-vs-memcpy decision), scene-change, and getFrame/cache integration.
No discipline (D1–D15), trigger, worked example, accuracy rule, or recorded divergence was changed.
...
Latest committed phase:     CMS07-P.9A-native-luma-downsample-bridge-proof (ninth pixel phase)
...
R-PROCESS-19 culmination). **The scalar pixel decision pipeline AND the native byte-buffer
access/bridge layer are now proven on top of the milestone: P.1A (response tables) → P.2A
(config/geometry) → P.3A (int64 blend) → P.4A (downsampled-luma) → P.5A (signed-difference bridge)
→ P.6A (chroma-plane traversal) → P.7A (source-luma downsample traversal) → P.8A (native byte-plane
access) → P.9A (native luma downsample bridge).**

Phase history (recent): ... → **C.13B** → **C.14A (cache-core milestone)** → **P.1A** (response
tables) → **P.2A** (config) → **P.3A** (blend) → **P.4A** (downsampled-luma) → **P.5A** (signed
bridge) → **P.6A** (chroma traversal) → **P.7A** (source-luma downsample) → **P.8A** (native
byte-plane access) → **P.9A** (native luma downsample bridge).

### MOST RECENT PHASE — P.9A (DONE) and the NEXT work (real frame-memory / VS integration, not yet proposed)

The **scalar pixel pipeline and the native byte-buffer access/bridge are complete.** Nine pixel
phases are proven, committed, and pushed; the most recent is **P.9A**:

```text
PHASE:    CMS07-P.9A — native luma downsample bridge proof
STATUS:   DONE. Committed and pushed (git ab37443). Baseline marker:
          CMS07-P.9A-native-luma-downsample-bridge-proof
RESULT:   Debug 41/41 exit 0; Release 41/41 exit 0; forced-fail 40/41 exit 1; verbose 41/41.
          No D-SUM change (no macro-off run needed).
IN FLIGHT NOW:  NONE. The next work crosses into REAL VapourSynth machinery: real frame-memory
          adaptation (getReadPtr/getWritePtr/getStride, real per-plane dims, typed-row-pointer
          vs memcpy decision), scene-change accumulation, and getFrame/source-lifecycle/cache
          integration. Not yet proposed. Review as text first (Part 1 cadence). See Part 6B for
          the live hunting list.
NOTE:     P.6A is chroma-plane traversal (NOT the original roadmap's "scene-change," still deferred).
          P.7A–P.9A built the scalar→native bridge. The roadmap below is updated through P.9A.
```

What P.7A–P.9A delivered (the scalar→native bridge — added since v1.3; the P.1A–P.6A summary
follows below and is unchanged):
...
P.9A  native luma downsample bridge proof                         <-- DONE (committed, ab37443)
      ** P.6A is chroma-plane traversal, NOT the original "scene-change." P.7A–P.9A built the
         scalar→native bridge. The scalar pixel pipeline AND native byte-buffer access/bridge
         are COMPLETE at P.9A. Remaining phases cross into real VapourSynth machinery. **
--- complete through P.9A; below crosses into REAL VapourSynth frame memory + integration ---
NEXT  REAL VAPOURSYNTH FRAME-MEMORY adaptation: wire the P.8A native byte-plane layer to actual
        VS frame memory — getReadPtr/getWritePtr/getStride, real per-plane dims (getFrameWidth/
        getFrameHeight, plane-dependent due to subsampling), and the typed-row-pointer
        (reinterpret_cast<const uint16_t*>) vs P.8A-memcpy decision against the real frame contract
        and its alignment guarantees. Validate real subsampled source dims here (P.7A ceil was
        harness-only). P.8A/P.9A proved the byte-stride LOGIC on synthetic buffers; this binds it.
NEXT  SCENE-CHANGE accumulation (the deferred original "P.6A"): diff_total during the chroma pass;
        over threshold → return SOURCE UNCHANGED, no recursive blend.
VS.x  VapourSynth getFrame lifecycle (arInitial/arAllFramesReady), source request/retrieve,
        return-transfer — where the proven CACHE CORE (C.14A) connects to the proven PIXEL CHAIN
        (P.9A). Predecessor = explicit previous FILTERED output from the cache, NEVER source[N-1].
...
OP    confirm cnr3_frame_processing.cpp is a member of the cnr3 PLUGIN project (it is in the
        selftest project from P.3A) BEFORE the real-frame-memory phase — via VS2026 GUI Add Existing
...
Document B v3.2.7              Document_B_..._Current_State_v3_2_7.md  — current build state + work plan (P.9A proven)
Companion v7.3                CNR3_CMS_Future_Investigations_..._v7_3.md — NON-NORMATIVE, NOT in coder pack
This document v1.4            CNR3_Designer_Reviewer_Role_Handover_v1_4.md — the role/disposition handover
```
...
## PART 6B — THE PIXEL-ARC REVIEW CHECKLIST (P.3A–P.9A done; real-frame / VS phases live)
...
REAL VAPOURSYNTH FRAME-MEMORY ADAPTATION (bind the proven P.8A byte layer to real frames):
  [ ] real frame access via getReadPtr/getWritePtr/getStride; real per-plane dimensions via
      getFrameWidth/getFrameHeight (plane-dependent due to subsampling) — NOT the P.7A harness ceil.
      Validate real subsampled source dims at this layer.
  [ ] the typed-row-pointer decision: reinterpret_cast<const uint16_t*> on REAL frame memory vs the
      P.8A memcpy approach — decide against the actual VS frame-memory ALIGNMENT contract. If typed
      pointers are used, stride must be a multiple of 2 (assert/confirm); P.8A's memcpy is the safe
      fallback if alignment is not guaranteed. P.8A/P.9A proved the byte-stride LOGIC on synthetic
      buffers; this binds it to real frames where alignment actually matters.
...
VS INTEGRATION (where the cache core meets the pixel chain):
  ...
  [ ] cnr3_frame_processing.cpp confirmed a member of the cnr3 PLUGIN project (not just selftest)
      — load-bearing at the real-frame-memory phase (the first to need it in the plugin build).
