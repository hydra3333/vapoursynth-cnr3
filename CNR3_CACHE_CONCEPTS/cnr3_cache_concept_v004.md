...
pool slots. See Section B.15 for the checkpoint pruning edge case that makes pin_count
...
- `{releases mutex}`
- `{sets mutex}`
- Decrement `pin_count` on the checkpoint pool slot that was pinned in arInitial
- `{releases mutex}`
...
**Bookkeeping note — passing the chosen checkpoint frame number from arInitial to arAllFramesReady:**

VapourSynth provides a `frameData` pointer parameter to both the arInitial and
arAllFramesReady callbacks for exactly this purpose. CNR3 currently ignores this with
`(void)frameData`. To carry the chosen checkpoint frame number through, arInitial stores
the frame number into `*frameData` (cast to an appropriate integer type) and
arAllFramesReady reads it back. This is the correct VapourSynth-sanctioned mechanism for
per-invocation state that must survive the gap between the two callbacks.
...
// at the end of the matching arAllFramesReady. The pruning pass skips any
// slot where pin_count > 0. See Section B.15.
...
3.  compute hole fills in ascending frame order using retrieved source frames
    and cached output frames as the chain checkpoint
...
- Per-slot `pin_count` on each checkpoint pool slot is required — see Section B.15
...
**At arAllFramesReady completion, under mutex, after all computation and pruning:**
- Read the chosen checkpoint frame number from `frameData`
- Decrement the `pin_count` on that checkpoint pool slot
- The slot is now available for normal eviction by future pruning passes
...
- A `pin_count` that fails to reach zero (due to a bug in error handling paths) would
  ...
  `pin_count` values at teardown and warn if any are non-zero.
...
// CKPT_ANCHOR      checkpoint selected as computation checkpoint for this invocation
