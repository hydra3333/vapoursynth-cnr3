...
**Version:** v1.8
**Date:** 2026-06-25
**Status:** Handover with the **keystone NEARLY COMPLETE**. v1.8 advances the v1.7 state pointers:
the cache↔pixel / getFrame keystone is now **committed through K.1F**, the live getFrame dispatch
handles **all four branches except recovery** (present-N cache-hit, fresh-start, predecessor-present
all live and proven), selftest count is **49/49**, controlling CMS is **CMS07.10**, and the next
phase is **branch-(d) D.1 recovery**. v1.8 is a state-pointer + version-table refresh only: the role
