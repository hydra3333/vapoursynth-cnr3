...
NEW rules added during review (CONFIRM):   R-PROCESS-10 (stderr), R-PROCESS-11 (diag
                                           formatting), R-PROCESS-12 (compile-time
                                           gating), R-PROCESS-13 (no print/long work in
                                           atomics)
CONFIRMED (was candidate; user upgraded):  R-CAND-01 (ASCII-only), R-CAND-02 (minimise
                                           unrelated changes)
MERGED (no longer separate):               R-CAND-03 -> R-PROCESS-11
                                           R-CAND-04 -> R-PROCESS-12 (Part B)
                                           R-CAND-05 -> R-PROCESS-04
```

**Resolved overlaps (your queries):**
- Diagnostic formatting: R-CAND-03 (compact/one-line) folded into R-PROCESS-11, which now
  states both the compact-detail allowance and the human-readable-summary requirement.
- Compile-time gating: R-CAND-04 (no correctness behind disabled guards) folded into
  R-PROCESS-12 as Part B; the gating mechanism is `#if defined` for BOTH the compute gate
  and the print gate (decision: compute and print may live in separate places, so
  preprocessor exclusion is required for each — `if constexpr` is unsuitable for a
  separated print gate).
- The duplicate rule number is resolved: the gating rule is now R-PROCESS-12 (stderr stays
  R-PROCESS-10), giving a clean R-PROCESS-10/11/12/13 run.

**Most important items:** R-ARCH-01 (was unsafe; rewritten with fmParallel guard and the
warm-up concept removed), R-PACK-07 (was conflated; restated as pack-is-a-snapshot), and
the new R-PROCESS-12 (compile-time diagnostics gating, the mechanism for safe
enable/disable of diagnostics without affecting correctness). The systemic fixes —
version-neutral "latest prevailing CMS", no lossy re-summarising, and pack-vs-spec
clarity — are applied throughout and called out in the general comment.
