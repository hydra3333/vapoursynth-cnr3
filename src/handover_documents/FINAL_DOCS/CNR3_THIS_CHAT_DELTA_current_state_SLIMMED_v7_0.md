...
**Version:** v7.0 (2026-07-18). Supersedes v6.0. **The plugin is SHIPPABLE.** Committed marker at this
DELTA: **CMS07-RELEASE.production-config**, on branch **main** (dev_cache_manager merged and retired).

*** v7.0 DELTA (2026-07-18) — the ship cycle ***

### 1. The option surface is COMPLETE and self-explanatory
Eleven descriptive options, cnr2-equivalent defaults, strict validation, and errors that state what was
received AND what is acceptable — one line, inside `detail[128]`/`message[256]`:
```
...
```
Expectation text built ONCE per kind, shared by both paths (anti-drift). Float via `std::to_chars`
shortest round-trip. Curve echo: `value_size`-bounded byte loop (NO libc scan), sanitised 0x20..0x7E,
truncated at 32 with an in-quotes `...`.
**17/17 runtime gate PASSED**, discharging the parser's deferred items 8 and 9 (`y_threshold=0` SUCCEEDS
-> `y=0/192/wide`; `u_curve="o"` correctly REJECTED). Frames byte-identical (595,336,353, fc /b clean).

### 2. THE MASTER DIAGNOSTICS GATE (the biggest structural change this cycle)
`CNR3_DIAG_MASTER_PERMIT_DIAGS` — one line, ship = commented out. EVERY dev-instrumentation gate is
INDIVIDUALLY wrapped (15 D-SUM families, PLANTRACE, keystone scaffolds, startup provenance).
**W3X's design rulings, in order, each better than the draft:** (1) a central `#undef` list -> REJECTED,
"will too easily get out of sync"; (2) one long wrapping region -> REJECTED, "a maintainer could put
something outside a single long hard-to-ascertain #ifdef"; (3) per-item wraps -> ADOPTED, the pattern is
visible at every site. Master at the TOP above filter-mode selection. Provenance gated like a diag
(ship quiet like other DLLs). Dead K1E3 scaffold trebly commented AND wrapped.
**Counts: 56/56 master-OFF (ship), 57/57 master-ON (dev).** Verified by preprocessing both ways: OFF
defines 0 diag/provenance macros; ON restores all 15 computes.

### 3. Build hygiene + headers
NOMINMAX in both vcxproj configs and the CI cl line (windows.h min/max macros vs ~20 std::min/max uses;
`cnr3_memory_diagnostics.cpp` includes windows.h and was safe only by include-order luck; its own defines
now `#ifndef`-guarded, clearing the C4005 the project-wide define introduced). Vendored headers refreshed
to the **R78 release** set + VERSION.txt. **`VS_USE_LATEST_API` deliberately NOT defined** — CNR3 compiles
at API 4.0 (max compatibility); declaring 4.2 would refuse older cores for zero gain. CNR3 is
**property-transparent**: every output frame is `copyFrame(source)`, so `_Range`/`_FieldBased`/etc flow
through regardless of compiled API level (verified cold).

### 4. README shipped (PQ-2 closed)
Correct 11-option surface with per-parameter behavioural docs, the cnr2->CNR3 equivalence table
(incl. cnr2's permissive-'x' quirk as history), progressive + interlaced usage using the PROVEN
`split_into_fields`/`reweave_fields` helpers (Appendix A, verbatim from the harness + one added
`SetFieldBased` line, marked), the metadata-poor `.avi` warning, the errors section, and a
mechanism-by-mechanism cache/technical section (wavefront -> bounds/eviction -> checkpoints -> recovery
-> hot zones -> pins -> measured results).

### 5. CI/release pipeline live (first dispatch run PASSED, 40s)
Explicit `cl` command with a FLAG MAP comment tying every flag to its vcxproj setting (W3X ruling:
visibility over msbuild). Wildcard `src\*.cpp` MINUS `cnr3_cache_core_selftest_main.cpp` — cannot
fossilize (note `cnr3_cache_core_selftest.cpp` IS in the DLL). Security kept (`/sdl`,`/GS`);
`/fp:precise` explicit; `/OPT:NOICF` deliberate. Hard-fails without `VapourSynthPluginInit2`.
BUILD_INFO.txt = marker/commit/ref/UTC/sha256. Zip = [cnr3.dll, README_CNR3.md, SHA256SUMS.txt,
BUILD_INFO.txt]; PDB separate artifact; zip attaches to Releases automatically.
Runner: `windows-latest` = WS2025 + VS2026 Enterprise v145 (June 2026 image migration).

### KEY FACTS A NEW CHAT MUST NOT RE-DERIVE (carried forward + new)
- **Ship = master OFF = SILENT log + 56/56.** Dev = master ON = provenance + D-SUM + 57/57.
- **Every new diagnostic gate gets its own individual master wrap** (R-BUILD-01 containment rule).
- **API stays 4.0**; property-transparency means downstream sees everything anyway (R-BUILD-02).
- **Error domains:** mapSetError=creation, setFilterError=frame, stderr=diagnostics (R-PROCESS-34).
- **VS's Python binding rejects int/float/bool TYPE mismatches before the plugin runs**; data-typed
  params (curves) reach the plugin. Gate criteria must accept either layer.
- **Y is NOT filtered** (luma-change guard gating chroma via cross-plane weighting). **threshold==0 is
  VALID** (centre-only). **scene_chroma=false is cnr2 parity**; True is the flashing-lights knob.
- **Ship config gates are the product** (PyPI DLL); never change them as a side effect.
