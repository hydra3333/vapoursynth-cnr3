# CNR3 vs cnr2 — option/default mapping + option-parsing spec — v4

(v4 incorporates the coder scope review: threshold-zero policy, bit-depth policy, curve-wiring proof form,
...
8. **IN-CODE documentation is a deliverable, not a nicety (W3X requirement).** At the option-parsing site
   in vapoursynth-Cnr3.cpp, a comment block MUST carry BOTH:
   (a) the full cnr2->cnr3 name-equivalence table (every param: cnr2 name, cnr3 name, default, range) —
       so a maintainer reading the parser never needs the README to know what maps to what; and
   (b) a condensed version of the plain-English option explanations (one or two lines per param: what it
       does and the effect of raising/lowering it), including the special semantics: y_threshold gates
       chroma via LUMA change (it does not filter luma); curve wide-vs-narrow = the j*j vs j cosine shape;
       threshold==0 special case; scene_chroma's chroma-only-cut use case.
   The parser comment block and the README table must agree; the proof gate includes checking both exist
   and match. Rationale: the K.1E.2 regression survived precisely because intent lived nowhere near the
   code — the defaults said 255 and nothing beside them said why that was wrong.

## Proof gate for the option-parsing patch (coder list, adopted; names updated)

1. Debug+Release x64 build; canonical 4-way selftest (count unchanged).
2. Grep/source proof all new options are parsed from create-time input.
3. Default .vpy (no options): response_config identical to the committed interim line.
4. Explicit-defaults .vpy (all params = defaults): response_config IDENTICAL to (3).
5. Non-default .vpy: response_config reflects the supplied values.
6. Invalid-option tests all throw with clear messages: scene_threshold <0 / >100; any threshold/strength
   <0 / >255; curve string not "wide"/"narrow".
7. Threshold-zero proof: y_threshold=0 (and u/v) must not crash/NaN; table centre-only behaviour verified.
8. Curve proof: wide/narrow per plane takes effect (via the wiring evidence of amendment 4 plus a run pair).
9. Output sanity: default-run output vs the committed interim build is byte-identical (same defaults, same
   tables — parsing must not perturb the no-args path).
10. Documentation proof: the parser-site comment block contains the full cnr2->cnr3 equivalence table AND
    the condensed per-param explanations, consistent with the README text (amendment 8).
