...
6. **Experiment counters (gated under the same macro, observe-only, count-never-compute).** Emit a
   ...
   Print-only self-checks (never wired to selftest): kept buckets sum to (attempt1+2+3plus);
   retry_sleeps_total == plans_dumped_total; attempts == kept + dumped. WARN only on failure.
