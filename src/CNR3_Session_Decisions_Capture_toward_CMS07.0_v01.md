...
## 8. Locking / atomicity — framing (item still OPEN; this is the setup)
...
named places, not scattered invariants). But the difficulty concentrates in the
locking design.
...
**8.3 Inside the lock:** only safety-guaranteeing in-memory operations — search,
pin, hole-catalogue, store, pin-record, final unpin, eviction-predicate read.
Indivisible commits.
**Outside the lock:** all slow work — pixel computation, VapourSynth source
requests. So compute and I/O never serialise threads through the cache mutex.

**8.4 OPEN — item 6, the keystone:** ONE cache-wide lock (simple, correct, but
serialises all threads through the cache — possibly murderous for 12-thread
scaling) vs FINER-GRAINED locks per region/zone (scales better, but lock-ordering
→ deadlock risk, and the "one consistent snapshot" property gets harder across
lock boundaries). This choice likely DOMINATES fmParallel performance and
determines the exact atomic boundaries CMS07.0 must specify. MUST be settled
before writing CMS07.0. **DO NOT LOSE THIS QUESTION.**

**8.5 Mutex-scoping audit (genuine-gap-candidate, for the coder):** enumerate
every cache operation that checks-then-acts and confirm none drops the lock
between check and act under fmParallel. Designer to FRAME this for the coder as
"what level of atomicity does each scenario need," covering at least:
search-identify-and-reserve; store-on-compute-complete; full pruning pass;
hot-zone slide; checkpoint establish; reservation/pin release — PLUS a broad
open ask for the coder to identify scenarios the designer has NOT enumerated,
given the redefined caching goal.

---

## 9. Still-parked / OPEN items

1. **Item 6 — locking granularity** (Section 8.4). KEYSTONE. Settle before CMS07.0.
2. **Item 3 (residual) — hot-zone SIZING under fmParallel:** zone width, decay
   margin, MAX_HOT_ZONES, vs thread spread. Entangled with item 7.
3. **Item 7 — parameter-coherence / derived sizing:** cache capacity, BACK/FORWARD
   radius, JUMP_THRESHOLD, CHECKPOINT_INTERVAL/MAX/MIN_RETAIN, decay margin,
   MAX_HOT_ZONES, thread count — currently set independently; several are
   interdependent and should be coherently derived. (e.g. zone wider than cache is
   meaningless; search window wider than BACK_RADIUS can't find what it needs.)
