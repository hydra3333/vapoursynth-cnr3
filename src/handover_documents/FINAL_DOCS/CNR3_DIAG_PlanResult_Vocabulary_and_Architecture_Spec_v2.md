# CNR3 — PLAN/RESULT DIAGNOSTIC (D-SUM PLAN-TRACE) — VOCABULARY + ARCHITECTURE SPEC v2

**From:** designer/reviewer (W3D), via coordinator (W3X).
**Status:** CONTROLLING DOCUMENT for the DIAG.3c scope. Supersedes SPEC v1 (which was a DRAFT for coder
cross-check). The cross-check has happened (`CNR3_DIAG3a_PlanResult_Cross_Check_Report_2026-07-04.md`);
its findings are folded in here as RESOLVED. Do NOT re-commission the cross-check.
...
**Bail-site count: >50, specifically 66 as of the cross-check snapshot. [finding (c)]**
```text
cnr3_arInitial.cpp        14 sites   (v1's "~50" OMITTED these)
cnr3_arAllFramesReady.cpp 51 sites   (v1's "~50" was this file only)
vapoursynth-Cnr3.cpp       1 site    (top-level getFrame state; also omitted by v1)
--------------------------------------
total                     66 sites
```
Confirmed against committed source (per-file `cnr3_set_filter_error` counts: 14 / 51 / 1). These counts are a
snapshot; the authoritative site-to-category TABLE is built at 3c.2 scope time against then-current source
(see §12). "~50" in v1 was AR-side only and is corrected here.
...
- THE INVASIVE PART (3c.2): capturing R-item X (not_reached) and E (error_here) and the FAILED reason
  requires the 66 bail sites (§4) to WRITE their outcome/reason before the existing `return nullptr`. This is
...
6. R-PROCESS-21/25: the 66 bail-site writes are ADDITIVE only; the proven getFrame paths are otherwise
...
3c.2  DUMP-ON-BAIL + E/X + FAILURE-REASON BAIL-SITE WRITES  — INVASIVE, R-PROCESS-21/25
  In scope:
    ...
    - the SITE-TO-CATEGORY TABLE (66 sites: arInitial 14 / AR 51 / top-level 1), built from live source at
      scope time — the FOUNDATION ARTIFACT for this half
    - the per-site additive writes before each existing `return nullptr`
  Touches the 66 bail sites (additive only). Its own R-PROCESS-21/25 review + the §10 items 4-6 proof gate.
```
...
(c) +3 CATEGORIES, >50 SITES         RESOLVED §4 Set 5. 13 -> 16 (ALLOCATION_FAILED,
                                     ...
                                     Bail-site count corrected: 66 (14 + 51 + 1), not "~50" (AR-only).
