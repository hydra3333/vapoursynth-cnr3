...
CMS07.7 is an ADDITIVE consolidation over CMS07.3: it adds §9.7 (the keystone
predecessor-sourcing decision, collecting existing rules from §4.8, §6.3-§6.6, §9.1-§9.3,
§9.5-§9.5.1, §9.6, and §9A.7 into one place from the keystone-writer's view); no design
rule, decision, constant, or section number is changed or removed (see §14 changelog).
...
`_v7.7.md`). It records deferred design questions and open tuning investigations. It is
...
AS3  reused-frame pin during ascending fill (Section 9.2/9.5), as encountered:
       ...
       STATUS NOTE (see §9.6): AS3 is RESERVED but DEFERRED from the current minimal
       recovery path. Under the currently proven nearest-present-start-point +
       contiguous-hole planner, no AS3-positive reused-intermediate state is reachable
       (a present frame between start point and N would have become the start point;
       an absent one is a planned hole consumed by AS2). AS3 must not be implemented
       against an unreachable synthetic plan shape; it awaits a future sparse-plan /
       recompute-avoidance planner revision.
...
**9.6.2 AS3 is reserved but deferred.** Because no reused-intermediate-distinct-from-
start-point-and-holes state is reachable under §9.6.1, the AS3 atomic (§8.7 "reused-frame
pin during ascending fill") has no reachable trigger in the current path and is DEFERRED.
AS3 must not be implemented against an unreachable synthetic plan shape, and the planner
must not be extended to sparse reused-intermediate frames, without a future, separately
approved sparse-plan / recompute-avoidance CMS revision. Until then, recovery execution
consists ONLY of:
```text
- start-point / anchor pin-record from AS1 (proven: H.2A);
- contiguous planned holes consumed through AS2 (proven: H.3A);
...
**CMS07.7 (2026-06-21) — additive consolidation; no rule/decision/constant/section-number change.**
