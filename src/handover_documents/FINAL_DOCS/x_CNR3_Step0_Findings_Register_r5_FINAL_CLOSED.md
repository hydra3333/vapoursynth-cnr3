# CNR3 STEP 0 — Findings Register (Round 5 C confirmation appended)
...
## HANDOFF SUMMARY — r5 -> X/D (read this first)

> **Round 5 (coder) closeout summary for coordinator/designer.** C confirms both r4 asks:
> (1) AGREE with X ruling SR-C-04 = option **(B)**; (2) AGREE with the full r4 findings-status table.
> No new SR-C findings were raised. No code was produced.
>
> **ARCHITECTURE CORRECTION CARRIED FORWARD:** there is **NO checkpoint pool/container**. The cache is
> one unified `slots_` vector; checkpoint is an `is_checkpoint` FLAG on a slot plus a flagged-position index
> and retention counts. All "checkpoint pool" wording in older comments is stale terminology only.
>
> **R5 CONFIRMATIONS APPENDED:**
> - `#0042 | C | r5 | agree`: C agrees with X ruling **SR-C-04 = (B)**. Add an independent
>   checkpoint-flag-count retention trigger (`checkpoint_count > CHECKPOINT_MAX_RETAIN`) inside the same
>   combined helper prune step. No single-activation safety objection, provided it uses the same locked
>   retire-before-select / select-detach / free-after-unlock discipline and honours pins, hot zones, frame 0,
>   K-bounded selection, and the unified-slot `is_checkpoint` flag.
> - `#0043 | C | r5 | agree`: C agrees with every one-line finding status from r4. No disputed statuses.
>
> **FULL FINDINGS STATUS AFTER R5:**
> ```
> SR-D-01 BLOCKER AGREED    Prune trigger in a NEW combined locked helper, not inside store_owned_frame_locked.
> SR-D-02 BLOCKER AGREED    Lifecycle split: AS2 consumer inputs pin-protected; target/frame0 outputs hot-zone-protected.
> SR-D-03 MAJOR   AGREED    Hot-zone observation wired at arInitial, frame number only.
> SR-D-04 MAJOR   RESOLVED  Single-activation now; concurrent -> fmParallel; all mutation under cache_mutex_.
> SR-D-05 MAJOR   RESOLVED  By X ruling B: independent checkpoint-flag-count trigger to be added; CMS 6.3 unchanged.
> SR-D-06 MINOR   RESOLVED  Non-checkpoint capacity trigger is self-debouncing.
> SR-D-07 MAJOR   RESOLVED  Six-step combined-helper order ratified and buildable.
> SR-C-01 BLOCKER AGREED    Combined-helper shape accepted.
> SR-C-02 BLOCKER AGREED    Target/frame0 output[N] protected by arInitial hot-zone observation, not pin.
> SR-C-03 MAJOR   AGREED    Conservative once-computed cached-output byte estimate.
> SR-C-04 MAJOR   RESOLVED  By X ruling B and C confirmation #0042.
> SR-C-05 MAJOR   AGREED    Lazy retire-before-select inside prune.
> SR-C-06 MINOR   AGREED    Temporary KDT for live wiring proof.
> ```
>
> **EXIT CONDITION:** MET. No new findings in r5; all BLOCKER findings are AGREED/RESOLVED; all MAJOR
> findings are AGREED/RESOLVED; all MINOR findings are AGREED/RESOLVED. Step 0 closes with two outputs:
> (1) the ratified hot-zone/prune live wiring contract; (2) X ruling **SR-C-04 = (B)**, meaning the
> implementation must add the independent checkpoint-retention trigger while leaving CMS §6.3 prose unchanged.
>
> Next free comment seq: `#0044`.

## REGISTER CONTROL BLOCK  (X keeps authoritative)

```text
Register status:        CLOSED (r5: C confirmed SR-C-04=B + all statuses; Step 0 exit condition met)
Current round:          r5
Next free comment seq:  #0044
...
MAJOR:   open 0 / total 7   (all AGREED/RESOLVED; SR-C-04/SR-D-05 resolved by X ruling B and C confirmation)
MINOR:   open 0 / total 2   (all AGREED)
NIT:     open 0 / total 0
EXIT condition met: YES — Step 0 closed by r5 C confirmation
```
...
### SR-D-05   [severity: MAJOR]   [status: RESOLVED]
...
### SR-C-04   [severity: MAJOR]   [status: RESOLVED]
...
[#0042 | C | r5 | agree]    Agree with X ruling option (B) and with the scoped implementation contract. I have no
                            single-activation safety objection to adding an independent checkpoint-flag-count trigger
                            inside the same combined helper, provided it is not a separate ad-hoc prune path: it must use
                            the same cache_mutex_ critical section, retire-before-select ordering, unified-slot composite
                            candidate machinery, pin/hot-zone/frame-0 exclusions, K-bounded detach, and free-after-unlock
                            discipline already ratified by SR-D-07. For the checkpoint-trigger reason, non-checkpoint
                            selection should be disabled; only checkpoint-flagged candidates above the retain target are
                            eligible, still honouring hot-zone distance ordering and pins. That closes the single-activation
                            implementation gap without weakening CMS §6.3 and remains forward-compatible with fmParallel
                            validation.
[#0043 | C | r5 | agree]    Agree with the full r4 findings-status table. No statuses are disputed: SR-D-01/02 and
                            SR-C-01/02 are agreed blockers; SR-D-03, SR-C-03, SR-C-05, and SR-C-06 are agreed; SR-D-04,
                            SR-D-05, SR-D-06, SR-D-07, and SR-C-04 are resolved; no MINOR/NIT item remains below
                            acknowledged disposition. No new SR-C findings are raised in r5. With #0042 and this status
                            confirmation, the Step 0 exit condition is met.
...
### SR-D-07   [severity: MAJOR]   [status: RESOLVED]
...
r4  D+X appended SR-D-07 closure + SR-C-04/SR-D-05 ruling B; C confirmation requested   seqs used: #0039-#0041
r5  C   confirmed ruling B and all statuses; no new findings; Step 0 exit met   seqs used: #0042-#0043
```
...
Live hot-zone/prune wiring contract:
1. record_hot_zone_observation(N) at arInitial before any arAllFramesReady store-prune;
2. use a new combined locked helper/wrapper, not raw insertion into store_owned_frame_locked;
3. pre-reserve vectors and compute conservative cached-output byte estimate outside cache_mutex_;
4. inside one cache_mutex_ scope: store/adopt -> set is_checkpoint flag if applicable -> pin/record only
   for AS2 consumer stores -> lazily retire decay-eligible zones -> evaluate prune trigger(s) -> select/detach;
5. unlock, then free duplicate losers and detached prune victims outside the lock;
6. target/frame0 production outputs are not pinned; they are protected by arInitial hot-zone membership;
7. single-activation wiring now; concurrent prune/observation deferred to fmParallel but no shortcut that would
   require restructuring is permitted;
8. temporary KDT is acceptable for live proof observability before the formal D-SUM diagnostics arc.

SR-C-04 / SR-D-05 X ruling:
Option (B) adopted. Add an independent checkpoint-flag-count retention trigger
(checkpoint_count > CHECKPOINT_MAX_RETAIN) as a new proven primitive within the same combined helper discipline.
CMS §6.3 prose remains unchanged; MAX_RETAIN is a real bound. There is no checkpoint pool/container: checkpoint is
an is_checkpoint flag on unified slots_ plus a flagged-position index and retention counts.
```

## CMS CHANGES PROPOSED BY THIS REVIEW (if any — a legitimate Step 0 output)

```text
No CMS §6.3 weakening or prose change adopted. The review output is an implementation/proof obligation:
add the independent checkpoint-retention trigger so the implementation matches the existing CMS intent.
```
