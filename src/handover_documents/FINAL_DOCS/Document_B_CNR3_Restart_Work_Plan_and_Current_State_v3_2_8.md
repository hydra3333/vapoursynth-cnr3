...
**Version:** v3.2.8 (RESUME-state work plan; focused status update. The version label is
kept at the "3.2" generation to stay aligned with Document A v3.2; the `.8` patch level
marks this update. Records the **real-frame pixel path as proven/committed through the
caller-supplied layer** — P.10A VapourSynth plane-view adapter, P.11A caller-supplied
frame-triplet validation, P.11B caller-supplied real-frame pixel composition, and P.11C
caller-supplied scene-change/reset — on top of the scalar→native bridge (P.7A–P.9A), the
complete scalar pixel pipeline (P.1A–P.6A), and the C.14A cache-core milestone; selftest
count is now **45/45**. The entire pixel path — from real VS frame memory through composition,
predecessor semantics, and scene-change — is now proven on CALLER-SUPPLIED frames. The next
phase is the **cache↔pixel / getFrame keystone** (the HARD DESIGNER GATE: where the proven
cache core meets the proven pixel chain, and predecessor sourcing / lifecycle / ownership
become load-bearing). It also records the refined **P.11A/P.11B/P.11C/keystone decomposition**
(see §8) and notes `cnr3_frame_processing.cpp` is now a member of BOTH the selftest and the
`cnr3` plugin projects. Earlier sections carry forward except the version pointers below.)  
**Date:** 2026-06-19  
