...
**Date:** 2026-06-19
**Version:** v2.5
**Supersedes:** Production Spec v2.4. Production Spec v2.4 superseded v2.3; v2.3 superseded
Production Spec v1.5, which governed continuity through the CMS06.x proof phases. v2.5
continues the **CMS07.0 restart** production-spec line: a clean architectural supersession,
not a continuation. The governing purpose changed at v2.3, so this remains a new version
line rather than an in-place edit of v1.5.
**v2.5 change (additive):** adds register rule **R-PROCESS-20** (Patch Delivery and Apply
Protocol) to §3A.5. No other rule, the canonical context (§3.2), or any section is changed
or removed. **Note on Document A lag:** the most recently generated Document A is v3.2,
which predates R-PROCESS-20 and therefore does not yet reproduce it. This is an intentional,
documented lag, not a defect: per R-PACK-02, on any Document A / §3A mismatch §3A (this
spec) is authoritative, so §3A is the controlling source for R-PROCESS-20 until Document A
is next regenerated (which will reproduce it). v2.4 had added R-PROCESS-19; v2.5 adds
R-PROCESS-20.
...
- the coder's own sandbox validation, including:
  ...
  ```
- the expected changed-files list;
