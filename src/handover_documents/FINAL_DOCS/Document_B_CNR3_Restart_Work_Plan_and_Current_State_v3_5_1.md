...
**Version:** Document B v3.5.1 (state advanced K.1F -> D.5; branch-(d) recovery arc COMPLETE; controlling CMS07.13 / Spec v2.11 / Document A v3.5). The v3.5.1 UPDATE block at the very top is the prevailing current-state record; all update blocks below it are retained as history of record (superseded). Per Production Spec §4, Document B is the volatile current-state document, re-issued each session against the prevailing phase; the durable scaffolding (working method, invariant disciplines, do-not-implement list, salvage inventory) is carried forward unchanged. v3.5 advanced K.1E.3 -> K.1F; v3.4 was a version bump over the v3.2.9.x generation.  
