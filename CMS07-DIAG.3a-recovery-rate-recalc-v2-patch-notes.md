# CMS07-DIAG.3a recovery-rate/recalculation v2 patch notes

Patch: `CMS07-DIAG.3a-recovery-rate-recalc-v2.patch`

## Replacement reason

This replaces `CMS07-DIAG.3a-recovery-rate-recalc.patch`.

Designer review D-1 found that the D-SUM-12 destroy observer was defined but not invoked in the production teardown path. v2 applies the requested Option A fix: `cnr3_discard_frame_data_with_cache()` now invokes `cnr3_diag_live_observe_recovery_plan_destroyed_if_needed(*request_data)` immediately before `delete request_data;`, under `CNR3_DIAG_COMPUTE_DSUM12_RECOVERY_PLAN`.

This is the single teardown site for frameData, so it covers success and bail paths. The observer's branch/stats-pointer guard makes it a no-op for non-recovery frameData.
...
- D-1: the D-SUM-12 destroy observer is now invoked at the single production teardown site before `delete request_data;`.
...
git apply --check CMS07-DIAG.3a-recovery-rate-recalc-v2.patch
git apply --check --whitespace=error CMS07-DIAG.3a-recovery-rate-recalc-v2.patch
git apply CMS07-DIAG.3a-recovery-rate-recalc-v2.patch
git diff --check
```
...
3. S-series `-r 1`:
   - S1/S3/S7/S8
   - `recovery_plan_balance == 0` on all four. This is the specific D-1 regression check.
